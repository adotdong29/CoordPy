"""W59 M8 — Multi-Hop Translator V9.

Strictly extends W58's ``coordpy.multi_hop_translator_v8``. V9
adds:

* **14 backends** (A..N) — V8 had 12. Edge count: 14 × 13 = 182
  directed edges.
* **Chain-length 13** transitivity probes.
* **Four-axis trust composite** —
  ``substrate × hidden × attention × retrieval``. An adversary
  must now corrupt all four axes in agreement to forge a
  compromise; corrupting any single axis collapses the composite.

V9 strictly extends V8: when ``retrieval_trust = 1.0`` is supplied
everywhere, V9's composite reduces to V8's three-axis composite.

Honest scope
------------

* The 14 "backends" are still *named*, not *executed*. This is a
  graph + trust arbiter, not a multi-machine harness. The W59
...
W59_MH_V9_SCHEMA_VERSION: str = (
    "coordpy.multi_hop_translator_v9.v1")
W59_DEFAULT_MH_V9_BACKENDS: tuple[str, ...] = (
    "A", "B", "C", "D", "E", "F", "G",
    "H", "I", "J", "K", "L", "M", "N")
W59_DEFAULT_MH_V9_CHAIN_LEN: int = 13
W59_DEFAULT_MH_V9_AGREEMENT_FLOOR: float = 0.3
...
def four_axis_trust_arbitration(
        *, paths: Sequence[DecBackendChainPathV9],
        agreement_floor: float = (
            W59_DEFAULT_MH_V9_AGREEMENT_FLOOR),
        trust_floor: float = 0.1,
) -> tuple[list[float], dict[str, Any]]:
    """V9 arbitration: four-axis trust composite + greedy
    agreeing subset."""
    if not paths:
        return [], {
            "n_paths": 0,
...
class MultiHopV9Witness:
    ...
    def cid(self) -> str:
        return _sha256_hex({
            "kind": "multi_hop_v9_witness",
            "witness": self.to_dict()})


def evaluate_dec_chain_len13_fidelity(
        *, backends: Sequence[str] = W59_DEFAULT_MH_V9_BACKENDS,
        chain_length: int = W59_DEFAULT_MH_V9_CHAIN_LEN,
        payload_dim: int = 8,
        seed: int = 590720,
) -> MultiHopV9Witness:
    import random
    ...
    paths: list[DecBackendChainPathV9] = []
    ...
    for pi in range(4):
        ...
        paths.append(DecBackendChainPathV9(
            chain=chain,
    ...
    out, info = four_axis_trust_arbitration(paths=paths)
    payload_l2 = math.sqrt(sum(v * v for v in out))
    return MultiHopV9Witness(
        schema=W59_MH_V9_SCHEMA_VERSION,
        backends=backends_t,
...
def emit_multi_hop_v9_witness(
        backends: Sequence[str] = W59_DEFAULT_MH_V9_BACKENDS,
        *,
        chain_length: int = W59_DEFAULT_MH_V9_CHAIN_LEN,
        seed: int = 590720,
) -> MultiHopV9Witness:
    return evaluate_dec_chain_len13_fidelity(
        backends=backends,
        chain_length=int(chain_length),
        seed=int(seed))
...
__all__ = [
    "W59_MH_V9_SCHEMA_VERSION",
    "W59_DEFAULT_MH_V9_BACKENDS",
    "W59_DEFAULT_MH_V9_CHAIN_LEN",
    "W59_DEFAULT_MH_V9_AGREEMENT_FLOOR",
    "DecBackendChainPathV9",
    "MultiHopV9Witness",
    "four_axis_trust_arbitration",
    "evaluate_dec_chain_len13_fidelity",
