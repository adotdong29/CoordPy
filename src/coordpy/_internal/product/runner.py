...
Usage (as a module):
    ...
    coordpy --profile local_coder \\
            --out-dir /tmp/coordpy_run/coder
...
from coordpy._internal.product import report as _report
...
def _enforce_trust(prof: dict, profile_name: str,
    ...
    if "docker" in sandboxes:
        from coordpy._internal.tasks.swe_sandbox import DockerSandbox
...
def _run_profile_capsule_native(profile_name: str, *,
    ...
    """
    from coordpy.capsule_runtime import CapsuleNativeRunContext
    from coordpy.llm_backend import backend_from_config
    from coordpy.provenance import build_manifest
    from coordpy.runtime import (
