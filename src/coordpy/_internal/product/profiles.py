...
Canonical profiles (stable API surface):
  ...
  * ``local_coder``        — local Ollama, qwen2.5-coder:14b.
  * ``local_frontier``     — local Ollama, qwen3.5:35b.
  * ``public_jsonl``       — template; operator swaps ``jsonl`` at call.

All real-LLM profiles default to ``http://localhost:11434``; point
them at any Ollama server with the ``COORDPY_OLLAMA_URL`` env var.
"""
...
_PROFILES: dict[str, dict[str, Any]] = {
    ...
    "local_coder": {
        "description": (
            "Local Ollama qwen2.5-coder:14b real-LLM sweep, 57 "
            "instances, strict+robust parser, raw capture on. "
            "Canonical coder-class cell. Override the endpoint with "
            "COORDPY_OLLAMA_URL."),
        "readiness": {
            "jsonl": _BUNDLED_BANK,
            "limit": None,
    ...
    "local_frontier": {
        "description": (
            "Local Ollama qwen3.5:35b real-LLM sweep, 57 instances, "
            "strict+robust parser, raw capture on. Canonical "
            "frontier cell. Override the endpoint with "
            "COORDPY_OLLAMA_URL."),
        "readiness": {
            "jsonl": _BUNDLED_BANK,
            "limit": None,
    ...
    "local_coder_70b": {
        "description": (
            "Frontier slot: a ≥70B coder-finetuned model on a local "
            "Ollama server. The slot is declared so adding the model "
            "is a pure config change — populate ``sweep.model`` with "
            "the Ollama tag (e.g. ``qwen2.5-coder:70b`` or "
            "``deepseek-coder-v3:70b``) once the model is resident, "
            "and set COORDPY_OLLAMA_URL if it is not on localhost."),
