...
_STABLE_PUBLIC: tuple[str, ...] = (
    # ADK — the library-first product front door (the primary surface).
    "adk",
    # Run model
...
# W146 — the wildcard / tooling export surface is the curated stable
# set (``_STABLE_PUBLIC``). Research / experimental names remain
# importable as ``coordpy.<name>`` (see ``__experimental__``) but are
# intentionally absent from ``from coordpy import *`` and ``dir(coordpy)``.
