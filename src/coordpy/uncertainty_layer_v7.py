"""W59 M15 — Uncertainty Layer V7.

Strictly extends W58's ``coordpy.uncertainty_layer_v6``. V7 adds
a **sixth confidence weighting axis**: ``retrieval_fidelity`` (in
``[0, 1]``). The W59 composite is now
...
sensitivity (now 6 axes) carry forward from V6.

V7 reduces to V6 byte-for-byte when ``retrieval_fidelities`` is
all 1.0 (the sixth axis becomes a no-op).
"""
...
W59_UNCERTAINTY_V7_SCHEMA_VERSION: str = (
    "coordpy.uncertainty_layer_v7.v1")
...
class RetrievalAwareWeightedComposite:
    composite: float
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema": W59_UNCERTAINTY_V7_SCHEMA_VERSION,
...
def compose_uncertainty_report_v7(
        *,
...
) -> RetrievalAwareWeightedComposite:
    if not component_confidences:
        return RetrievalAwareWeightedComposite(
            composite=1.0,
    ...
    cfs = [
        ...
        for k in keys]
    rfs = [
        float(retrieval_fidelities.get(k, 1.0))
        for k in keys]
    composite = float(
        sum(cs[i] for i in range(len(cs)))
        / float(len(cs)))
    weighted = _weighted_composite_v7(
        cs, ts, sfs, hfs, cfs, rfs)
    eps = max(0.0, float(adversarial_radius))
    pess = _weighted_composite_v7(
        [max(0.0, c - eps) for c in cs],
        ts, sfs, hfs, cfs, rfs)
    opti = _weighted_composite_v7(
        ...
        for i in range(len(cs)):
            ...
            if axis_idx == 0:
                ...
                rfs2[i] = float(max(0.0, min(
                    1.0, rfs[i] + delta)))
            perturbed = _weighted_composite_v7(
                cs2, ts2, sfs2, hfs2, cfs2, rfs2)
...
@dataclasses.dataclass(frozen=True)
class UncertaintyLayerV7Witness:
    schema: str
    composite_cid: str
    n_components: int
    hidden_aware: bool
    ...
    def cid(self) -> str:
        return _sha256_hex({
            "kind": "uncertainty_v7_witness",
            "witness": self.to_dict()})


def emit_uncertainty_v7_witness(
        *, composite: RetrievalAwareWeightedComposite,
) -> UncertaintyLayerV7Witness:
    p_le_w = bool(
    ...
    return UncertaintyLayerV7Witness(
        schema=W59_UNCERTAINTY_V7_SCHEMA_VERSION,
...
__all__ = [
    "W59_UNCERTAINTY_V7_SCHEMA_VERSION",
    "RetrievalAwareWeightedComposite",
    "UncertaintyLayerV7Witness",
