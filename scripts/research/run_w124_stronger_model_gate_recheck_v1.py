"""W124 Lane γ — stronger-model cutoff-disclosure gate re-check (NIM-free, read-only).
...
primary-KNOWN-and-certifiable enough to supersede Maverick on the matched ICPC
family. Confirms the decision CID is invariant (``258b6ed7…``) and the
{KNOWN, UNKNOWN} split is unchanged. Emits
results/w124/stronger_model_gate/gate_recheck_v1.json.

W124 note: even if a stronger model were primary-KNOWN, Lane α earned NO hosted
probe (the local transformer-native mechanism did not show a translatable gain),
so the hosted target is moot for W124 regardless — this re-check documents the
gate state, it does not gate any spend this milestone.
"""
...
OUT_DIR = os.path.join(ROOT, "results", "w124", "stronger_model_gate")
...
def main() -> int:
    ...
    # GLM-5 is tracked as a known-uncertifiable 5th model in the W118+ narrative
    # (no primary card disclosing a cutoff); it is UNKNOWN-from-primary.
    glm5_unknown_from_primary = True
    verdict = {
        "schema": "coordpy.w124_stronger_model_gate_recheck.v1",
        ...
        "w124_hosted_spend_moot": True,
        "w124_hosted_spend_moot_reason": (
            "Lane α earned no hosted probe (no translatable local mechanism gain), "
            "so the hosted target is moot for W124 independent of the gate."),
        "nim_spend": 0,
