"""W92 cross-modal role-specialized bench driver — NIM
90B-Vision VLM (Planner + Verifier) + NIM Llama-3.3-70B-Instruct
text-LM (Implementer).

Three arms on HumanEval-Visual all_docstring corpus:
  A0_text:       text-only single-shot (no image)
  A1_vlm:        single-agent VLM, K=5 independent first-pass
  B_role_spec:   VLM-Planner + Code-Implementer-x3 + VLM-Verifier
                 (K=5 budget; 2× VLM + 3× code-LM)
"""
...
def _make_nim_text_gen(model_id, *, api_key, timeout=240.0):
...
def _make_nim_vlm_gen(model_id, *, api_key, timeout=240.0):
    url = "https://integrate.api.nvidia.com/v1/chat/completions"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=("W92 cross-modal role-specialized bench"))
    parser.add_argument(
    ...
    parser.add_argument(
        "--strip-mode", default="all_docstring",
    ...
    safe_text = text_model_id.replace("/", "_").replace(":", "_")
    timestamp = datetime.now(timezone.utc).strftime(
        "%Y%m%dT%H%M%SZ")
    run_dir = (
        Path(args.out_dir)
        / f"w92_cmrs_{args.strip_mode}_"
    ...
    report, _corpus = run_cross_modal_role_specialized_bench_v1(
    ...
    print(
        f"B_role_spec   mean pass@1: "
        f"{report.b_role_spec_mean_pass_at_1:.4f}")
    print(
        f"B_role_spec beats A0_text per seed: "
        f"{report.b_role_spec_beats_a0_text_per_seed}")
    print(
        f"B_role_spec beats A1_vlm  per seed: "
        f"{report.b_role_spec_beats_a1_vlm_per_seed}")
    print(
        f"B mean strictly > A0_text mean: "
        f"{report.b_role_spec_mean_strictly_beats_a0_text_mean}")
    print(
        f"B mean strictly > A1_vlm  mean: "
        f"{report.b_role_spec_mean_strictly_beats_a1_vlm_mean}")
    print(
        f"B − A0_text: "
        f"{report.b_role_spec_mean_minus_a0_text_mean_pp:+.2f} pp")
    print(
        f"B − A1_vlm:  "
        f"{report.b_role_spec_mean_minus_a1_vlm_mean_pp:+.2f} pp")
    print(
        f"bench Merkle root: "
