"""W130 Lane γ — stronger-model cutoff-disclosure gate re-check (NIM-free, read-only).
...
W129.  Confirms the decision CID is invariant (``258b6ed7…``) and the {KNOWN:1, UNKNOWN:4} split.
The W130 spend gate is Lane β (the EXPOSED hard-cluster GENERATOR dev bench) + Lane γ T1∧T2, NOT
this cutoff gate.  Emits results/w130/stronger_model_gate/gate_recheck_v1.json.
"""
...
OUT_DIR = os.path.join(ROOT, "results", "w130", "stronger_model_gate")
...
def main() -> int:
    ...
    verdict = {
        "schema": "coordpy.w130_stronger_model_gate_recheck.v1",
        ...
        "w130_spend_gate_is_lane_beta_dev_bench_and_gamma_t1_t2_not_cutoff_gate": True,
