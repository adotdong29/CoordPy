"""W128 Lane γ — stronger-model cutoff-disclosure gate re-check (NIM-free, read-only).
...
primary-KNOWN-and-certifiable enough to supersede Maverick on the matched ICPC family since
W127.  Confirms the decision CID is invariant (``258b6ed7…``) and the {KNOWN:1, UNKNOWN:4}
split (Maverick KNOWN Aug-2024; Qwen3-Coder-480B / DeepSeek-V4-pro / Mistral-Small-4-119B-2603 /
GLM-5 UNKNOWN-from-primary).  The W128 spend gate is Lane β (the EXPOSED hard-cluster dev bench)
+ Lane γ T1∧T2, NOT this cutoff gate.  Emits results/w128/stronger_model_gate/gate_recheck_v1.json.
"""
...
OUT_DIR = os.path.join(ROOT, "results", "w128", "stronger_model_gate")
...
def main() -> int:
    ...
    verdict = {
        "schema": "coordpy.w128_stronger_model_gate_recheck.v1",
        ...
        "w128_spend_gate_is_lane_beta_dev_bench_and_gamma_t1_t2_not_cutoff_gate": True,
