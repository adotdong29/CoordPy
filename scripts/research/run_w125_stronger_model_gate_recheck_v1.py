"""W125 Lane γ — stronger-model cutoff-disclosure gate re-check (NIM-free, read-only).
...
Confirms the decision CID is invariant (``258b6ed7…``) and the {KNOWN:1, UNKNOWN:4} split.

W125 note: the spend gate this milestone is Lane β (resistant headroom), NOT Lane γ; this
re-check documents the hosted-target state — Maverick remains the only certifiable target.
Emits results/w125/stronger_model_gate/gate_recheck_v1.json.
"""
...
OUT_DIR = os.path.join(ROOT, "results", "w125", "stronger_model_gate")
...
def main() -> int:
    ...
    verdict = {
        "schema": "coordpy.w125_stronger_model_gate_recheck.v1",
        ...
        "w125_spend_gate_is_lane_beta_not_gamma": True,
        "nim_spend": 0,
