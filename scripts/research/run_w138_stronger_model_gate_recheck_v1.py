"""W138 Lane γ — stronger-model cutoff-disclosure gate re-check (NIM-free, read-only).
...
For W138 this gate governs only WHICH model an EARNED frontier rerun would target — the generated
band field is resistant BY CONSTRUCTION (freshly minted, unmemorisable for any cutoff), so it does NOT
itself require a stronger model; the default frontier target stays ``meta/llama-3.3-70b-instruct``.
Emits results/w138/stronger_model_gate/gate_recheck_v1.json.
"""
...
OUT_DIR = os.path.join(ROOT, "results", "w138", "stronger_model_gate")
...
def main() -> int:
    ...
    verdict = {
        "schema": "coordpy.w138_stronger_model_gate_recheck.v1",
        ...
        "primary_recheck_note": (
            "Re-derived from the primary-source registry (invariant W114->W137): Llama-3.3-70B = "
        ...
        "gate_state": ("CLOSED — no primary-KNOWN stronger-than-Maverick model certifiable; the W138 "
                       "band field is resistant BY CONSTRUCTION so it does not require a stronger model; "
                       "an EARNED frontier rerun targets meta/llama-3.3-70b-instruct (KNOWN ~Dec-2023)"),
        "w138_note": ("W138 frontier is earned ONLY if Lane-alpha lands a >=2-mode/>=3-family band AND "
                      "the X1 lead arm clears the held-out §7b earn rule (beats A1 AND B0 by >=+5pp, "
                      "same-sign on >=2 tiers). The gate state here governs only WHICH model the earned "
                      "frontier targets (it stays Llama-3.3-70B); it does not by itself authorise NIM."),
        "w123_through_w137_caps_closed": True,
        "no_405b_run": True,
        "nim_spend": 0,
