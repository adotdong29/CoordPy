#!/usr/bin/env python3
"""W97 — RealWorldQA D2-B0 cheap NIM pilot runner.

Drives the pre-committed Phase 2 pilot (1 seed × 30 problems ×
K=5) for the W97 D2-B0 candidate on `lmms-lab/RealWorldQA`
test (765 problems; 2 SHA-anchored parquet shards).  Mirrors
`scripts/run_w95_mathvista_pilot.py` (Phase 2 path) on the W97
bench module.
...
Hard contract (locked in `docs/RUNBOOK_W97.md`):
  ...
  * slice = deterministic
    ...
    n_problems=30, corpus=test)` BEFORE any NIM call;
...
def _evaluate_phase2_gates(
    ...
    `docs/RUNBOOK_W97.md`.  Gate texts are byte-identical to W95
    / W96-A / W96-C Phase 2; only the arm name and slice seed
    change.
    """
...
def main() -> int:
    ...
    ap.add_argument(
        "--n-problems", type=int,
        default=int(os.environ.get("W97_N_PROBLEMS", "30")))
    ap.add_argument(
        "--n-seeds", type=int,
        default=int(os.environ.get("W97_N_SEEDS", "1")))
    ap.add_argument(
    ...
    if not api_key:
        raise SystemExit(
            "NVIDIA_API_KEY env var required for the W97 NIM "
            "pilot.")
    ...
    run_dir = (
        Path(args.out_dir)
        / f"w97_realworldqa_pilot_{scale_tag}_"
    ...
    print(f"[w97.pilot] run_dir={run_dir}")
    ...
    if shas != REALWORLDQA_TEST_EXPECTED_PARQUET_SHA256:
        ...
        return 3
    print(
        f"[w97.pilot] parquet shards SHA-anchored: "
        f"{[s[:8] for s in shas]} ({total_bytes} bytes)")
    print("[w97.pilot] decoding corpus …")
    ...
    print(
        f"[w97.pilot] corpus n_problems={len(corpus)} "
    ...
    # Pre-commit the slice pids per seed BEFORE any NIM call.
    ...
    for s in seeds:
        ...
        print(
            f"[w97.pilot] pre-committed slice (seed={s}): "
            f"{len(pids)} pids; slice_sha256="
            f"{slice_sha[:16]}…")
    (run_dir / "pre_committed_slice.json").write_text(
        json.dumps({
            "schema": "coordpy.w97_pre_committed_slices.v1",
            "n_seeds": int(n_seeds),
    ...
    print(
        f"[w97.pilot] starting bench: vlm={vlm_model_id} "
    ...
    (run_dir / "phase2_gates.json").write_text(
        json.dumps({
            "schema": "coordpy.w97_phase2_gates.v1",
    ...
    summary_lines.append(
        f"# W97 RealWorldQA Phase 2 pilot — "
        f"{run_dir.name}\n")
    ...
    verdict_label = (
        ...
        if overall else
        "FAIL — W97 D2-B0 Phase 2 KILLED at this scale; "
        "document W97-L-* carry-forward")
    summary_lines.append(
    ...
    print()
    print(
        f"[w97.pilot] total wall: {dt:.0f}s, "
        f"text={n_text} vlm={n_vlm}")
    print(
        f"[w97.pilot] A0_text    = "
        f"{report.a0_text_mean_pass_at_1 * 100.0:.2f}%")
    print(
        f"[w97.pilot] A1_vlm K=5 = "
        f"{report.a1_vlm_mean_pass_at_1 * 100.0:.2f}%")
    print(
        f"[w97.pilot] B_vlm_team = "
        f"{report.b_vlm_team_mean_pass_at_1 * 100.0:.2f}% "
        f"(B − A1 = "
        f"{report.b_mean_minus_a1_vlm_mean_pp:+.2f} pp; "
        f"B − A0 = "
    ...
    for g in gates:
        print(
            ...
            f"{'PASS' if g['pass'] else 'FAIL'} — "
            f"{g['summary']}")
    print(f"[w97.pilot] OVERALL: {verdict_label}")
