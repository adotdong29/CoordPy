...
#
# Usage:
#   ./scripts/release/release.sh build      # build sdist + wheel
#   ./scripts/release/release.sh check      # build, twine-check, run smoke driver
#   ./scripts/release/release.sh testpypi   # build + check + upload to TestPyPI
#   ./scripts/release/release.sh upload     # build + check + upload to real PyPI
#
...
# release.sh lives at scripts/release/, so the repo root is two up.
...
clean() {
    ...
    rm -rf dist build src/coordpy.egg-info src/coordpy_ai.egg-info \
           coordpy.egg-info coordpy_ai.egg-info
...
smoke() {
    ...
    # Stable SDK contract (legacy).
    "$venv/bin/python" tests/test_smoke_full.py
    # ADK product front door, exercised from the INSTALLED wheel.
    "$venv/bin/python" tests/test_adk_wheel_smoke.py
    "$venv/bin/python" -m coordpy.adk.examples.research_assistant > /dev/null
    # Console scripts ship and run.
    "$venv/bin/coordpy" --version
    "$venv/bin/coordpy-team" --help > /dev/null
...
verify_tag_matches_version() {
    ...
    local version
    version="$(grep -E '^__version__' src/coordpy/_version.py \
        | head -n 1 \
        | sed -E 's/.*"([^"]+)".*/\1/')"
    if [ -z "$version" ]; then
        echo "ERROR: could not parse __version__ from src/coordpy/_version.py" >&2
        exit 3
    fi
    local tag
    ...
    if [ -z "$tag" ]; then
        ...
        echo "       src/coordpy/_version.py is $version, so tag with:" >&2
