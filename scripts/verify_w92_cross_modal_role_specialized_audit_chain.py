"""W92 cross-modal role-specialized bench — offline audit verifier."""
...
def _verify_sidecar(path):
    ...
    for rec in calls:
        ...
        if (hashlib.sha256(p.encode("utf-8")).hexdigest().lower()
                != p_exp.lower()
                or hashlib.sha256(r.encode("utf-8")).hexdigest().lower()
                != r_exp.lower()):
            n_bad += 1
...
def main():
    ...
    if args.report:
        ...
        ptr = (
            ROOT / "results" / "w92"
            / "cross_modal_role_specialized" / "latest_run.txt")
        if not ptr.exists():
            raise SystemExit(
    ...
    b = float(report["b_role_spec_mean_pass_at_1"])
    d_a0 = float(report.get(
        "b_role_spec_mean_minus_a0_text_mean_pp", 0.0))
    d_a1 = float(report.get(
        "b_role_spec_mean_minus_a1_vlm_mean_pp", 0.0))
    n_seeds = int(report.get("n_seeds", 0))
    ba0 = list(report.get(
        "b_role_spec_beats_a0_text_per_seed", []))
    ba1 = list(report.get(
        "b_role_spec_beats_a1_vlm_per_seed", []))
    print()
    print(f"  A0_text       mean pass@1: {a0:.4f}")
    print(f"  A1_vlm        mean pass@1: {a1:.4f}")
    print(f"  B_role_spec   mean pass@1: {b:.4f}")
    ...
    s_a0 = bool(report.get(
        "b_role_spec_mean_strictly_beats_a0_text_mean", False))
    ...
    print()
    print("  W92 retirement bars vs "
          "W88-L-CROSS-MODAL-CODE-V1-SPLIT-NOT-LOAD-BEARING-CAP "
          "(role-specialized replacement):")
    check("(1) b > a0_text mean (image load-bearing)", s_a0)
    check("(2) b > a1_vlm mean (role-spec load-bearing)", s_a1)
    ...
    if n_fail == 0:
        ...
        print("OVERALL: FAIL")
    print(
        f"W92 cross-modal role-spec strong-retirement bar "
