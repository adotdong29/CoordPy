...
* **Stronger method (PART C).**
  ...
  ``vision_mvp/coordpy/team_coord.py``) is a **deterministic,
...
* The success condition (cohort_buffered vs FIFO, accuracy_full
  ...
  ``test_coordpy_cross_role_coherence.test_buffered_cohort_strictly_beats_fifo_at_K4``
...
> payloads.
>
> **Anchor.** ``test_coordpy_cross_role_coherence.CohortPolicyUnitTests::test_streaming_locks_to_first_admitted_tag``;
...
* **Not** a CoordPy product runtime contract change. The CoordPy
  ...
  ``vision_mvp.coordpy.team_coord`` (the multi-agent coordination
  research module).
...
* **`docs/RESULTS_COORDPY_CROSS_ROLE_COHERENCE.md`** *(this file)*.
...
```text
$ python3 -m unittest -v vision_mvp.tests.test_coordpy_cross_role_coherence
