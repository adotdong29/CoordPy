...
> capsule-native coordination move* in the CoordPy programme. Where
...
* New SDK method ``MultiRoundBundleDecoder`` in
  ``vision_mvp/coordpy/team_coord.py`` — multi-bundle, contradiction-
...
* **Not** "the runtime now needs multi-round capsule memory." The
  CoordPy single-run product runtime is unchanged. ``W11`` is
...
* New benchmark:
  ``vision_mvp/experiments/phase58_multi_round_decoder.py``.
* New tests:
  ``vision_mvp/tests/test_coordpy_multi_round_decoder.py`` — 19
...
* Doc updates:
  ...
  ``docs/RESULTS_COORDPY_MULTI_ROUND_DECODER.md`` (this file).
...
* **The CoordPy programme has three structural axes**
...
* Method: ``vision_mvp/coordpy/team_coord.py``
  (``MultiRoundBundleDecoder``)
* Tests: ``vision_mvp/tests/test_coordpy_multi_round_decoder.py``
* Prior milestone: ``docs/RESULTS_COORDPY_BUNDLE_DECODER.md``
