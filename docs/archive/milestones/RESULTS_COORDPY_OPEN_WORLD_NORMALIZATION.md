...
> normalisation move** in the CoordPy programme, and the **first
...
* **Not** "we solved multi-agent context." R-60-wide's win is
  *conditional* on (a) the R-58 delayed-causal-evidence shape, (b)
  ...
  W11-4).
* **Not** "CoordPy now generalises to open-world LLM drift." The W13
...
* **Not** "the runtime now needs the layered normalisation layer."
  The CoordPy single-run product runtime is unchanged. ``W13`` is
...
* Public surface:
  ``vision_mvp/coordpy/__init__.py`` — re-exports the W13 surface
  + bumps ``SDK_VERSION = "coordpy.sdk.v3.14"``.
* New benchmark:
  ``vision_mvp/experiments/phase60_open_world_normalization.py``.
* New tests:
  ``vision_mvp/tests/test_coordpy_open_world_normalization.py`` — 22
...
* Doc updates:
  ...
  ``docs/START_HERE.md``,
  ``docs/RESULTS_COORDPY_OPEN_WORLD_NORMALIZATION.md`` (this file).
...
* **The synthetic→real-LLM transfer story sharpens (and a fifth
  ...
  *open-world* drift inside the heuristic closure (W13-1,
  ...
  observation).
* **The CoordPy programme has five structural axes**
...
* Tests: ``vision_mvp/tests/test_coordpy_open_world_normalization.py``
* Prior milestone: ``docs/RESULTS_COORDPY_REAL_LLM_MULTI_ROUND.md``
