...
- **Anchor confirmation — cross-tier usefulness is FAMILY-DEPENDENT; SHARED cross-tier lift CONFIRMED on the overlap; strict ≥2-SHARED-family earn endpoint-blocked on cell-4.** The anchor (`meta/llama-3.3-70b-instruct`, the frontier target) probe ran on the SAME hard cells, and the per-cell A1 reveals that **the two tiers' needed-and-unknown technique sets DIFFER** (the falsifier-first A1≤0.34 gate makes this machine-checkable):

  | cell (technique) | 8B A1 | anchor A1 | both-lack (shared)? | anchor T2 |
  |---|---|---|---|---|
  | `subarrays_sum_and_range@30000` (two-deque) | 25% | **25%** | **YES — SHARED** | **100% (+75)**, T4 100% (+75) |
  | `sum_nearest_smaller_left@50000` (monotonic-stack) | 0% | **75% (knows it)** | no — 8B-only | dropped (substitute @ anchor) |
  | `max_j_minus_i_le@50000` (prefix/suffix) | 50% (knows it) | **0%** | no — anchor-only | **100% (+100)**, T4 100% (+100) |
  | `kth_smallest_pair_distance@20000` (binary-search-on-answer) | 0% | _endpoint-blocked_ | _pending_ | _endpoint-blocked_ |

  **The tutor lifts the FRONTIER 70B by +75–100pp on the 2 families it lacks** — `subarrays@30000` (two-deque) A1=25→100 (+75, both T2 and T4) AND `max_j@50000` (prefix/suffix) A1=0→100 (+100, both arms) — just as it lifts the 8B on the 3 families *it* lacks. **So the mechanism matters on BOTH tiers (the milestone's stated goal), and the anchor's OWN gain spans ≥2 families.** On the **SHARED-difficulty technique** (two-deque, the ONE both tiers lack) BOTH are at A1=25% AND BOTH lift +75pp = a **CONFIRMED cross-tier (anchor∧weak) teacher-compiled lift**. Cross-tier usefulness is **family-dependent**: the tutor lifts EACH tier on the techniques IT lacks (the anchor already knows monotonic-stack ⇒ weak-tier *democratization* there; the anchor lacks prefix/suffix where the 8B doesn't), and the cross-tier (same-family) lift holds on the overlap. The strict §7b **≥2-SHARED-family** earn needs a 2nd family BOTH tiers lack (cell-4 `kth`: the 8B lacks it; the anchor's A1 there was endpoint-blocked by NIM oscillation ~6.5s↔~30-36s/call). So: **anchor lifts ≥2 families, 8B lifts 3 families, cross-tier SHARED overlap = 1 confirmed (two-deque, +75 both tiers/both arms) + cell-4 endpoint-blocked.** On the 70B the controller `T4` lifts as well as `T2` (+75/+100, unlike the 8B where T4 was hampered) — its tutored code reliably passes the public samples so the revert doesn't mis-fire.
