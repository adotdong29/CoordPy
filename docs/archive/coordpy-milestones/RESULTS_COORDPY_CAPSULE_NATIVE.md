# RESULTS — CoordPy capsule-native runtime (SDK v3.1)
...
CoordPy run executed end-to-end as ordinary Python, the
...
divergence. The capsule layer is no longer a description of CoordPy;
it is now (partially) the **execution contract** CoordPy runs under.

---

## 1. The claim in one paragraph

> **Capsules drive the CoordPy runtime, not just describe it.** Every
...
Let $S$ be the runtime's execution stages for one CoordPy run:
...
the same as CoordPy's sandbox boundary. The in-flight register
...
- The bytes of $d$ being *honest* — the writer's correctness is
  the trust unit. CoordPy's substantive writers (readiness verdict,
...
independently audit-able via ``coordpy-capsule verify``.
...
- **CoordPy is a context-capsule runtime, in execution, not just in
...
- **The SDK v3.0 public surface is unchanged.** Every v3.0
  ...
  (``coordpy.capsule_view.v1``); the new ``construction`` field
...
- **The CoordPy runtime is fully capsule-native at every level.**
...
- ``vision_mvp/coordpy/run.py`` — ``RunSpec.capsule_native`` flag
  (default True).
- ``vision_mvp/tests/test_coordpy_capsule_native.py`` — 16 tests
  witnessing the four theorems.

Cross-references:
- The post-hoc fold's adapters
  (``vision_mvp/coordpy/capsule.py::capsule_from_*``,
