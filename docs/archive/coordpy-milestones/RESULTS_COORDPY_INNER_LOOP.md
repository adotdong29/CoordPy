...
> `vision_mvp/coordpy/capsule.py` (PROMPT, LLM_RESPONSE),
> `vision_mvp/coordpy/capsule_runtime.py` (`seal_prompt`,
> `seal_llm_response`),
> `vision_mvp/coordpy/runtime.py` (`_make_intra_cell_hooks`,
> `_seal_prompt_response_pair`, `_real_cells`, `_synthetic_cells`),
> `vision_mvp/coordpy/synthetic_llm.py` (`SyntheticLLMClient`,
> `SYNTHETIC_MODEL_PROFILES`),
> `vision_mvp/coordpy/lifecycle_audit.py` (L-9 / L-10 / L-11),
...
3. **Synthetic mode lets the inner-loop slice run in CI.**
   ...
   ``vision_mvp.coordpy.synthetic_llm``) instead of an Ollama
...
- It does **not** claim "CoordPy is now fully capsule-native." The
