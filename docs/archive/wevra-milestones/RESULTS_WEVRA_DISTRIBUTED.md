...
> load-bearing **between agents**; SDK v3.6 attaches Wevra to the
...
single-model path (§ 1) and shipped the smallest honest Wevra
...
**experimental infrastructure**, not product (§ 6); Wevra's
...
## 2. Wevra integration — the smallest honest boundary

### 2.1 What ships in SDK v3.6

A new module `vision_mvp/wevra/llm_backend.py`:
...
* **`MLXDistributedBackend`** — talks an OpenAI-compatible
  ...
  run. The Wevra adapter is **neutral on the sharding strategy**:
...
`vision_mvp/tests/test_wevra_llm_backend.py` — 9 tests covering:
...
> run lands at ``/tmp/wevra-distributed/real_cross_model_n10.json``).
> Reproducible with one command on any machine that can reach
> ``192.168.12.191:11434`` and has both Ollama models loaded.

> **W5-2 (proved, integration boundary).** The Wevra inner-loop
...
>
> **Anchor.** ``test_wevra_llm_backend.py::
...
brings up. Wevra does **not** ship `mlx`, `mlx-lm`, or `mpirun`
as dependencies; it does not auto-bring-up the cluster; it does
not manage MPI hostfiles or model-conversion pipelines. The
`docker` extra is at `pip install wevra[docker]`; there is
deliberately no `pip install wevra[mlx_distributed]` extra,
because Wevra does not own the cluster.

### 6.2 What Wevra ships

* **Wevra can use a stronger distributed-inference backend.**
  YES — via `run_sweep(..., llm_backend=MLXDistributedBackend(
  model=..., base_url=...))`.
* **Wevra ships a universal distributed-inference framework.**
  NO. The integration is one HTTP-client class.

### 6.3 What this means for users

* If you have one MLX-distributed endpoint up, you point Wevra
  at it. Done.
* If you have two Macs and want Wevra to bring up MLX
...
## 7. The Wevra single-run product runtime contract is unchanged
...
* `python3 -m unittest -v vision_mvp.tests.test_wevra_llm_backend`
  → **9 tests pass**.
...
* `python3 -m vision_mvp.experiments.parser_boundary_real_llm
  ...
  /tmp/wevra-distributed/real_cross_model.json`
  → **n=6 saturation** (W5-1 evidence).
* `python3 -m vision_mvp.experiments.parser_boundary_real_llm
  --n-instances 10 --max-tokens 320 --out
  /tmp/wevra-distributed/real_cross_model_n10.json`
