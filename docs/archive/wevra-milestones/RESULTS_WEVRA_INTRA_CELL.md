# RESULTS — Wevra intra-cell capsule-native execution + detached witness (SDK v3.2)
...
view); (ii) **stronger on-disk verification** by ``wevra-capsule
...
> primary ``root_cid`` and ``chain_head``. ``wevra-capsule verify``
...
a Wevra run; let $V$ be the rendered view of the primary ledger at
...
- **``wevra-capsule verify`` is no longer a header-trust check.**
...
| Adversarial concurrent-writer detection           | **Out of scope** (trust boundary is the same as Wevra's sandbox) |

---

## 6. Code anchors

- ``vision_mvp/wevra/capsule.py`` — three new
...
- ``vision_mvp/wevra/capsule_runtime.py`` — three new methods on
...
- ``vision_mvp/wevra/_cli.py`` — ``wevra-capsule verify`` now
