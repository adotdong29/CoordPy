...
> LLM transfer move* in the Wevra programme. SDK v3.7 attacked
...
* New SDK method ``RobustMultiRoundBundleDecoder`` in
  ``vision_mvp/wevra/team_coord.py`` — wraps W11
  ...
  (``vision_mvp.wevra.RobustMultiRoundBundleDecoder``).
...
* **Not** "the runtime now needs the normalisation layer." The
  Wevra single-run product runtime is unchanged. ``W12`` is
...
* New benchmark:
  ``vision_mvp/experiments/phase59_real_llm_multi_round.py``.
* New tests:
  ``vision_mvp/tests/test_wevra_real_llm_multi_round.py`` — 24
...
* Doc updates:
  ...
  ``docs/START_HERE.md``,
  ``docs/RESULTS_WEVRA_REAL_LLM_MULTI_ROUND.md`` (this file).
...
* **The Wevra programme has four structural axes**
...
* Method: ``vision_mvp/wevra/team_coord.py``
  (``RobustMultiRoundBundleDecoder``, :data:`CLAIM_KIND_SYNONYMS`)
* Tests: ``vision_mvp/tests/test_wevra_real_llm_multi_round.py``
* Prior milestone: ``docs/RESULTS_WEVRA_MULTI_ROUND_DECODER.md``
