...
> capsule-aware context packing move** in the Wevra programme, and
...
* The W15 layer is **not** a producer-protocol move, a
  ...
  *decoder-side* context-shaping intervention that runs *between*
  ...
  directly. The Wevra programme has now accumulated **seven**
...
* The Wevra single-run product runtime contract is byte-for-byte
  ...
  :mod:`vision_mvp.wevra.team_coord` only and is opt-in via the
...
The seven Wevra structural layers gate the column where each is
...
full programme regression in `test_wevra_*.py` (430/430 pass).
...
* **Not** "the runtime context is now token-bounded." The Wevra
...
* **Not** "every multi-hypothesis regime favours W15." The
  ...
  Wevra programme's next-axis open question).
...
The Wevra programme now has **seven** structurally-distinct
...
* Public surface:
  ``vision_mvp/wevra/__init__.py`` — re-exports the W15 surface +
  bumps ``SDK_VERSION = "wevra.sdk.v3.16"``.
* New benchmark:
  ``vision_mvp/experiments/phase62_attention_aware_packing.py``.
* New tests:
  ``vision_mvp/tests/test_wevra_attention_aware.py`` — 37 tests
...
* Doc updates:
  ...
  ``docs/RESULTS_WEVRA_ATTENTION_AWARE.md`` (this file).
...
* **The Wevra programme has seven structural axes** with named
...
* Tests: ``vision_mvp/tests/test_wevra_attention_aware.py``
* Prior milestone: ``docs/RESULTS_WEVRA_PRODUCER_AMBIGUITY.md``
