...
* **Backward-compat preserved byte-for-byte.**
  ...
  * **W23-3-B** (full programme regression). 703 prior wevra-anchor
    + capsule + recent-phase tests pass before the W23 milestone
    landed; **742 / 742** wevra-suite tests pass after (+ 39 new
...
``vision_mvp/wevra/team_coord.py`` (``SessionDigestEnvelope`` /
...
* **W23-3-B** *(proved-empirical full programme regression,
  742 / 742 wevra-anchor + capsule + recent-phase tests pass)*. On
...
| W23-3-B (regression) | ``test_wevra_*.py`` + ``test_phase69_*`` + ``test_phase70_*``; 742 / 742 pass |
...
* Prior milestone (SDK v3.23 W22):
  `docs/RESULTS_WEVRA_CAPSULE_LATENT_HYBRID.md`.
