...
* **Backward-compat preserved byte-for-byte.**
  ...
  * **W22-3-B** (full programme regression). 633 / 633 prior
    coordpy tests pass before the W22 milestone landed; **675 /
    675** coordpy-suite tests pass after (+ 32 new W22 tests + 10
    misc).
...
``vision_mvp/coordpy/team_coord.py`` (``SchemaCapsule`` /
...
* **W22-3-B** *(proved-empirical full programme regression,
  675 / 675 coordpy-suite tests pass)*. On R-54..R-68 default banks,
...
| W22-3-B (regression) | ``test_coordpy_*.py`` full suite; 675 / 675 pass |
