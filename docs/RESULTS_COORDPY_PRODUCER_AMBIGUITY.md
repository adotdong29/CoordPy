...
> move** in the CoordPy programme, and the **first real-Ollama benchmark
> in which a CoordPy cross-round capsule decoder produces a strict
...
* **W14-1 strong success on R-61 synthetic + R-61-OLLAMA-A on real
  Ollama.** The new
  :class:`vision_mvp.coordpy.team_coord.StructuredProducerProtocol`
...
* **W14-3 backward-compat preserved.** The W14 protocol surface is
  ...
  CoordPy single-run product runtime contract is unchanged.
...
* The W14 protocol is a *boundary-side* intervention. The CoordPy
...
* The protocol preserves CoordPy's stable runtime sober. The CoordPy
...
The five CoordPy structural layers gate the column where each is
...
The CoordPy programme now has **six** structurally-distinct layers
...
* New tests:
  ``vision_mvp/tests/test_coordpy_producer_ambiguity.py`` — 27 tests
...
* Doc updates:
  ...
  ``docs/RESULTS_COORDPY_PRODUCER_AMBIGUITY.md`` (this file).
...
* **The synthetic→real-LLM transfer story is now complete (six
  ...
  *open-world* drift inside the heuristic closure (W13-1,
  ...
  empirical).
* **The CoordPy programme has six structural axes** with named limit
...
* Tests: ``vision_mvp/tests/test_coordpy_producer_ambiguity.py``
* Prior milestone: ``docs/RESULTS_COORDPY_OPEN_WORLD_NORMALIZATION.md``
