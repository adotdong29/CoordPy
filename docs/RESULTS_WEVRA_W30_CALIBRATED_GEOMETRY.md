...
| **H9** | Release-readiness clause | **PASS** — SDK_VERSION bumped to `wevra.sdk.v3.31`, `__experimental__` updated with W30 symbols, pyproject.toml 0.5.4, CHANGELOG entry added. |
| **H10** | Focused regression green | **PASS** — 36/36 W30 unit tests + 273/273 phase69-77 tests + 84/84 wevra_team_coord/runtime/public_api/extensions/provenance tests pass. |
...
* **MODIFIED**: `vision_mvp/wevra/team_coord.py` — appended ~1100
...
* **MODIFIED**: `vision_mvp/wevra/__init__.py` — added W30 exports
  under `__all__`, added W30 entries to `__experimental__`, bumped
  `SDK_VERSION` to `wevra.sdk.v3.31`.
...
* **NEW**: `docs/RESULTS_WEVRA_W30_CALIBRATED_GEOMETRY.md` — this
  file.
...
* `pytest vision_mvp/tests/test_wevra_team_coord +
  test_wevra_capsule_native + test_wevra_runtime + test_wevra_public_api +
  test_wevra_extensions + test_wevra_provenance` — **84/84 PASS** in 12s.
