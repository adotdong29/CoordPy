## W132 (resistant-by-construction hard-family battlefield + exact-oracle gates — CoordPy MINTED a 33-problem resistant battlefield that earned the Maverick pilot, but the pilot is NIM-infra-blocked this session; no third retirement) — explicit do-not-overstate

**DO say:** "W123 bounded the OFFICIAL resistant-package supply and W131 found the model axis blocked on cutoff DISCLOSURE; both share one upstream dependency — a contamination-RESISTANT battlefield we either inherit or certify against a disclosed cutoff. W132 REMOVED that dependency by MINTING the battlefield itself. `coordpy.resistant_by_construction_battlefield_v1` + `_slate_v1` mint a CoordPy-owned battlefield of **33 admitted** (>=30) resistant-by-construction problems targeted at the failure families that beat the stack (WRONG_ALGORITHM_ADMISSIBLE / HIDDEN_EDGE_STATE_MISS / COMPLEXITY_BLIND / SEARCH_ENUM). Each problem ships an exact executable answer-key oracle (`ref_source`) cross-checked by an INDEPENDENT exhaustive `brute_source`, an admissible-wrong `naive_source` trap enforced by a looks-right-on-public / fails-hidden discriminating gate, a near-duplicate + official-identity novelty guard, deterministic regeneration, and a content-addressed manifest (`manifest_cid 562aafbd...`, core 30-slice `f6a2ebed...`). The model under test sees ONLY the statement + public samples and is graded by the audited token-diff oracle (NO LLM judge); minted problems are emitted as `IcpcPilotProblemV1` so the *already-validated* W120 reflexion bench consumes them verbatim. The battlefield EARNED the Maverick pilot cleanly (>=30 + all quality gates + Maverick resistance-certified by date AND construction + eval rule locked). But the NVIDIA NIM endpoint was unreachable this session (the background calibration completed 1/66 calls before repeated 240s read-timeouts; isolated 32-token + 16-token Maverick probes both timed out), so the Maverick frontier pilot is **NIM-INFRA-BLOCKED / push-button** (`W132-L-MAVERICK-FRONTIER-PILOT-NIM-INFRA-BLOCKED`) — an infra outage, NOT a `FAIL` and NOT a science cap. A clearly-labelled DEV_ONLY local-Ollama run validated the beta pipeline end-to-end on the minted field. Stronger-model gate CLOSED (`258b6ed7` invariant). W89 (+5.56) + W105 (+7.00) STAND as the only two retirements; W132 retires none and adds `W132-T-RESISTANT-BY-CONSTRUCTION-BATTLEFIELD-MINTABLE` (the construction advance)."

**DO NOT say:**

* "the Maverick pilot ran / B beat A1 / W132 found a resistant margin" — the Maverick frontier pilot did NOT execute (NIM endpoint unreachable). No frontier B-A1 number exists. The pilot is wired + earned + push-button; it re-runs when the endpoint recovers.
* "W132 is a FAIL on resistant code / registers a resistant-by-construction pilot cap" — NO: `W132-L-RESISTANT-BY-CONSTRUCTION-PILOT-CAP` is reserved for an EXECUTED `FAIL`; an unrun pilot blocked by infra is an OPERATIONAL block, reported as such (the throughput-sibling of W131's CPU-bound local 32B).
...
* "resistance-by-construction proves the mechanism works on resistant code / solves the contamination confound" — it removes the *instrument-supply* (W123) and *cutoff-disclosure* (W131) blockers on the EXISTENCE of a resistant instrument; whether the same-budget mechanism beats A1 on the minted field is the push-button Maverick pilot, NOT yet run. W132 touches neither retirement nor the confound.
