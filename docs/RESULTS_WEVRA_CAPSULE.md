# RESULTS — Wevra SDK v3: the Context Capsule abstraction
...
Before v3, Wevra's top-line was *"bounded-context orchestration and
...
**Contract tests (all green).** `vision_mvp/tests/test_wevra_capsules.py`:
...
reference for a Wevra run, the way a Git commit SHA is a
...
Framing Wevra as a *context-capsule runtime* is a paradigm
...
| Claim | Type | Evidence |
|---|---|---|
| Capsules exist and satisfy C1..C6 | **Code-backed** | `vision_mvp/wevra/capsule.py` + `test_wevra_capsules.py` |
...
**What is new, claimed cautiously.**
  ...
  * The **end-to-end implementation** — the capsule graph lands on
    disk in a canonical schema (`wevra.capsule_view.v1`), is
    auditable offline via `wevra-capsule verify`, and the root
    `RUN_REPORT` CID is a durable public reference for a Wevra
...
**What the programme does NOT claim.**
  ...
  * The programme does not claim that all LLM-agent-team runtimes
    *must* centre on capsules. It claims that Wevra does, and that
...
After SDK v3, the product story has a mechanism-shaped centre:
  ...
  * **Wevra is a context-capsule runtime. The research results of
...
## 9. Code pointers (for another agent arriving cold)
  ...
  * Reference implementation: `vision_mvp/wevra/capsule.py`
    (~700 LOC, no external dependencies beyond the stdlib and
    the existing package).
  * Public surface: `vision_mvp/wevra/__init__.py` (SDK v3).
  * Runner integration:
  ...
  * CLI: `vision_mvp/wevra/_cli.py::_cmd_capsule` — subcommands
    `view`, `verify`, `cid`. Wired as `wevra-capsule` in
    `pyproject.toml`.
  * Contract tests: `vision_mvp/tests/test_wevra_capsules.py`
    (25 tests covering C1..C6 + the adapters + the CLI +
    end-to-end).
  * Public-API lock: `vision_mvp/tests/test_wevra_public_api.py`
    — `SDK_VERSION == "wevra.sdk.v3"`; any removal or rename of
