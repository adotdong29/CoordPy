...
regression pass** AND **68/68 wider wevra suite passes**.  The new
...
| **H9** | Release-readiness clause | **PASS** — SDK_VERSION bumped to `wevra.sdk.v3.33`, `__experimental__` updated with W32 symbols, pyproject.toml 0.5.6, CHANGELOG entry added; W32 is in experimental; stable runtime contract byte-for-byte unchanged. |
| **H10** | Focused regression green | **PASS** — 45/45 W32 unit tests + 437/437 phase69-78 tests + 45/45 phase79 tests + 68/68 wider wevra suite passes. |
...
* **MODIFIED**: `vision_mvp/wevra/team_coord.py` — appended ~880
...
* **MODIFIED**: `vision_mvp/wevra/__init__.py` — added W32 exports
  under `__all__`, added W32 entries to `__experimental__`, bumped
  `SDK_VERSION` to `wevra.sdk.v3.33`.
...
* **NEW**: `docs/RESULTS_WEVRA_W32_LONG_WINDOW_CONVERGENT.md` —
  this file.
...
* `pytest vision_mvp/tests/test_wevra_team_coord +
  ...
  test_wevra_provenance` — **68/68 PASS**.
