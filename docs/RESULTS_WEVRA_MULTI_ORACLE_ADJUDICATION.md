...
* **Backward-compat preserved byte-for-byte.**
  ...
  * **W21-3-B** (vs W20): With ``quorum_min = 1`` AND a single
  ...
  Full suite regression: **585 / 585 wevra tests pass** before the
...
``vision_mvp/wevra/team_coord.py`` (``OracleRegistration`` /
...
* **W21-3-A** *(proved-empirical full programme regression,
  ...
  suites (442 prior wevra tests + 40 W20 + 48 W21 = 633 total).
...
* ``vision_mvp/wevra/__init__.py`` — ``SDK_VERSION = "wevra.sdk.v3.22"``.
...
python3 -m unittest vision_mvp.tests.test_wevra_multi_oracle_adjudication
