...
> composition move** in the CoordPy programme — pairing the W14
...
The CoordPy programme now has **eight** structurally-distinct moves
named one-by-one:
...
The eight CoordPy structural layers gate the column where each is
...
full programme regression in ``test_coordpy_*.py`` (442/442 pass).
...
* **Not** "the composition layer is the runtime contract." The
  CoordPy single-run product runtime contract is byte-for-byte
...
* New tests:
  ``vision_mvp/tests/test_coordpy_composed.py`` — 22 tests covering
...
* SDK version bump:
  ``vision_mvp/coordpy/__init__.py`` — ``SDK_VERSION = "coordpy.sdk.v3.17"``.
...
* Doc updates:
  ...
  ``docs/RESULTS_COORDPY_COMPOSED_REAL_LLM.md`` (this file),
...
* **Joint correctness + decoder-side context efficiency under
  ...
  *both* the W14 prompt-discipline ingredient AND the W15
  ...
  bar 13.
* **The CoordPy programme has eight structural axes** with named
...
* Tests: ``vision_mvp/tests/test_coordpy_composed.py``
* Prior milestones:
  ``docs/RESULTS_COORDPY_ATTENTION_AWARE.md`` (W15),
  ``docs/RESULTS_COORDPY_PRODUCER_AMBIGUITY.md`` (W14)
