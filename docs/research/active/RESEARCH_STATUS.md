...
## Long-term programme goal

This is the eventual architecture objective, not a claim about what the
programme has already achieved:

**Can CoordPy turn the mechanism stack it has been testing — memory,
routing, verification, iterative feedback, and auditable coordination —
into a coordination-native neural architecture that outperforms plain
transformers on hard, contamination-resistant tasks at matched
compute?**

## Architecture North Star

*** FUTURE-AGENT NOTE: this is the canonical architecture-level takeaway
from the full W1->W143 arc. ***

**Canonical mechanism name: `Latent State Transition`.**

If attention was the defining primitive of transformers, **latent state
transition** should be the defining primitive of a CoordPy-style
replacement.

The deepest lesson so far is not "bigger context windows," "bounded
context," or a prompt/tool trick. It is that a transformer is too
sequence-centric: it repeatedly recomputes over token history, while the
replacement target should be state-centric. The primary computation of a
post-transformer CoordPy-style model should be a learned **state
transition** over a long-lived structured latent state, with routing,
verification, memory updates, and reuse all happening on that state.
Tokens are observations into the system, not the system's true memory.

In short: the architectural shift is from **attention over sequence** to
**latent state transition over persistent causal state**.
...
> multi-agent retirement on this amortizable-family line. W89 and W105
> remain the only two MULTI-AGENT retirements; W142b remains the
> distinct discover-then-amortize retirement.
> `docs/HONEST_FRAMING_POST_W87.md`. Last touched: **W142b — the §7b earn EARNED (operator-directed iterate-on-fail): the FIRST CONFIRMED discover-then-amortize two-seed 2-mode same-budget superiority — and W142-L is OVERTURNED. The W142 cap's three Lane-β walls were FIXABLE no-oracle verifier/extractor engineering artifacts, NOT a fundamental bound: `coordpy/no_oracle_verifier_v2.py` (multi-brute cluster-with-a-brute on a constraint-covering bank, small deterministic clustering bank) fixes the count_pairs FN + the subarrays FP; an additive ACCEPT-PREDICATE extension to `self_tutoring_technique_extractor_v1` makes the counting-by-condition technique extractable (`count += 1` ⇒ blank the accept condition; controls still rejected); a MULTI-WINNER extraction loop (try every verified-correct winner, not just the committed one) + K_b=12 multi-brute tame the per-seed stochasticity. RESULT (NIM, `meta/llama-3.3-70b-instruct`, K_a=4, K_d=24 one-time amortized, two seeds): seed1 count_pairs(COMPLEXITY) ST10/B09 + subarrays(HIDDEN_EDGE) ST7/B04/NEG2 ⇒ ST−B0=+23.5pp; seed2 ST10/B09 + ST6/B02/NEG1 ⇒ +31.2pp — BOTH seeds earn the §7b span (≥+5pp, 2 modes, NEG≤B0, ST>NEG); a post-restart re-run with a discovery-RETRY loop CONFIRMED the earn at the FULL pre-registered M=10 in BOTH seeds (s1r/s2r: subarrays ST7/B02 + ST9/B04, +30.0pp each — no truncation asterisk); a strict structurally-distant-alien NEG check (sum_nearest, non-counting) gives NEG_strict=3≈B0 ⇒ the subarrays lift is TECHNIQUE-CLASS-SPECIFIC (counting-by-accept-predicate), not generic-structural. PRECISE FRAMING: the FIRST confirmed §7b discover-then-amortize (no-oracle self-tutoring) resistant same-budget superiority — a mechanism-class DISTINCT from the W89 (+5.56) + W105 (+7.00) MULTI-AGENT retirements, which REMAIN the two multi-agent retirements (project now: 3 confirmed resistant same-budget superiority results across 2 classes). Insights: RELIABLE-B0-SATURATION (a reliable verifier strengthens B0 ⇒ ST's (1−p)^Ka edge resolves at low p [subarrays] and at adequate M for moderate p [count_pairs +10pp robust both seeds]); NO-ORACLE STOCHASTICITY IS TAMEABLE. Adds W142b-T-V2-VERIFIER-AND-EXTRACTOR-FIX-THE-THREE-W142-WALLS + W142b-T-DISCOVER-THEN-AMORTIZE-§7b-RETIREMENT-EARNED-TWO-SEED-2-MODE + W142b-T-RELIABLE-B0-SATURATION-AND-TAMEABLE-NO-ORACLE-STOCHASTICITY; W142-L OVERTURNED. `docs/RESULTS_W142b_DISCOVER_THEN_AMORTIZE_RETIREMENT_V1.md`; commits `91dde67`+`af608e8`; gate `258b6ed7` invariant; COO-9 lead; no version bump (0.5.20). — Prior W142 (SUPERSEDED — the cap was a verifier/extractor-engineering artifact, now OVERTURNED): moderate-`p` family screen + high-K discover reliability + ≥3-family retirement attempt: Lane α SUCCESS (the moderate-`p` band EXISTS, 3 veins / 2 modes — `count_pairs_sum` sort+two-pointer p=0.333, `subarrays_sum_and_range` two-deque HIDDEN_EDGE p=0.167, `sum_nearest` monotonic-stack p=0.500; all 4 NEW surfaces 0-supply ⇒ a THIN FIXED band, advancing W141's '1 family') but Lane β CAP — RETIREMENT NOT EARNED (ST−B0 = +0.0pp), each admitted family failing for a DISTINCT machine-diagnosed reason: `sum_nearest` B0-SATURATES at p=0.5 (no headroom), `count_pairs_sum` hits an S1 FALSE-NEGATIVE at low-`p` (K_d=48 ⇒ 0 winners committed despite the screen's 4/12 secret-pass — S1 reliability is self-brute-convention-sensitive), `subarrays` (the only 2nd mode) hits a VERIFIER FALSE-POSITIVE (W125 looks-right-fails-hidden: the sum-only naive agrees on the non-binding small bank, fails secret, non-extractable ⇒ discarded). ⇒ the discover-then-amortize conversion is VERIFIER-RELIABILITY + BASELINE-SATURATION bound (win-size and discovery-difficulty are COUPLED via `p`). The decisive $0 result: G3 gated-accumulator extractability ADMITS counting-pair + two-deque veins and REJECTS prefix-hash (technique in dict-maintenance) + binary-search-on-answer (printed answer is a reassignment) — the teaching-compiler representation R6′ is itself a binding constraint on R7′. `coordpy.{moderate_p_family_slate_v1, moderate_p_family_screen_v1, discover_amortize_accounting_v1}` + 2 drivers + `tests/test_w142_moderate_p_screen_v1.py`; `docs/{RUNBOOK_W142, RESULTS_W142_MODERATE_P_SCREEN_V1, ARCHITECTURE_REQUIREMENTS_W142_V4, FRONTIER_RELEVANCE_AUDIT_W142_V1}.md`; `slate_cid 9b8cf758`. Stronger-model gate re-confirmed CLOSED (`258b6ed7` invariant {KNOWN:1, UNKNOWN:4}; Maverick Aug-2024 SETTLED; Qwen3-Coder-480B/DeepSeek-V4-pro/Mistral-Small-4-2603/GLM-5 officially UNDISCLOSED; aggregator 'Mistral Nov-2024' non-primary, REJECTED). W89 (+5.56) + W105 (+7.00) STAND as the only two retirements; W142 retires none but converts W141's single-family observation into a SYSTEMATIC machine-diagnosed understanding + sharpens R7′. W143 = a low-`p`-robust no-oracle verifier (dual independent self-brutes + constraint-covering adversarial bank) + role-diverse discovery (W128) + a family strictly inside `p∈(0.1,0.4)`. Adds W142-T-MODERATE-P-FAMILY-SCREEN-MINTABLE + W142-T-SELF-TUTORING-EXTRACTABILITY-NARROWS-TO-GATED-ACCUMULATOR-VEINS + W142-T-MODERATE-P-BAND-IS-A-THIN-FIXED-SET-3-VEINS-2-MODES + W142-T-DISCOVER-THEN-AMORTIZE-FAILS-FOR-THREE-DISTINCT-REASONS-AT-THE-FRONTIER + W142-L-DISCOVER-THEN-AMORTIZE-CONVERSION-IS-VERIFIER-RELIABILITY-AND-BASELINE-SATURATION-BOUND. `258b6ed7` gate invariant; COO-9 lead; no version bump (0.5.20). — Prior milestone W141: emergent no-oracle SELF-TUTORING (self-derived scaffold from the team's OWN verified-correct sample): the programme's FIRST clean no-oracle contamination-resistant SAME-BUDGET superiority (+70.6pp on a family the frontier 70B lacks), but NOT a §7b retirement (1 moderate-p family < the ≥3-family/≥2-mode span; blocked by the BIMODAL-generation wall). W140 lifted the frontier with a scaffold compiled from the EXACT-ORACLE witness (teaching). W141 removes the oracle: the team generates K diverse → a NO-ORACLE verifier (S1 self-written-brute agreement on a CONSTRAINT-ADVERSARIAL bank ∧ S2 self-synthesized worst-case large-input timeout) picks the correct+efficient winner → the holed-skeleton scaffold is SELF-DERIVED from that winner (`coordpy.self_tutoring_technique_extractor_v1` — AST blanks the discriminating decision; `correct_fill` = the winner's own expressions ⇒ completable by construction; the W140 `tutor_leak_gate_v1` clears it via a new `spec_override`) → amortized across the family (`coordpy.{no_oracle_verifier_v1, self_tutoring_controller_v1}`; 3 modules + 2 scripts + offline analyzer + 10 tests). THREE ESTABLISHED results: (1) the DISCOVER loop works END-TO-END on the live frontier 70B with ZERO oracle — the first oracle-free technique mint on a frontier model; (2) the no-oracle verifier is BYTE-IDENTICAL to the hidden grader (0 false-positive / 0 false-negative on 115 REAL 70B samples; S1 catches 8/8 real fast-but-WRONG candidates the efficiency signal alone accepts — the W125 cap escape; the constraint-adversarial S1 bank gives 28/30 discrimination vs 0/30 public-only); (3) the self-derived scaffold lifts generation to q=100% (required a real-model extractor fix: INTER-PROCEDURAL return-tracing for models' helper-function shape — `result = solve(arr); print(result)` — after which 8/9 real 70B programs self-derive a clean leak-passing scaffold; buggy ones DISCARDED → KEEP ≡ A1, non-negativity structural). THE WIN: on `count_pairs_sum_le_t` — a resistant family the frontier 70B LACKS (raw efficient-rate p=0.08 under a FAIR self-consistency baseline) — the scaffold lifts 8%→100% = a **+70.6pp per-problem same-budget superiority** over no-oracle verified-selection, clean member-solve (ST 3/3 vs B 1/3), no oracle — the FIRST clean resistant same-budget win (W125–W130 got zero). THE WALL: NOT a §7b retirement because the amortization win needs a MODERATE-p band, but the frontier's generation is BIMODAL (the W137 wall, now confirmed at the GENERATION level): the 70B either KNOWS the technique (high-p — NSL 0.67, count_inversions 0.83, longest_bnd 1.0 → verified-selection saturates at K≥4, superiority <1.2pp) or is CLUELESS (count_subarrays/absdiff/max_j/kth/subarrays ≈ 0-supply → nothing to extract); exactly ONE of the tested slate (count_pairs_sum, p=0.08) fell in the moderate band ⇒ §7b ≥3-family span UNMET. (The amortization advantage also concentrates at LOW per-problem budget: ST−B = (1−p)^K, e.g. +33pp on NSL at K=1 but +1.2pp at K=4 — a generation-EFFICIENCY win, not a standard-budget superiority on high-p families.) Methodology: the FAIR baseline must name NEITHER technique NOR efficiency (any "efficient/time-limit/large-input" cue inflates p — NSL 0.32→0.92 — and masks the win). W89 (+5.56) + W105 (+7.00) STAND as the only two retirements. 3 new explicit-import leaf modules + 2 driver scripts + offline analyzer + 10 tests; no version bump (0.5.20 / coordpy.sdk.v3.43); no PyPI; `coordpy/__init__.py` untouched (one additive backward-compatible `spec_override` on `tutor_leak_gate_v1`). Adds W141-T-NO-ORACLE-SELF-TUTORING-DISCOVER-WORKS-ON-THE-FRONTIER + W141-T-NO-ORACLE-VERIFIER-IS-BYTE-IDENTICAL-TO-THE-HIDDEN-GRADER + W141-T-SELF-DERIVED-SCAFFOLD-LIFTS-GENERATION-TO-Q-100PCT + W141-T-EXTRACTOR-REQUIRES-INTERPROCEDURAL-RETURN-TRACING + W141-T-FIRST-CLEAN-NO-ORACLE-RESISTANT-SAME-BUDGET-SUPERIORITY + W141-L-RESISTANT-RETIREMENT-BLOCKED-BY-BIMODAL-FRONTIER-GENERATION (+ W141-T-SELF-TUTORING-IS-SUPPLY-LIMITED + W141-T-NEUTRAL-BASELINE-REQUIRED). W142 = a moderate-p frontier regime (a harder battlefield where the 70B is ~30–50% on ≥3 techniques / a higher discover-K for low-p reliability / the 8B democratization at scale) ⇒ the standard-budget ≥3-family retirement. `258b6ed7` gate invariant; COO-9 lead.** Previously: **W140 — teacher-compiled family-level witness tutors + weak-model lift + cross-tier USEFULNESS (the milestone's stated goal, ACHIEVED in the A1≈0 regime; NOT a third retirement). W139 left the blocker as weak-model witness USABILITY (the 8B's raw-witness usability was 0.00 — it could not act on the per-problem diagnostic). W140 compiled the anchor's witness into a family-LEVEL holed-skeleton TUTOR (`coordpy.family_tutor_compiler_v1`: TC1 card / TC2 witness→rewrite + holed skeleton / TC3 compressed / TC5 staged / T6 neg-control; the DISCRIMINATING logic is ALWAYS a HOLE the model must fill; `tutor_leak_gate_v1` falsifiable — planted ref-paste + discriminator-in-card BITE, grading on the disjoint hidden bank is the decisive guarantee; 27 tests) + `coordpy.cross_tier_tutor_bench_v1` (the T1–T6 same-budget slate + `measure_tutor_usability_v1` + the keep/apply/abstain tutor controller). THREE results: (1) the tutor REVERSES the W139 usability floor — per-tier tutor-usability ≥τ=0.34 on EVERY tier (8B 0.60/0.75, mid 1.00, strong 1.00 vs W139's 0.00/0.00/0.50; `tutor_usability_cid 849f1ebf`) ⇒ the 8B's failure was a TEACHING gap, not a capability wall; (2) on the original easy-for-8B families the dev shows cross-tier SAFETY BY CONTENT (the skeleton tutor T2 is non-negative vs A1 + beats blind reflexion +33–67pp, where the RAW witness C0 + the BARE card T1 HARM the 8B −66.7) but TIES A1 — a SUBSTITUTE regime (K=5 self-consistency already reaches the answer; the dev's mid/strong tiers crashed on a NIM HTTP-500, only the small tier survived); (3) refusing to settle on the tie, two $0 failure-analysis agents proved the tutor GENUINELY shifts the 8B's distribution to the technique (two-deque 0/60 self-generated → 24/24 with the holed skeleton on public), so the lift hides where A1≈0 — and the iter-2 FALSIFIER-FIRST probe (a cell qualifies only if A1≤0.34; 2 new hard technique specs `prefix_min_suffix_max_two_pointer` + `binary_search_answer_two_pointer`; a diversity-preserving split-K controller; leak gate hardened [hole-substance on the SECRET bank, fraction-of-ref paste, knob-N sanitized]; 4s grading W134-timeout-invariant) CONFIRMS a +75–100pp WEAK-MODEL lift on 3 families where the technique is needed-and-unknown: `subarrays_sum_and_range@30000` (two-monotone-deque) 25→100, `sum_nearest_smaller_left@50000` (monotonic-stack) 0→100, `kth_smallest_pair_distance@20000` (binary-search-on-answer, a conceptual reframe) 0→75; `max_j_minus_i_le@50000` (A1=50%, SUBSTITUTE regime) correctly DROPPED by the falsifier. The LEAD arm is T2 (always-apply scaffold); the W139-style controller T4 is dominated (its public-sample revert can't see a hidden-only bug — W125/W129). The mechanism is ADDITIVE iff A1≈0, SUBSTITUTIVE otherwise (the routing gate must condition on the per-ability A1 baseline, not only tutor-usability). Lane γ: `docs/ARCHITECTURE_REQUIREMENTS_W140_V3.md` adds R6′ (cross-capability teaching-compilation — a verifier a weak sub-module cannot act on must be COMPILED into a sub-module-consumable teaching object; suppression is a floor, not a ceiling; gate on the per-ability A1; always-apply the scaffold; the gate must see the HIDDEN failure); primary-source research (ACL'24 2404.17140 small-models-refine-but-can't-verify → carry the oracle; CHI'24 2404.02213 + ICLR'25 2410.09008 principle-only-misleads / two-layer-lifts-a-7B → the scaffold; 2503.08681 + 2510.06101 mechanism-differs-by-capability; 2402.02823 behavioral-decisive-leak); stronger-model gate re-derived UNCHANGED (`258b6ed7` invariant {KNOWN:1, UNKNOWN:4}; Gemma 4 noted with a primary Jan-2025 cutoff AT-not-before the boundary ⇒ not a qualifying stronger model). **[Anchor confirmation — the tutor lifts the FRONTIER 70B too, on the 2 families it lacks; cross-tier usefulness is FAMILY-DEPENDENT: the anchor (frontier target) probe (cells 1–3 complete; cell-4 endpoint-blocked) reads `subarrays_sum_and_range@30000` (two-deque) A1=25→T2=100/T4=100 (+75 both arms), `sum_nearest_smaller_left@50000` (monotonic-stack) A1=75% DROPPED (anchor knows it), `max_j_minus_i_le@50000` (prefix/suffix) A1=0→T2=100/T4=100 (+100 both arms) ⇒ the tutor lifts the FRONTIER 70B +75/+100pp on the 2 families IT lacks, just as it lifts the 8B on its 3 — the mechanism matters on BOTH tiers (the milestone goal), each gain spanning ≥2 families. The two tiers' needed-and-unknown sets DIFFER (anchor knows monotonic-stack / lacks prefix-suffix vs the 8B's opposite), so cross-tier usefulness is family-dependent; the SHARED-difficulty technique (two-deque) is CONFIRMED cross-tier (both tiers +75, both arms). The strict §7b ≥2-SHARED-family earn is 1-of-2 confirmed — the 2nd candidate shared family (cell-4 `kth`, which the 8B lacks) had the anchor's A1 endpoint-blocked (NIM oscillation ~6.5s↔~30-36s/call) ⇒ W140-T-CROSS-TIER-USEFULNESS-IS-FAMILY-DEPENDENT + W140-L-STRICT-ANCHOR-WEAK-≥2-SHARED-FAMILY-EARN-1-OF-2-CONFIRMED-CELL4-ENDPOINT-BLOCKED. The weak-model +75–100pp win on 3 families STANDS. W141 = measure cell-4 (the 2nd shared family) + a multi-seed run when the endpoint recovers ⇒ a strict cross-tier earn.]** W89 (+5.56) + W105 (+7.00) STAND as the only two retirements (a retirement is multi-agent same-budget superiority on the FRONTIER model; W140's win is a teacher-compiled WEAK-MODEL lift, a distinct and complementary claim). 2 new explicit-import leaf modules + 5 scripts + 27 tests; no version bump (0.5.20 / coordpy.sdk.v3.43); no PyPI; `coordpy/__init__.py` untouched. Adds W140-T-FAMILY-TUTOR-COMPILER-MINTABLE + W140-T-TUTOR-LEAK-GATE-FALSIFIABLE + W140-T-COMPILED-TUTOR-REVERSES-WEAK-TIER-USABILITY-FLOOR + W140-T-COMPILED-TUTOR-ACHIEVES-CROSS-TIER-SAFETY-BY-CONTENT + W140-T-COMPILED-TUTOR-LIFTS-WEAK-TIER-+75-100PP-IN-NEEDED-UNKNOWN-REGIME + W140-T-TUTOR-GAIN-ADDITIVE-IN-NEEDED-UNKNOWN-SUBSTITUTIVE-OTHERWISE + W140-T-ALWAYS-APPLY-SCAFFOLD-BEATS-PUBLIC-REVERT-CONTROLLER-ON-HIDDEN-FAILURE. COO-9 lead.** Previously: **W139 — per-tier band recalibration + capability-matched witness compiler + cross-tier validation (NO third retirement, frontier NOT earned; non-negativity ACHIEVED + mid-saturation root-caused + 2nd-mode revived, but §7a SPAN_FAIL). W138 found the complexity witness earns multi-family AT THE ANCHOR but is capability-gated (8B −25) + anchor-specific (mid +0); W139 attacked both. Lane α (coordpy.per_tier_band_calibration_v1 — A1-as-a-RATE at EVERY tier, the W138/W137 engine measured only the anchor; ladder V2 = llama-3.1-8b/llama-3.1-70b/llama-3.3-70b; 221 NIM; per_tier_calibration_cid 72c4a4f1) ROOT-CAUSES the W138 mid +0: the mid (llama-3.1-70b) is SATURATED on the complexity family (a1=1.00 at N=20000 AND 50000) — it is STRONGER than the anchor — so it has no headroom for any mechanism (confirms R4′ per-ability difficulty); only the anchor is witness-eligible (usability 0.50; small 0.0, mid 0.0). Lane β (coordpy.capability_matched_witness_compiler_v1; dev bench n_per_cell=2, diagnostics all tiers, 210 NIM): the capability-matched controller ACHIEVES NON-NEGATIVITY on every tier — it KEEPs on the ineligible 8B + mid (Cm≡A1) so Cm−A1=+0 on the 8B (the W138 −25 harm ELIMINATED) and it protects the 8B from BLIND REFLEXION's harm (B0 drove the 8B 25%→0%; Cm held 25% ⇒ Cm−B0=+25); validates R2′/R5′. The large-probe counterexample (build_large_probe_set_v1, SMALL_PROBE_TOKEN_CAP 400→6000) REVIVES the W138-dead 2nd mode (Nb=75% at the anchor on HIDDEN_EDGE; W138 N0=0 was a probe-budget artifact, also proven $0). BUT the §7a dev gate FAILS by SPAN_FAIL: at the anchor Cm−A1=+75, Cm−B0=+25 (margins pass) but Cm's rescues over strong blind reflexion span only 1 mode/1 family (B0 already passed both anchor HIDDEN_EDGE problems). The apparent SECOND positive tier (mid Cm−A1=+50) is KEEP-tier SAMPLING NOISE (n=2; Cm≡plain self-consistency on the ineligible mid; its Nb=0 confirms no real benefit) ⇒ NOT robust. The structural cause: NO second tier on this ladder has BOTH headroom AND witness-usability (mid saturated, 8B capability-bound) ⇒ a capability-matched controller can only earn at the single tier with both ⇒ $0 eval, $0 frontier, no third retirement. Lane γ: docs/ARCHITECTURE_REQUIREMENTS_W139_V2.md (refines R2′ verifier-matched-to-capability, R4′ per-ability difficulty, R5′ capability-aware allocation); stronger-model gate re-derived CLOSED (258b6ed7 invariant {KNOWN:1,UNKNOWN:4}); frontier NOT launched ($0). Sample sizes tiny (n=2–4 per tier) ⇒ noisy DEV signals. 431 total NIM calls; 2 new explicit-import leaf modules + 4 scripts + 11 tests; no version bump (0.5.20 / coordpy.sdk.v3.43); no PyPI; coordpy/__init__.py untouched. W89 (+5.56) + W105 (+7.00) STAND as the only two retirements; W139 retires none but ELIMINATES the W138 harm, ROOT-CAUSES the mid +0, REVIVES the 2nd mode. Adds W139-T-PER-TIER-BAND-CALIBRATION-MINTABLE + W139-T-MID-TIER-SATURATED-ON-COMPLEXITY-ROOT-CAUSES-W138-MID-PLUS-ZERO + W139-T-CAPABILITY-MATCHED-CONTROLLER-ACHIEVES-NON-NEGATIVITY + W139-T-LARGE-PROBE-REVIVES-COUNTEREXAMPLE-2ND-MODE + W139-L-CAPABILITY-MATCHED-NON-NEGATIVITY-ACHIEVED-BUT-WITNESS-GAIN-STAYS-ANCHOR-CONCENTRATED. W140 = a DENSER/different model ladder (a tier between 8B and 70B with both headroom and usability), a mechanism independent of the single-tier headroom∧usability coincidence, or a primary-KNOWN stronger model when the gate opens. COO-9 lead; COO-64.** Previously: **W138 — headroom-band battlefield search + cross-scale mechanism validation + architecture-requirements extraction (NO third retirement, frontier NOT earned, but REVISES W137). W137 concluded the parser-neutral resistant field is BIMODAL with no mechanism-headroom band at 70B; W138 shows that was an ARTIFACT: A1 was measured with n_a1=1 (a single any-of-K draw ⇒ a1_rate∈{0,1} by construction) and the hidden-N knob was pinned past the complexity cliff. Lane alpha (4 new leaf modules — headroom_band_slate_v3 [9 knob-parameterized families over a CONTINUOUS hidden-N grid; build self-test PASS: 18 cells HC1+HC2, the W136 confound + W137 bimodality + fake-different fixtures all BITE] + headroom_band_calibration_v2 [A1 as a POPULATION RATE over n_a1=8 + Wilson + HB3 intermediate-band + HB4 cross-scale-discrimination; metabench arXiv:2407.12844 / continuous-knob CLRS-Text arXiv:2406.04229] + headroom_band_corpus_v3 + band_mechanism_bench_v1, reusing the W137/W120 machinery verbatim): calibration (ladder 8b+70b; n_a0=5+n_a1=8 K=5; 180 NIM; calibration_cid ff87cb60) lands a REAL 0<A1<1 band — 5/18 cells ⇒ 3 families / 2 modes (count_pairs_sum_le_t@20000 a1=0.60 [read A1=1 SATURATED in W137], sum_nearest_smaller_left@50000 a1=0.20, subarrays_sum_and_range@30000 a1=0.40) ⇒ REVISES W137-T-BIMODAL + W137-L (the field is NOT inherently no-headroom). Lane beta: a $0 witness-firing de-risk found the counterexample witness (N0/EW1) CANNOT fire on functional families (it searches only small probe inputs, which they pass) ⇒ the 2nd mode is firing-bound (mechanistically explains W133's EW1 +0; R3/R7/R9), so the operative lead is the complexity witness C0 (the X1 controller is DOMINATED by C0). On the FRONTIER ANCHOR (llama-3.3-70b): §7a dev (5 complexity families, n=3) C0=80.0/A1=26.67/B0=33.33 ⇒ C0−A1=+53.33, C0−B0=+46.67, rescues span ALL 5 families, PASS; §7b HELD-OUT eval (n=4) C0=65.0/A1=25.0/B0=40.0 ⇒ C0−A1=+40.00, C0−B0=+25.00, span ≥3 families — the FIRST time the W133/W137 single-family complexity witness clears the §7a/§7b SPAN gate (multi-FAMILY). BUT the §7b two-tier robustness check FAILS: the witness HURTS the small tier (llama-3.1-8b C0−A1=−25.00, capability-gated; arXiv:2404.17140/arXiv:2310.01798) AND the witness-over-REFLEXION edge does NOT reproduce on a capable proxy (llama-3.1-70b near-saturated A1=70 ⇒ C0−B0=+0.00) ⇒ the gain is CAPABILITY-GATED + ANCHOR-SPECIFIC ⇒ per the discipline NO frontier rerun (§7c not run), NO third retirement. Lane gamma: docs/ARCHITECTURE_REQUIREMENTS_W138_V1.md (R1-R12 — internal verifier matched to generator capability R2, in-band difficulty estimator R4, capability-aware allocation R5); stronger-model gate re-derived CLOSED (258b6ed7 invariant). W89 (+5.56) + W105 (+7.00) STAND as the only two retirements; W138 retires none but REVISES W137-L. Adds W138-T-RESISTANT-HEADROOM-BAND-IS-CONSTRUCTIBLE-AT-70B + W138-T-COMPLEXITY-WITNESS-EARNS-MULTI-FAMILY-AT-THE-ANCHOR + W138-T-WITNESS-GAIN-IS-CAPABILITY-GATED-AND-ANCHOR-SPECIFIC + W138-T-COUNTEREXAMPLE-2ND-MODE-IS-FIRING-BOUND-AT-70B + W138-L-RESISTANT-BAND-WITNESS-NOT-CROSS-TIER-ROBUST. W139 = per-model band recalibration + a capability-MATCHED abstain/KEEP controller to recover cross-tier robustness. COO-9 lead; COO-63.** Previously: **W137 — parser-neutral HARD battlefield v2 + model-ladder hardness calibration + bounded complexity-mechanism diagnostic (battlefield-repair milestone; NO third retirement). W136 root-caused the W132–W135 "wrong-algorithm ceiling" as an I/O-FORMAT CONFOUND and found the field one-shot once I/O-normalised; W137 rebuilt it parser-neutral AND hardness-calibrated across a model ladder, then re-tested mechanisms only where the field earned it. Lane alpha ($0 build SUCCESS): coordpy.parser_neutral_io_v1 (a canonical one-logical-item-per-line normal form + the HC1 parser-neutrality gate — a STRICT per-line reader and a read-all-tokens reader must recover byte-identical structured data, so the W136 confound is machine-impossible to recur; the confound-regression fixture BITES: a W132-flattened input FAILS HC1, normal-form PASSES) + coordpy.hard_battlefield_slate_v2 (17 parser-neutral templates / 4 modes [7 COMPLEXITY_BLIND, 5 WRONG_ALGORITHM, 2 HIDDEN_EDGE, 3 SEARCH_ENUM]; slate_fingerprint_cid 2ce207c5; all 17 pass HC1 + HC2 [exact-oracle ref / INDEPENDENT-brute / admissible-wrong-naive] + HC5 [max pairwise statement Jaccard 0.4737 < 0.55]; deterministic + mint-timeout-invariant; targets the complexity/case-analysis/wrong-algorithm axis where 70B headroom should live per LiveCodeBench-Pro arXiv:2506.11928 / AlgBench arXiv:2601.04996 / arXiv:2506.22954) + coordpy.model_ladder_calibration_v1 (HC3 strong-anchor A0 headroom + HC4 floor; discrimination-across-tiers per tinyBenchmarks arXiv:2402.14992 / arXiv:2505.15055) + coordpy.hard_battlefield_corpus_v2 (seed-disjoint train/dev/eval/frontier splits, construction-based resistance per DyCodeEval arXiv:2503.04149). Lane alpha-CALIBRATION (NIM; ladder small meta/llama-3.1-8b-instruct + strong meta/llama-3.3-70b-instruct; 3 A0 + 1 A1[K=3] instances/template; 102 A0 gens + A1; calibration_cid 8b0f6231; wall 2735s on a degraded endpoint) — THE DECISIVE FINDING: at 70B the repaired field's difficulty is BIMODAL — EVERY template's strong-anchor pass@5 (A1) is EXACTLY 1.00 or 0.00, nothing in between: 12/17 HC3-SATURATED (strong A0=1.00 one-shot — ALL 5 WRONG_ALGORITHM [knapsack/weighted-interval/circular-robber/min-coins/min-subset-diff] + both HIDDEN_EDGE + all 3 SEARCH_ENUM + 2 COMPLEXITY), 4/17 HC4-DEAD (strong A0=A1=0 — the non-obvious-efficient-algorithm complexity: sort+sliding-window/monotonic-stack/two-pointer/prefix-suffix), 1/17 band (cb_count_pairs_sum_le_t, A0=0/A1=1.00). 1/17 surviving << the 40/40/40/30 floors => THRESHOLD MISS; no template has 0<A1<1 (the only regime a same-budget mechanism can beat A1). GENERALIZES W136: parser-neutral textbook DP/greedy/counting traps do NOT fool the 70B's own generation — the I/O confound was the only thing making them "hard". Lane beta (bounded complexity-mechanism diagnostic, n=3 complexity = survivor + 2 HC4-dead; A0/A1/B0/M1; 972s): A0=0.00 A1=B0=33.33% M1(the auto-routing complexity witness, arm C3)=66.67% (M1-A1=M1-B0=+33.33pp, MLB-2 0.50) — M1 RESCUED the dead cb_count_pairs_absdiff (A1=B0=0 => the efficient algorithm the i.i.d. samples + blind reject bit both missed) RECONFIRMING the W133 exact-oracle complexity witness on a PARSER-NEUTRAL field (genuine, not an I/O artifact); it did NOT crack cb_max_j_minus_i. BUT the gain is SINGLE-MODE (COMPLEXITY only) => the §7a dev gate FAILS by SPAN (modes=1<2; every non-complexity mode is one-shot-saturated so the >=2-mode span is structurally unmeetable) => NOT earned, $0 frontier — the W133 single-mode cap reconfirmed on the clean field. The M3 relabeled-reflexion negative control + B0 classify FAKE_DIFFERENT; C0/M1/M2 REAL (fake-different discipline bites). Lane gamma: the stronger-model cutoff gate re-derived IN CODE => NO_CERTIFIABLE_STRONGER_MODEL, decision CID 258b6ed7 byte-identical to the W114 lock (Maverick Aug-2024 KNOWN but C4-settled; Qwen3-Coder-480B/DeepSeek-V4-pro/Mistral-Small-4/GLM-5 primary-UNDISCLOSED); construction-based resistance sidesteps the cutoff gate; frontier NOT launched ($0; not earned). NET: at 70B clean algorithmic difficulty is BIMODAL (one-shot or impossible), leaving NO mechanism-sensitive multi-mode headroom band — the structural reason resistant code superiority is 0-clean across the W120->W136 arc; the W89 (+5.56) + W105 (+7.00) retirements STAND BECAUSE HumanEval(+) DOES have a middle band the 70B partially solves and reflexion completes. The parser-neutral kernel + calibration harness + slate + corpus LAND as reusable assets; the blocker is machine-checkable. W89 + W105 STAND as the only two retirements; W137 retires none. Adds W137-T-PARSER-NEUTRAL-IO-KERNEL-MINTABLE + W137-T-HARD-BATTLEFIELD-SLATE-V2-MINTABLE + W137-T-MODEL-LADDER-CALIBRATION-INSTRUMENT-MINTABLE + W137-T-REPAIRED-FIELD-DIFFICULTY-IS-BIMODAL-AT-70B + W137-T-COMPLEXITY-WITNESS-IS-REAL-BUT-SINGLE-MODE-ON-PARSER-NEUTRAL-FIELD + W137-L-REPAIRED-FIELD-NO-MECHANISM-HEADROOM-BAND-AT-70B. W138 = a code-competent model with a populated 0<A1<1 band / a genuinely different (non-bimodal) battlefield / accept the bimodal-ceiling cap.** Previously: **W136 — machine-structured ALGORITHM-STATE TRACE instrument + ROOT-CAUSE of the W132–W135 "wrong-algorithm capability ceiling" = an I/O-FORMAT CONFOUND (not algorithm capability). Lane alpha ($0) SUCCESS: coordpy.algorithm_state_trace_v1 (AT1 decision-path / AT2 subproblem-state incl. the full 2-D StateGridV1 DP table assembled from BLACK-BOX oracle calls / AT3 search-frontier / AT5 typed capsule; AT4 invariant EXCLUDED for leakage) + family-aware _typed_subinstances_v1 (the W135 generic 1-D slicer produced MALFORMED sub-instances on exactly the 3 structured-input traps) + coordpy.algorithm_state_trace_corpus_v1 (fresh 136_0xx seeds; admitted 80/79/79/78 >= 36/36/36/30 floors; corpus_cid ce1a6bc6 / eval_split 13519353 / frontier_slice 3f75b302 LOCKED; held-out integrity TRUE; 70/75 train fire genuinely-new, 75/75 ref-silent; 18 tests; no-leakage enforced+tested; REAL 1-hop bridge to 6 modules). Lane beta: the machine-structured trace arm T1 — including the full 2-D 0/1-knapsack DP table + a recurrence-derivation scaffold — cracks 0/3 traps, IDENTICALLY to prose structure S4 and counterexample C1 (algorithm feedback in ANY form fails) — THEN, refusing the apparent null, root-caused WHY via a diagnostic capturing the model's ACTUAL code + execution-diff-vs-oracle: the model writes the CORRECT algorithms (2-D knapsack DP, blocked-lattice DP, weighted-interval DP) but the W132 battlefield emits input WHITESPACE-FLATTENED (the reference reads sys.stdin.read().split(); grid rows / pairs / triples on one line) while the model assumes the STANDARD one-per-line layout => it crashes/misparses even the PUBLIC samples. QUADRUPLE-confirmed: (1) corpus-wide the easy/hard split is EXACTLY standard/non-standard I/O (13 easy = SINGLE_INT/SINGLE_ARRAY one-shot; 3 traps = GRID/MULTI_TUPLE flattened); (2) $0 the IDENTICAL correct 0/1-knapsack DP passes 7/7 with split() parsing and fails 7/7 with input()-per-line parsing; (3) the execution-grounded I/O-repair arm T_IO (run_io_grounded_trace_arm_v1; weightless, no-leakage; prepends a generic whitespace-parse directive ONLY when the model's own code fails a valid public sample) cracks 3/3 traps (+18.75pp, both modes) — but the gains are PARSING-only => §7b condition 4 EXCLUDES them => NOT a frontier earn ($0 frontier); (4) STANDARD-I/O A0 single-shot (NO feedback, NO mechanism) one-shots 3/3 traps (ref_still_passes_reformatted=True). => the apparent 'wrong-algorithm capability ceiling' of W132/W133/W135 is an I/O-FORMAT CONFOUND, REVISING W133-L-WITNESS-FEEDBACK-SINGLE-MODE-CAP and W135-L-SOLUTION-STRUCTURE-WITNESS-DEV-CAP (the feedback addressed the wrong bug); the generated WA/SE battlefield is I/O-confounded AND (I/O-fixed) one-shot/low-algorithm-headroom => it does NOT test hard ALGORITHM capability and cannot demonstrate coordination superiority. Learned-memory line re-opened HONESTLY: differentiable_memory/composed_learned_memory/live_composed KILLED (random-until-trained nets on synthetic rng.standard_normal data; live one GPU-blocked); only the weightless controller_native_code_mechanism_v1 router is usable (as T2) — corroborates W124. Lane gamma: stronger-model gate CLOSED (258b6ed7 invariant {KNOWN:1,UNKNOWN:4}); frontier NOT launched ($0; not earned); graphify START+END (83922 nodes/199104 edges; the new module = REAL 1-hop imports_from bridge to exact_oracle_witness_v1 + solution_structure_witness_v1 + resistant_by_construction_battlefield_v1 + icpc_reflexion_bench_v1 + executor_grounded_patcher_v1 + controller_native_code_mechanism_v1). W89 (+5.56) + W105 (+7.00) STAND as the only two retirements (REAL HumanEval, entirely outside this confound); W136 retires none, earns no frontier, demonstrates no coordination superiority — but CORRECTS a four-milestone misinterpretation and lands the execution-grounded I/O-repair mechanism as a tested asset. Adds W136-T-ALGORITHM-STATE-TRACE-INSTRUMENT-MINTABLE + W136-T-MACHINE-STRUCTURED-STATE-FEEDBACK-DOES-NOT-LIFT-APPARENT-CEILING + W136-T-WRONG-ALGORITHM-CEILING-ON-GENERATED-WA-SE-IS-IO-FORMAT-CONFOUND + W136-T-EXECUTION-GROUNDED-IO-REPAIR-CRACKS-ALL-TRAPS + W136-T-STANDARD-IO-A0-ONESHOTS-ALL-TRAPS + W136-L-GENERATED-WA-SE-BATTLEFIELD-IS-IO-CONFOUNDED-AND-ALGORITHM-LOW-HEADROOM. W137 = rebuild the generated battlefield with STANDARD one-per-line I/O for a CLEAN algorithm-capability test / pursue genuinely hard algorithm battlefields (the official contamination-resistant ICPC/LiveCodeBench lines remain the real ones).** Previously: **W135 — oracle-derived solution-STRUCTURE witness + dedicated NON-COMPLEXITY (WRONG_ALGORITHM + SEARCH_ENUM) held-out corpus + held-out dev bench (frontier rerun NOT earned). W133 showed the EW1 COUNTEREXAMPLE witness adds +0.00 over blind reflexion on these modes (the wrong-algorithm capability ceiling); W135 asked whether oracle-derived SOLUTION STRUCTURE breaks it. Lane alpha ($0) SUCCESS: coordpy.solution_structure_witness_v1 (SW1 greedy-failure certificate / SW2 optimal-substructure ladder over OBVIOUS disjoint sub-instances [exploratory + leak-constrained per Pu OOPSLA'11 + KNARsack arXiv:2509.15239] / SW3 search-frontier exact-vs-naive count contrast / SW4 controller = the union + SolidCoder arXiv:2604.19825 shift-left step [LEAD]) anchored on a token-minimal fresh disjoint counterexample (reuses W133 EW1); research CHANGED the FORM (PGS arXiv:2506.18315 minimal-by-token + attribution + property phrasing; LDB arXiv:2402.16906 = own-program states not oracle sub-values => genuinely new in FORM). No-leakage ENFORCED+TESTED (executes ref/naive/brute but never renders solver source; counterexample + every sub-instance byte-disjoint from secret_cases; never the recurrence/clever-state; grading on the DISJOINT hidden bank => GENERALISATION). coordpy.noncomplexity_structure_corpus_v1: 8 wa_ + 8 se_ families x 5 seed-disjoint seeds/split => admitted 80/79/80/78 (>= 36/36/36/30 floors); corpus_cid 306610ae, eval_split 3f6e3e59 + frontier_slice 8aa53564 LOCKED before spend; held-out integrity TRUE after a principled cross-split dedup of seed-independent fib_no_adjacent collisions (1 dev + 2 frontier dropped, floors untouched); naive/ref separation 80/80, genuinely-new-vs-EW1 54/80, all leakage-clean, neg control (silent on a COMPLEXITY naive) + pos control (ref => NONE) + 4 clean-ladder fixtures genuinely-new. Lane beta (held-out DEV, 16 problems = 8 SE + 8 WA, meta/llama-3.3-70b-instruct, 1 seed x K=5, 336 NIM, wall 8507s; DECISIVE LEAD subset A0/A1/B0/C1/S4 per a transparent §9 amendment — the tiny latency probe under-measured real per-call latency [~20s base / ~58s witness arms >> 12s throttle], full 9-arm slate projected ~9h, so S1/S2/S3 ablations + D0 were $0-staged on an S4 earn, NOT earned, NOT run): A0 = A1 = B0 = C1 = S4 = 81.25% (13/16). B0-A1=+0.00, C1-B0=+0.00, S4-C1=+0.00, S4-B0=+0.00; S4 rescues 0 over C1 (0 modes/0 families); MLB-2 C1 25% / S4 0%. THREE findings: (1) near-ZERO feedback headroom at 70B — A0 single-shot already 13/16 (the 70B writes the correct textbook DP/counting directly, so the admissible-wrong naive does not fool its OWN generation); (2) the SAME 3 problems (se_lattice_paths_blocked / wa_knapsack_01 / wa_weighted_interval_scheduling) are CAPABILITY-bound for EVERY arm; (3) the structure witness fired genuinely-new + leakage-clean on the 4/16 failed problems but the model converted NONE => oracle-derived attributed structure adds nothing over a bare counterexample on WA/SE at 70B. §7a dev gate FAILS (S4-B0=S4-C1=+0.00 < +3.33; 0 modes/families) => $0 eval, $0 frontier; the wrong-algorithm ceiling is CAPABILITY-bound, NOT feedback-form-bound, at 70B (SHARPENS W133: the value/algorithm sub-mode is not merely counterexample-unliftable but structure-unliftable). Lane gamma: stronger-model gate CLOSED (258b6ed7 invariant {KNOWN:1,UNKNOWN:4}; Qwen3-Coder-480B/DeepSeek-V4-Pro/Mistral-Small-4/GLM-5 primary-UNDISCLOSED; new MiniMax-M3 [2026-06-01] no published card); frontier NOT launched; no Maverick/405B spend; graphify START (b023ee4) + END. W89 (+5.56) + W105 (+7.00) STILL the only two retirements; W135 retires none. Adds W135-T-SOLUTION-STRUCTURE-WITNESS-INSTRUMENT-MINTABLE + W135-T-NONCOMPLEXITY-STRUCTURE-CORPUS-MINTABLE + W135-T-NONCOMPLEXITY-FIELD-LOW-HEADROOM-AT-70B + W135-T-STRUCTURE-WITNESS-TIES-COUNTEREXAMPLE-ON-HELD-OUT-DEV + W135-L-SOLUTION-STRUCTURE-WITNESS-DEV-CAP.** Previously: **W134 — DEPLOYABLE complexity witness (constraint-derived, oracle-free) + dedicated COMPLEXITY-ONLY held-out corpus + held-out dev bench (frontier rerun NOT earned). Lane alpha ($0) SUCCESS: coordpy.deployable_complexity_witness_v1 distils the W133 exact-oracle EW2 into a DEPLOYABLE public-signal witness — DW1 constraint-derived budget (bridges public_signal_selection_oracle_v1.parse_max_constraint_v1: parses N_max, ops budget 5e8, O(N^2) inadmissible at N=1e5) + DW2 public-format stress-growth ladder [1000..8000, shapes {descending,constant,random} adversarial-first early-exit, baseline-subtracted log-log exponent + extrapolation] + DW3 rewrite + DW4 KEEP/REWRITE/ABSTAIN controller (bridges stronger_generator_slate_v1.complexity_admissible_v1); the same-budget arm consumes ONLY (code, statement, public samples) — NO ref/naive/brute/secret on any path (the deployability proof). Per SwiftSolve (arXiv:2510.22626) / GuessCompx (arXiv:1911.01420) small-n-instability, a non-TLE super-linear verdict needs R^2>=0.7 AND >=0.1s top runtime else ABSTAINS (calibrated diagnostic, not an oracle). coordpy.complexity_only_corpus_v1: the 9 cb_ COMPLEXITY_BLIND families x 5 seed-disjoint seeds/split => admitted 45/45/45/45 (>= 36/36/36/30 floors); corpus_cid 191d9954 (byte-identical at mint timeout 8.0s and 3.0s); eval_split 748dd6fa + frontier_slice 31a81304 LOCKED before spend; held-out integrity TRUE (content-CID + seed pairwise-disjoint). Self-tests ($0): naive/ref separation 45/45 (deployable == exact-oracle EW2 == agreement 45/45), genuinely-new 43/45, deterministic ladder + split regen, regression fixtures fire, negative control (fast-wrong) silent. Lane beta (held-out DEV, 18 problems = 9 fam x 2 seeds, meta/llama-3.3-70b-instruct, 1 seed x K=5, 522 NIM): A0 44.44 / A1 55.56 / B0 88.89 / C0(exact-oracle EW2) 100.00 / D1 94.44 / D2 94.44 / D3(lead) 88.89. THREE findings: (1) blind reflexion B0 is ALREADY strong on a complexity-only field (88.89%, B0-A1=+33.33pp: the reject bit conveys 'too slow' and 70B knows the fast algorithms), little headroom; (2) even the EXACT-oracle C0's +11.11pp gain over B0 is SINGLE-FAMILY (window_distinct_sum) => the >=2-family bar blocks even C0 (a FIELD property, not a deployable weakness); (3) the deployable witness is REAL but SUB-ORACLE (D1/D2 +5.56pp over B0 = half of C0, MLB-2 90%+, 1 family, 1 witness-induced regression), and the controller D3 ties B0 (+0.00; KEEP/ABSTAIN deferred to blind reflexion — D1>D2>D3). §7a dev gate FAILS for every arm (>=2-family fails incl. C0; D1/D2 exceed the C0-tracking tolerance) => $0 eval, $0 frontier; the deployable-complexity lever does NOT earn a frontier rerun at 70B. Lane gamma: stronger-model gate CLOSED (258b6ed7 invariant); no frontier/Maverick/405B spend. W89 (+5.56) + W105 (+7.00) STILL the only two retirements; W134 retires none. Adds W134-T-DEPLOYABLE-COMPLEXITY-WITNESS-INSTRUMENT-MINTABLE + W134-T-DEPLOYABLE-WITNESS-FAITHFUL-TO-EXACT-ORACLE-ON-NAIVE-REF-SEPARATION + W134-T-DEPLOYABLE-COMPLEXITY-CORPUS-CID-TIMEOUT-INVARIANT + W134-T-DEDICATED-COMPLEXITY-FIELD-B0-CEILING-HIGH-AND-SINGLE-FAMILY-HEADROOM + W134-T-DEPLOYABLE-COMPLEXITY-WITNESS-IS-REAL-BUT-SUB-ORACLE-ON-HELD-OUT-DEV + W134-T-CONTROLLER-ABSTAIN-DISCIPLINE-NEUTRALISES-WITNESS-ON-COMPLEXITY-FIELD + W134-L-DEPLOYABLE-COMPLEXITY-WITNESS-DEV-CAP.** Previously: **W133 — exact-oracle WITNESS curriculum + held-out witness-guided mechanism bench on Llama-3.3-70B (Maverick infra-down) = a REAL but SINGLE-MODE complexity-witness gain; frontier rerun NOT earned. W132 closed the 'wrong-test' escape but left the BLIND hidden-test reject bit un-pulled; W133 turned the owned battlefield into a TEACHER (`coordpy.exact_oracle_witness_v1` EW1 counterexample / EW2 complexity-timing / EW3 edge / EW4 search-enum slate + same-budget witness arm + `coordpy.witness_curriculum_corpus_v1` seed-disjoint train/dev/eval; 99 admitted = 33/33/33 >=96; curriculum_cid e5e44a43; eval CID 88b9b79a locked before spend; witnesses fire leakage-clean + genuinely-new on every admitted problem AND all six W132 traps; no-leakage enforced BY WITNESS TYPE — EW1 disjoint-from-graded, EW2 size/timing-only; graded on a DISJOINT hidden bank = generalisation-not-memorisation). Held-out dev bench (33 problems, same-budget A0/A1/B0 + C1/C2/C3, 858 NIM): the EW2 complexity witness (C3) beats the blind W132 stack B0 by +6.06pp and A1 by +12.12pp (MLB-2 60%, 4 ALGORITHMIC complexity rescues) cracking O(N^2) COMPLEXITY_BLIND traps B0 + counterexample-only C1 cannot, BUT the EW1 counterexample witness (C1) adds +0.00pp over B0 (0 rescues vs B0) = the wrong-algorithm capability ceiling. SINGLE-MODE gain => meets -A1>=+5 + -B0>+3.33 but FAILS the >=2-mode condition => §7a dev gate FAILS => $0 eval, $0 frontier (NOT earned). LOCALISES the W132 cap: part feedback-fixable (complexity) + part capability-bound (value/algorithm). Stronger-model gate CLOSED (258b6ed7 invariant; all 4 stronger models primary-UNDISCLOSED). W89 (+5.56) + W105 (+7.00) STILL the only two retirements; W133 retires none, adds W133-T-EXACT-ORACLE-WITNESS-INSTRUMENT-MINTABLE + W133-T-COMPLEXITY-WITNESS-IS-REAL-AND-LOAD-BEARING-ON-HELD-OUT-DEV + W133-L-WITNESS-FEEDBACK-SINGLE-MODE-CAP.** Previously: **W132 — resistant-by-construction hard-family battlefield + exact-oracle gates + EXECUTED frontier pilot on Llama-3.3-70B (Maverick infra-down) = clean FAIL. CoordPy MINTED a 33-problem (>=30 admitted) resistant-by-construction battlefield (resistant_by_construction_battlefield_v1 + _slate_v1) targeted at the WRONG_ALGORITHM_ADMISSIBLE / HIDDEN_EDGE_STATE_MISS / COMPLEXITY_BLIND / SEARCH_ENUM atlas modes; each problem ships an exact ref_source answer-key oracle cross-checked by an INDEPENDENT exhaustive brute_source, an admissible-wrong naive_source trap (looks-right-on-public / fails-hidden discriminating gate), a near-dup + official-identity novelty guard, deterministic regeneration, manifest_cid 562aafbd / core 30-slice f6a2ebed; resistance-certified by date AND construction => W132-T-RESISTANT-BY-CONSTRUCTION-BATTLEFIELD-MINTABLE (removes the W123 supply cap + W131 cutoff-disclosure dependency on the EXISTENCE of a resistant instrument). The locked target Maverick was infra-down (machine-checked: same key, llama-3.1-8b/llama-3.3-70b respond instantly, maverick chat hangs 0-bytes), so the frontier target was substituted PRE-SPEND to meta/llama-3.3-70b-instruct (the EXACT W105 retirement model, KNOWN cutoff ~Dec-2023). Executed pilot (30-slice, 1 seed, K=5, 330 calls): A0=A1=76.67% B=80.00% B-A1=+3.33pp (< +5pp; 7/9 gates; MLB-1 26.7% FAIL, MLB-2 25% FAIL) => W132-L-RESISTANT-BY-CONSTRUCTION-PILOT-CAP; the +3.33pp is 1 B-unique COMPLEXITY rescue / 0 regressions, 6 capability-bound traps resist all arms; CLOSES the 'wrong test' escape (+3.33pp = SAME as the W121 matched-exposed-ICPC control). Stronger-model gate CLOSED (258b6ed7 invariant); Maverick = push-button CROSS-SCALE check on the same slice. W89 (+5.56) + W105 (+7.00) STILL the only two retirements; W132 retires none.**
...
**W143 update (post-W142b).** A direct attempt to make the W142b discover-then-amortize win team-borne found the team mechanism genuinely load-bearing (role-diverse discovery beats i.i.d. at a starved budget, 3/12 vs 0/12) but ANTI-CORRELATION-CAPPED: the advantage is large only where it is unreliable (low budget) and vanishes where the single controller is reliable (operating budget). So the answer is unchanged — W89 + W105 remain the only two multi-agent retirements; the discover-then-amortize win is single-controller-complete and does not compose into a multi-agent retirement on amortizable families.

## TL;DR — W143 true multi-agent composition of the W142b discover-then-amortize win (NO third retirement; multi-agent line closed sharply)

W143 fused the W89/W105 team line + W128 role-diverse discovery + W140/W141/W142b no-oracle self-tutoring into a genuine multi-agent discover-then-amortize TEAM (`coordpy.multi_agent_discover_amortize_v1`; graphify-confirmed real cross-community bridge). Result: the team is genuinely LOAD-BEARING but does NOT yield a retirement. (1) At the OPERATING budget (K_d≥8) it TIES the single controller (clean unloaded re-selection ST 3/3 = MA 3/3) — matched-budget DPI prior. (2) At a STARVED budget (K_d=4) it has a REAL discovery advantage (subarrays, 12 seeds: disc MA 3/12 vs ST 0/12; generation ceiling 5/12 vs 2/12; + extractability), load-bearing by ablation collapse (role-diverse-OFF = ST → 0/12). (3) BUT the advantage is ANTI-CORRELATED with reliability across the budget axis (large-but-unreliable low / reliable-but-vanishing high), forced by the amortizable family's i.i.d.-reachable supply (W141) ⇒ no budget gives a robust confirmable same-budget multi-agent retirement. The discover-then-amortize win is single-controller-complete at its operating budget; multi-agent reflexion (W89/W105) and discover-then-amortize (W142b) are complementary-but-non-composable on amortizable families. W89 + W105 remain the only two MULTI-AGENT retirements; W142b the discover-then-amortize retirement; W143 adds none. 345 NIM; gate 258b6ed7 invariant; no version bump; COO-9 lead; COO-68. W144 = the Latent State Transition architecture branch / a model where the efficient form is not i.i.d.-reachable.

## TL;DR — W101 second-code-benchmark battlefield tournament + MBPP+ lead selection + cheap NIM-free preflight (COO-9 charter executed end-to-end; cheap pilot conditional on operator MBPP+ fetch)

W101 is the **post-W100 code-pivot infrastructure** milestone.
Per the pre-committed Part H of ``docs/RUNBOOK_W100.md``,
`COO-9` (second code benchmark battlefield) was promoted to the
lead path when W100 confirmed BOTH B2 (frontier lead) and B5
(baseline-only ceiling reference) FAIL the +5 pp Phase 2 bar at
90B on RealWorldQA.  W101 executes the COO-9 charter end-to-end.

**No NIM call in W101.**  The empirical preflight verdict
(6 / 8 probes PASS; 2 DEFERRED on operator MBPP+ fetch) licenses
the *next* operator step (fetch the EvalPlus MBPP+ release
artifact + record its SHA pin + re-run preflight) and the cheap
NIM pilot is conditional on the preflight re-run clearing
P1 + P2.

### W101 battlefield-selection ranking (locked BEFORE any code built)
...
| **HumanEval+** | **BACKUP** | Same EvalPlus family; trivial port; built only if MBPP+ preflight FAILs |
...
``scripts/run_w101_arsenal_mining.py`` re-executed 2,640
candidate responses (W88 70B HumanEval + W91 5-seed 70B MBPP)
against the canonical W86 / W90 subprocess executors offline.
Per-seed numbers match the published W89 + W91 result docs
byte-for-byte.
...
**Structural findings**: reflexion-rescue surface is 2.5×
richer on HumanEval than on base MBPP (the empirical symptom
of the ceiling-saturation cap MBPP+ relieves); hard-cluster
size scales with ceiling-control (MBPP shared-fails 14 % vs
HumanEval 5.6 %).

### W101 NIM-free preflight verdict (6 / 8 PASS; 2 DEFERRED)

| Probe | Verdict | Summary |
|---|---|---|
| P1 corpus integrity | **DEFERRED** | MBPP+ cache absent; operator step |
| P2 executor self-test | **DEFERRED** | same gate as P1 |
| P3 A1@K=5 failure-residual | **PASS** | Predicted MBPP+ A1@K=5 = **69.97 %**; saturation margin **20.03 pp** |
| P4 decomposition argument | **PASS** | 1727 chars; W89 retirement as precedent |
| AddrW101-P1 mechanism-load-bearing | **PASS** | W89 rescue 9.76 %; threshold ≥ 5 % |
| AddrW101-P2 cluster structure | **PASS** | Both partitions well-formed |
| AddrW101-P3 cross-bench stability | **PASS** | Margin 20.03 pp; floor 10 pp |
| AddrW101-P4 anti-pattern guard | **PASS** | No `bounded_window` / `compaction` / `summary` tokens |

**Decision (per pre-committed runbook branch 1)**: cheap NIM
pilot NOT YET earned.  Operator step required: fetch MBPP+ +
record SHA pin + re-run preflight.  When P1 + P2 PASS at
re-run, launch ``scripts/run_w101_mbpp_plus_pilot.py`` at 1
seed × 30 problems × K=5 (~ 330 NIM calls; ~ 2-3 h wall at
70B).

### W101 deliverables (all explicit-import only)

* `coordpy/mbpp_plus_loader_v1.py` — MBPP+ corpus loader with
  SHA-pinned canonical EvalPlus release URL; refuses to operate
  against the all-zeros placeholder pin (W93 preflight-first
  discipline).
* `coordpy/mbpp_plus_executor_v1.py` — subprocess CPython
  executor running candidate against (base + plus) assertions;
  three modes (base_and_plus / base_only / plus_only) for the
  cross-bench-stability probe.
* `coordpy/mbpp_plus_reflexion_bench_v1.py` — A0 / A1 / B
  reflexion bench; W89 sequential-reflexion mechanism byte-
  identical; per-call sidecars + per-seed Merkle + bench Merkle
  re-derivation preserved.
* `coordpy/mbpp_plus_preflight_v1.py` — W93 5-gate + W101-
  specific AddrW101 probes.
...
* `scripts/run_w101_mbpp_plus_pilot.py` — conditional cheap-
  pilot driver (NIM-spending; gated on preflight P1+P2 PASS at
  re-run).
* `tests/test_w101_mbpp_plus_v1.py` — 25 unit tests; all PASS.

### Carry-forwards

**Added**: None this milestone (W101 is infrastructure +
preflight; no empirical NIM result has been produced yet).  The
W101 result docs list *candidate* carry-forward IDs that the
cheap pilot would add: `W101-L-MBPP-PLUS-REFLEXION-PHASE2-70B-CAP`,
`W101-L-MBPP-PLUS-PREFLIGHT-P3-SATURATION-CAP`,
`W101-L-MBPP-PLUS-MLB-2-NON-MECHANISM-DRIVEN-CAP`.

**Retired**: None.  W89 70B HumanEval K=5 remains the only
confirmed multi-seed same-budget multi-agent superiority
retirement.

### Frontier-audit reclassifications

* **Active frontier (NEWLY added)**: the 4 W101 modules + the
  W101 arsenal-mining script + the W101 multi-candidate-
  tournament discipline + ``COO-9`` (NEWLY promoted to lead).
* **Baseline-only (NEWLY classified)**: base MBPP via
  ``coordpy.mbpp_reflexion_bench_v1`` (the W91 cap is empirical;
  MBPP+ is the structural fix).
...
W96-D / W97 / W98 / W99 / W100 / **W101**.  W101's distinguishing
addition is the **codified battlefield-selection matrix** (the
W94 / W96-D rubric was implicit; W101 makes it explicit) plus
the **offline-sidecar-re-execution arsenal-mining infrastructure**
that re-derives the W89 / W91 published per-seed numbers byte-
for-byte without any new NIM calls.
...
untouched.  All 4 new ``coordpy.*`` modules are explicit-import
only.  The 3 new driver scripts are NOT library modules.
