...
> `docs/HONEST_FRAMING_POST_W87.md`. Last touched: **W119 — official ICPC public-package pivot DISSOLVES the W118 grader blocker (a REAL constructor `coordpy.coordpy_icpc_public_functional_v1` enumerated the official `github.com/icpc` org and verified the post-Maverick-cutoff Rocky Mountain Regional 2024-2025 + 2025-2026 packages ship real non-LFS `data/secret/*.in`+`*.ans` + shipped output validators + 119 accepted reference solutions; a NIM-free grader self-test ran official accepted Python solutions vs official secret cases = 16/16 cases PASS ⇒ the executable functional grader W118 proved ABSENT family-wide is now PRESENT + verified on a genuinely-new resistant battlefield); BUT the post-cutoff resistant pass-fail slice from the cleanest single official surface = 24 < MIN_RESISTANT_SLICE 30, which is load-bearing at BOTH levels (slice-admissibility AND the reused W114 C2 cert gate needs ≥30 resistant after cutoff) ⇒ even KNOWN-cutoff Maverick is NOT certifiable on the 24-slice, tier-2 C1-blocked (UNKNOWN cutoffs) ⇒ 0 certifiable models ⇒ NO pilot; verdict `NO_CERTIFIABLE_STRONGER_MODEL` (decision CID `258b6ed7…`, byte-identical to W114–W118), $0 NIM (NVIDIA_API_KEY present ⇒ clean count-gate no-go, not reachability); the blocker MOVED from W118's "abundant official identities, no official grader" to W119's "official grader present + self-test-passing, +6 post-cutoff resistant pass-fail tasks short of a clean pilot"; W89+W105 STAND (the only two retirements), resistant superiority still 0 clean across both scales, contamination-confound UNCHANGED; COO-9 lead; W120 fires on the post-cutoff resistant pass-fail count reaching 30 (next official ICPC regional drop / clean official second-surface aggregation — grader already present, so a slice alone unlocks the cheapest honest verdict-changing Maverick pilot) OR a reachable stronger-than-Maverick model disclosing a primary-KNOWN cutoff; 29th consecutive preflight/earn-discipline validation (W93–W119); 1 new explicit-import-only module + 1 script + 18 tests (110 across W113–W119); no version bump; no PyPI; `coordpy/__init__.py` untouched** on 2026-05-30.  Prior: **W118 — CoordPy-OWNED post-v6 functional-instrument CONSTRUCTION (LIVE; official Codeforces API) + deeper primary-source model-cutoff ATTACK + durable construction/admission/pilot-readiness pipeline → 894 official post-v6 functional problem IDENTITIES constructed (date/identity axis SOLVED at scale), but the executable functional GRADER is ABSENT family-wide (Codeforces/AtCoder/LeetCode), NO certifiable stronger model ($0 NIM)** on 2026-05-30 — a **gated branch milestone with THREE lanes (α LIVE CoordPy-owned construction, β deeper primary-source cutoff attack, γ construction/admission/pilot-readiness pipeline/graphify/truth), NOT another exposed rerun, NOT a "check if v7 exists yet" watch-and-wait, NOT another same-scale resistant reflexion rerun, NOT a bounded-context/compaction job**.  W118 escalated W117 from *proving no post-v6 instrument can be inherited* to *building one from official sources*: it STOPPED waiting for a packaged `release_v7` and ran a REAL CoordPy-OWNED constructor (`coordpy.coordpy_frontier_functional_v1`) LIVE against the official Codeforces API (`contest.list` + `problemset.problems`; raw-fetch SHA `b6342fd1…`), applying a total deterministic inclusion rule (PROGRAMMING ∧ FINISHED ∧ `contest_date` strictly after the `release_v6` frontier 2025-04-05) ⇒ **894 admitted post-v6 functional problems** from 11,223 candidates, 2025-04-07..2026-05-30, 130 contests, machine-generated manifest (CID `fb4185a6…`); the IDENTITY axis (O1..O6) is SOLVED at scale (894 ≫ 30) (`W118-T-COORDPY-OWNED-POST-V6-FUNCTIONAL-IDENTITY-CONSTRUCTIBLE`).  BUT the executable functional GRADER (O7) is ABSENT family-wide (Codeforces API exposes problem metadata only, no test field/endpoint; LeetCode hidden tests deliberately private; AtCoder system tests Dropbox-only, no official API), and sample-only grading is non-credible while operator-synthesised tests are operator curation (refused) ⇒ the instrument is IDENTITY-admissible but NOT PILOT-admissible (`W118-L-OFFICIAL-SOURCE-FAMILY-NO-EXECUTABLE-GRADER-CAP`); Maverick (KNOWN Aug-2024) is even C1∧C2∧C3∧C4 identity-CERTIFIABLE on this genuinely-new instrument, blocked ONLY by the missing grader (`W118-T-MAVERICK-IDENTITY-CERTIFIABLE-GRADER-BLOCKED`).  W118 (β) re-checked primary cutoffs DEEPER (DeepSeek V4 PDF re-fetched directly; Maverick "August 2024" verbatim) and noted the new GLM-5 (UNKNOWN-from-primary + C2-exposed + reachability-unverified) ⇒ nothing newly primary-disclosed since W117.  W118 (γ) shipped `coordpy.coordpy_frontier_functional_v1` (explicit-import-only; reuses the W113 registry + W114 `certify_model_v1` + the W117 `run_upstream_construction_v1` + the W116 disclosure types, no duplication): the O1..O7 rule, the official-source-family grader registry, the deterministic manifest constructor + thin live fetch, the reused C1..C4 + O7 certification, the W118 disclosure matrix, and a structured `W119FireConditionV1` (push-button; with a falsifiability test) (`W118-T-FRONTIER-FUNCTIONAL-CONSTRUCTION-PIPELINE-SHIPS`); the LCB-inherited verdict re-derives `NO_CERTIFIABLE_STRONGER_MODEL` (decision CID `258b6ed7…`, byte-identical to W114/W115/W116/W117; result CID `3ab0d186…`), **$0 NIM, no pilot**.  The blocker MOVED — from W117's "no post-v6 identities can be constructed" to W118's "abundant official post-v6 identities (894), no official executable grader".  The two retirements (W89, W105) STAND, W118 adds NONE; the contamination-confound is UNCHANGED (W118 tests construction + grader supply, not the confound); `COO-9` stays lead; **W119 fires the moment an OFFICIAL executable per-problem test suite for ≥30 post-v6 functional problems appears on a clean official surface (Maverick is already identity-certifiable ⇒ a grader alone unlocks the cheapest honest verdict-changing pilot), OR a packaged `release_v7`+ / LCB-published post-v6 construction provenance appears, OR a reachable stronger-than-Maverick model discloses a primary-KNOWN cutoff ≤ the manifest frontier** — until then the bounded ceiling STANDS and resistant-code NIM is BLOCKED on the missing official grader.  28th consecutive preflight/earn-discipline validation (W93–W118); stable boundary preserved (no version bump; no PyPI; `coordpy/__init__.py` untouched; 1 new explicit-import-only module + 1 script + 15 new tests = 96 across W113–W118).  Prior milestone: **W117 — upstream-DERIVED instrument CONSTRUCTION ATTACK (LIVE; eight provenance surfaces) + deeper primary-source model-cutoff ATTACK + durable upstream-DERIVED construction/admission pipeline → NO construction-admissible instrument can be built from authoritative provenance beyond `release_v6`, NO certifiable stronger model ($0 NIM); the only post-v6 path (raw-contest hand-assembly) is refused by the pre-committed B1 + B2 criteria** on 2026-05-30 — a **gated branch milestone with THREE lanes (α LIVE upstream-derived construction attack, β deeper primary-source cutoff attack, γ construction/admission pipeline/graphify/truth), NOT another exposed rerun, NOT a packaged-release "watch-and-wait" check, NOT another same-scale resistant reflexion rerun**. W117 escalated W116 from *packaged-release detection* to *construction from provenance*: it asked whether a post-v6 functional instrument can be CONSTRUCTED from official upstream PROVENANCE even before a numbered `release_v7` is packaged, and attacked that at EIGHT authoritative surfaces (HF dataset commit/revision log — latest data commit "add v6" 2025-04-21, HEAD "fix typos" 2025-06-05; HF refs — 1 branch/0 tags; HF discussions — no v7 thread; GitHub commits — newest 2025-07-16 runner-maintenance, no data commit; GitHub tags — 0; GitHub repo pipeline structure — `lcb_runner/`+`assets/` only, NO scraper/collection dir; dataset README — provenance = LeetCode/AtCoder/Codeforces, documents only loading published releases, NO generation tool/manifest; runner loader `code_generation.py` — loads HF-only via `load_dataset(version_tag=…)`) ⇒ **the authoritative construction provenance IS the packaged HF release**; LCB publishes no collection pipeline or forward problem-id manifest; no post-v6 LCB-published artifact exists at ANY surface; the ONLY post-v6 path (raw-contest hand-assembly) is **CONSTRUCTION-INADMISSIBLE** (refused by the pre-committed B1 authoritative-LCB-provenance ∧ B2 no-operator-curation criteria) ⇒ **0 construction-admissible NEW instruments** (sharper than W116's "no packaged v7": a post-v6 instrument cannot be *constructed* from authoritative provenance, only hand-curated, which is refused) (`W117-L-NO-CONSTRUCTION-ADMISSIBLE-POST-V6-INSTRUMENT-EIGHT-SURFACE-CAP` + `W117-T-LCB-CONSTRUCTION-PROVENANCE-IS-PACKAGED-RELEASE`). Lane β re-checked official cutoffs DEEPER from PRIMARY sources ⇒ no reachable stronger-than-Maverick model has a primary-KNOWN cutoff ≤ 2025-01 and nothing was newly-disclosed since W116: Maverick "August 2024" re-confirmed VERBATIM (Meta `MODEL_CARD.md`) but C4-settled; Qwen3-Coder-480B UNKNOWN (official HF card raw README: no cutoff); **DeepSeek-V4-pro UNKNOWN from primary — its official V4 model-card PDF re-checked still states NO cutoff; the only figure is a non-primary aggregator "Apr 2026" that is itself C2-exposed**, now matching Mistral-Small-4-119B-2603's "UNKNOWN-primary + C2-exposed-aggregator" pattern (`W117-T-DEEPSEEK-V4-PRIMARY-PDF-RECONFIRMED-NO-CUTOFF`). Shipped `coordpy.upstream_derived_instrument_construction_v1` (explicit-import-only; reuses + WRAPS the W116 `run_upstream_admission_v1` → W115 `run_frontier_certification_v1` → W114 `certify_model_v1` chain, no duplication; graphify-confirmed `run_upstream_construction_v1 --calls--> run_upstream_admission_v1`): the construction rule (A1..A5 reused ∧ B1 ∧ B2), an eight-surface provenance snapshot, a candidate-instrument constructor (raw-contest assembly triply refused; LCB-pipeline template construction-admissible-in-principle but artifact-absent ⇒ `constructed=False` + the EXACT missing artifact named), a sharpened disclosure matrix, and a structured `W118FireConditionV1` (packaged / construction-provenance / cutoff) — all driven by an `UpstreamProvenanceSnapshotV1` so W118 is push-button (`W117-T-UPSTREAM-DERIVED-CONSTRUCTION-PIPELINE-SHIPS`); the certification verdict re-derives `NO_CERTIFIABLE_STRONGER_MODEL` (decision CID `258b6ed7…`, byte-identical to W114/W115/W116; result CID `c3c60483…`); **$0 NIM, no pilot**. The two retirements (W89, W105) STAND, W117 adds NONE; the contamination-confound is UNCHANGED (W117 tests construction supply, not the confound); `COO-9` stays lead; **W118 fires the moment a packaged `release_v7`+ is admitted, OR an LCB-PUBLISHED post-v6 construction provenance (a dataset revision/commit/PR with post-2025-04 functional problems, OR a published collection pipeline + problem-id manifest) enables a B1∧B2 reproducible ≥30-problem post-cutoff slice, OR a reachable stronger-than-Maverick model discloses a primary-KNOWN cutoff month ≤ 2025-01** — until then the bounded ceiling STANDS and resistant-code NIM is BLOCKED. 27th consecutive preflight/earn-discipline validation (W93–W117); stable boundary preserved (no version bump; no PyPI; `coordpy/__init__.py` untouched; 1 new explicit-import-only module + 1 script + 13 new tests = 81 across W113–W117). Prior milestone: **W116 — upstream instrument-supply ATTACK (LIVE; four upstream surfaces) + primary-source model-cutoff ATTACK + durable upstream-ADMISSION pipeline → NO admissible new instrument beyond `release_v6`, NO certifiable stronger model ($0 NIM); `mistralai/mistral-small-4-119b-2603` CONFIRMED REAL with a primary card disclosing NO cutoff** on 2026-05-30 — a **gated branch milestone with THREE lanes (α LIVE upstream instrument-supply attack, β primary-source model-cutoff attack, γ upstream-admission pipeline/graphify/truth), NOT another exposed rerun and NOT another same-scale resistant reflexion rerun on the same `release_v6` slice**. W116 went one level UPSTREAM of `release_v6` and re-verified the LiveCodeBench frontier at FOUR authoritative surfaces (lite file tree highest = `test6.jsonl`, no `test7`+; loader `code_generation_lite.py` `ALLOWED_FILES` `v_list=[v1..v6]` with `release_latest`→`release_v6`; full `code_generation` dataset README `release_v6`=May-2023..Apr-2025; GitHub README no v7) ⇒ **NO admissible new instrument** (the "planned v7" is non-primary + INADMISSIBLE under the pre-committed A1..A5 rule); and re-checked official cutoffs ⇒ no reachable stronger-than-Maverick model has a primary-KNOWN cutoff ≤ 2025-01 (Qwen3-Coder-480B + DeepSeek-V4-pro UNKNOWN; **`mistralai/mistral-small-4-119b-2603` now CONFIRMED REAL — 119B MoE, 2026-03-16 — with its official Mistral docs card + announcement disclosing NO cutoff**; the only figure is a non-primary aggregator "2025-06" that is itself C2-exposed; Mistral-Small-3.2-24B KNOWN ~Oct-2023 but sub-70B/C3). Shipped `coordpy.upstream_instrument_admission_v1` (explicit-import-only; reuses the W113 registry + W114 `certify_model_v1` + W115 `run_frontier_certification_v1`, no duplication): a pre-committed admissibility rule, a multi-surface upstream-change detector, a certifiable-slice builder, a four-way disclosure-status matrix, and a structured `W117FireConditionV1` — all driven by a `UpstreamSupplySnapshotV1` so W117 is push-button (`W116-T-UPSTREAM-ADMISSION-PIPELINE-SHIPS`); the certification verdict re-derives `NO_CERTIFIABLE_STRONGER_MODEL` (decision CID `258b6ed7…`, byte-identical to W114/W115; result CID `193164c4…`); **$0 NIM, no pilot** (`W116-L-UPSTREAM-SUPPLY-NO-ADMISSIBLE-NEW-INSTRUMENT-FOUR-SURFACE-CAP` + `W116-T-MISTRAL-SMALL-4-CONFIRMED-REAL-PRIMARY-NO-CUTOFF`). The two retirements (W89, W105) STAND, W116 adds NONE; the contamination-confound is UNCHANGED (W116 tests certification supply, not the confound); `COO-9` stays lead; **W117 fires the moment `detect_upstream_change_v1` flags an admissible change** (a newer admitted `release_v7`+ / `release_latest` re-point / new upstream functional dataset with ≥30 post-frontier problems for a KNOWN-cutoff stronger model, OR a reachable stronger-than-Maverick model disclosing a primary-KNOWN cutoff month ≤ 2025-01) — until then the bounded ceiling STANDS and resistant-code NIM is BLOCKED. 26th consecutive preflight/earn-discipline validation (W93–W116); stable boundary preserved (no version bump; no PyPI; `coordpy/__init__.py` untouched; 1 new explicit-import-only module + 1 script + 16 new tests). Prior milestone: **W115 — external-frontier refresh (LIVE) + durable future-fire certification/instrument-supply pipeline → external frontier UNCHANGED, NO certifiable stronger model ($0 NIM)** on 2026-05-29 — a **gated branch milestone with THREE lanes (α LIVE external-frontier refresh, β future-fire certification/instrument-supply pipeline, γ claim/graphify/readiness), NOT another exposed rerun and NOT another same-scale resistant reflexion rerun on the same `release_v6` slice**. W115 RE-VERIFIED the external frontier LIVE from PRIMARY sources: the latest LiveCodeBench release is STILL `release_v6` (HF dataset file tree highest = `test6.jsonl`, "add v6" ~1yr ago, **no `test7`+**; functional frontier 2025-04-05 UNCHANGED), and NO reachable model stronger than Maverick has a KNOWN cutoff ≤ ~Jan-2025 (Qwen3-Coder-480B official HF card states NO cutoff; **the DeepSeek V4 official model card now EXISTS — published 2026-04-27, Pro = 1.6T/49B — but discloses NO training cutoff** ⇒ still UNKNOWN; Mistral-Small-4 UNKNOWN; Maverick Aug-2024 KNOWN but already SETTLED at W113) ⇒ both binding conditions still fail ⇒ the certification verdict re-derives `NO_CERTIFIABLE_STRONGER_MODEL` (decision CID `258b6ed7…`, byte-identical to W114), **$0 NIM, no pilot**. The one genuine external change since W114 (the DeepSeek V4 card's publication) does NOT move the verdict — it discloses no cutoff (`W115-L-EXTERNAL-FRONTIER-UNCHANGED-NO-CERTIFIABLE-SLICE-REVERIFIED-CAP`). Lane β shipped `coordpy.frontier_certification_pipeline_v1` (explicit-import-only; reuses the W113 registry + the W114 `certify_model_v1`/`decide_certification_v1` gate + the loader's `LIVECODEBENCH_KNOWN_RELEASES`, no duplication — graphify-confirmed 2-hop reuse chain) + `scripts/run_w115_frontier_certification_v1.py`: a latest-official-release detector (`newer_release_available=False`), a generalised frontier-date summary + threshold table (max KNOWN cutoff month admitting a ≥30 slice = 2025-01), a per-model go/no-go matrix, a disclosure-consistency guard, and a structured W116 fire condition — all driven by a `FrontierSnapshotV1` (external state as DATA) so W116 is push-button (`W115-T-FUTURE-FIRE-CERTIFICATION-PIPELINE-SHIPS`); the script re-verifies the histogram against the SHA-pinned corpus (`sha_ok`+`histogram_match` ✓). The two retirements (W89, W105) STAND, W115 adds NONE; the contamination-confound is UNCHANGED (W115 tests certification supply, not the confound); `COO-9` stays lead; **W116 fires when the pipeline trigger flips** (a newer admitted `release_v7`+ with ≥30 post-frontier functional problems for a KNOWN-cutoff stronger model, OR a reachable stronger-than-Maverick model disclosing a KNOWN cutoff month ≤ 2025-01) — until then the bounded ceiling STANDS and resistant-code NIM is BLOCKED. 25th consecutive preflight/earn-discipline validation (W93–W115); stable boundary preserved (no version bump; no PyPI; `coordpy/__init__.py` untouched; 1 new explicit-import-only module + 1 script + 10 new tests). Prior milestone: **W114 — bounded-ceiling registration + newer-certifiable-post-cutoff-instrument attempt → NO stronger-than-Maverick model is certifiable on the latest real data** on 2026-05-29 — a **gated branch milestone with THREE lanes (α bounded-ceiling registration, β genuinely-different-axis certification-supply analysis, γ per-model certification layer), NOT another exposed rerun and NOT another same-scale resistant reflexion rerun**. W114 registers the hardened bounded ceiling as the honest code-superiority floor (two retirements, both contamination-EXPOSED-HumanEval-family at 70B; resistant superiority **0 clean across BOTH 70B and Maverick scales**; the W112 +10 pp was EXPOSURE) AND verifies — from PRIMARY sources — that NO new certifiably-resistant slice can be built for any reachable model stronger than Maverick: the latest LiveCodeBench release is `release_v6` (HF file tree: `test6.jsonl` is the highest-numbered; the FUNCTIONAL subset is 63 problems, all dated **2025-01-11..2025-04-05**, instrument frontier **2025-04-05**); a ≥30 functional resistant slice requires a KNOWN cutoff ≤ ~Jan-2025; Llama-4-Maverick is the ONLY reachable model with a KNOWN cutoff (Aug-2024, official Llama-4 model card) and is already SETTLED here (W113 resistant FAIL ⇒ a second pilot is redundant); every reachable stronger-than-Maverick frontier model has an OFFICIALLY UNDISCLOSED cutoff (Qwen3-Coder-480B + DeepSeek-V4-pro + Mistral-Small-4 model cards/blogs state none — verified 2026-05-29) and, where estimable, a cutoff at/after the Apr-2025 frontier ⇒ the **resistant-instrument frontier has AGED OUT relative to the reachable model frontier** ⇒ certification verdict `NO_CERTIFIABLE_STRONGER_MODEL`, decision CID `258b6ed7…`, **$0 NIM** (`W114-T-BOUNDED-EXPOSED-CODE-CEILING-REGISTERED` + `W114-L-RESISTANT-INSTRUMENT-FRONTIER-LAGS-MODEL-FRONTIER-CAP` + `W114-T-STRONGER-MODEL-CUTOFFS-OFFICIALLY-UNDISCLOSED`). No pilot earned. The two retirements (W89, W105) STAND, W114 adds NONE; `COO-9` stays lead; **W115 fires only when a resistant FUNCTIONAL instrument with ≥30 problems dated strictly after a reachable frontier model's KNOWN cutoff exists** (a future LCB release_v7+ with post-Apr-2025 functional problems AND a frontier model disclosing a KNOWN cutoff < those problems) — until then the bounded ceiling STANDS and resistant-code NIM is BLOCKED on the missing instrument. 24th consecutive preflight/earn-discipline validation (W93–W114); stable boundary preserved (no version bump; no PyPI; `coordpy/__init__.py` untouched; 1 new explicit-import-only module + 1 script + 12 new tests). Prior milestone: **W113 clean resistant-for-Llama-4 benchmark construction + earned Maverick pilot → exposure confirmed** on 2026-05-29 — a **gated branch milestone with THREE lanes, NOT a new tournament and NOT a confounded exposed rerun**. W113 did the move W112 set up: it BUILT a benchmark VERIFIABLY contamination-resistant for Llama-4-Maverick (machine-checkable rule `contest_date > 2024-08-31`; the date-filtered LiveCodeBench `release_v6` functional subset is **63/63 resistant**, all dated 2025-01..04, **0 ambiguous/missing dates**; the deterministic resistant 30-slice CID `2afc318c…` is IDENTICAL to the W108 slice, so only model scale varies; preflight verdict CID `6f30990c…`), EARNED the cheapest honest Maverick pilot, and RAN it (1 seed × 30 × K=5 = 330 calls): **A0 30.00 / A1 50.00 / B 50.00 %; B − A1 = +0.00 pp; 7/9 gates; MLB-1 = 63.33 % PASS; MLB-2 = 21.05 % FAIL ⇒ `FAIL` → `EXPOSURE_CONFIRMED`** (reflexion genuinely invoked — MORE than 70B — but rescued 4 / regressed 2 / net 0). The **W112 +10.00 pp on EXPOSED BigCodeBench COLLAPSED to +0.00 pp on resistant LiveCodeBench at the same Maverick scale ⇒ the +10 pp was contamination EXPOSURE, not a capability reopening** (`W113-T-STRONGER-MODEL-RESISTANT-CODE-EXPOSURE-CONFIRMED`). Within the SAME model + mechanism the margin flips +10.00 (exposed) → +0.00 (resistant) purely on slice resistance — the **sharpest contamination dissociation in the programme** (corroborated by Maverick's resistant-slice A1 50 % being BELOW 70B's 63.33 %, removing the capability alternative); STRENGTHENS the confound a fourth time, NOT proof (`W113-T-CONTAMINATION-CONFOUND-STRENGTHENED-WITHIN-MODEL`). Resistant same-budget code superiority is now **0 clean across BOTH scales** (`W113-L-STRONGER-MODEL-RESISTANT-SUPERIORITY-FAIL-CAP`). Lane β: all three reachable tier-2 stronger models (Qwen3-Coder-480B, DeepSeek-V4-pro, Mistral-Small-4-119B) have UNKNOWN 2025-2026 cutoffs ⇒ NO certifiably-resistant slice on the pinned corpus ⇒ `$0` tier-2 NIM (`W113-L-TIER2-NO-CERTIFIABLE-RESISTANT-SLICE-ON-PINNED-CORPUS-CAP`). The two retirements (W89, W105) STAND, W113 adds NONE; `COO-9` stays lead; W114 = accept the bounded contamination-EXPOSED claim + pursue a GENUINELY DIFFERENT axis. 23rd consecutive preflight/earn-discipline validation (W93–W113); stable boundary preserved (no version bump; no PyPI; `coordpy/__init__.py` untouched; 3 new explicit-import-only modules + 3 scripts; 31 new tests). Prior milestone: **W112 stronger-model resistant-code gate + NIM-free M3-strengthening structural kill** on 2026-05-29 — a **gated branch milestone with THREE lanes, NOT a new benchmark tournament and NOT a 70B rerun**. The cross-scale-UP gate went LIVE for the first time: `meta/llama-3.1-405b-instruct` was 404 for the SIXTH consecutive time, but `meta/llama-4-maverick-17b-128e-instruct` (Aug-2024 cutoff) was reachable + same-budget-comparable; the earned BigCodeBench pilot on the EXACT W110 30-slice reopened B−A1 to **+10.00 pp (9/9 gates; MLB-2 37.5 % PASS; MLB-1 26.67 % FAIL ⇒ `PASS_NON_MECHANISM_DRIVEN`)** vs 70B's +0.00 pp — BUT BigCodeBench 2024-06 is contamination-EXPOSED for that Aug-2024 cutoff (a structural twin of the W109 APPS exposed control; A0 73.33 % identical), so it is an EXPOSED-column result, NOT a clean reopening of contamination-RESISTANT superiority (`W112-T-CONTAMINATION-RESISTANCE-IS-MODEL-CUTOFF-RELATIVE` + `W112-L-STRONGER-MODEL-RESISTANT-SUPERIORITY-NOT-CLEANLY-DEMONSTRATED-CAP`). A $0 harder fair-reachability mining showed the fair M3-strengthening design space is structurally sub-floor (reliably-reachable ceiling 8.3 %; 58 % mock/fixture-coupled; `W112-T-FAIR-M3-STRENGTHENING-CEILING-SUB-FLOOR`). The two retirements (W89, W105) STAND, W112 adds NONE, the contamination-confound is STRENGTHENED a third time (first within-benchmark flip) but NOT proven; `COO-9` stays lead; W113 = a benchmark verifiably resistant FOR Llama-4 (problem dates > Aug-2024). Prior milestone: **W111 different-MECHANISM
