...
> `docs/HONEST_FRAMING_POST_W87.md`. Last touched: **W131 — code-competent MODEL supply census + stronger-generator hard-cluster dev bench (W129 selector FIXED) + targeted resistant probe only if earned (3 lanes; operator-greenlit EXPOSED dev spend; ≈250 NIM dev). W130 found the hard-cluster ceiling generation-bound + capability-bound, so W131 attacked the MODEL-capability axis directly. LANE α (NIM-free $0): `coordpy.code_model_supply_census_v1` unified the THREE supply surfaces (local-HF / local-Ollama / hosted-NIM) into a machine-checkable matrix cross-cut with the cutoff gate (census_cid d360c117) ⇒ local-HF DEAD (no torch/transformers under Py3.14.5); 13 reachable STRONGER-than-Maverick code models on the NIM catalogue (Qwen3-Coder-480B / DeepSeek-V4-pro / Qwen3.5-397B / Mistral-Large-3-675B / GLM-5.1 …) but ALL UNKNOWN-from-primary on cutoff ⇒ DEV_ONLY, FRONTIER_ELIGIBLE=NONE ⇒ `W131-T-FRONTIER-ELIGIBLE-SUPPLY-IS-CUTOFF-DISCLOSURE-BOUND` (the model-axis supply gap MOVED from W124's 'no strong code model exists/loads' to 'no PRIMARY-KNOWN-cutoff stronger model on the ICPC family' = disclosure, not existence); a load-bearing parsing-fairness fix `normalize_fence_v1` stops the Qwen-Coder ` ```\npython ` fence quirk from crashing the audited `extract_candidate_code_v1` (NOT a capability lever; raw output preserved in the sidecar). LANE β (EXPOSED dev spend): `coordpy.generator_model_bench_v1` swapped the strongest reachable code model — `qwen/qwen3-coder-480b-a35b-instruct` (≈28× Maverick's active params) — through the FULL B1_PLAIN / B2_RDIV(GG1) / B3_GG2 / B4_GGLEAD slate at MATCHED K=5 with the W129 NIM-free SOLEAD selector held FIXED downstream so GENERATION is the only variable (220 NIM valid, 0 same-budget violations) ⇒ exactly the W130 `doubleup` solve (B1_PLAIN) and **0 genuinely-new** solves; B2/B3/B4 method arms added 0; NOT EARNED (1 < the +2 bar) ⇒ `W131-L-MODEL-AXIS-GENERATION-CEILING-DEV-BENCH-CAP` (empirical — a ≈28× larger frontier code model does NOT lift the hard-cluster generation ceiling at the reachable rungs; the MODEL-axis sibling of the W123→W130 cap taxonomy) + `W131-T-STRONGER-MODEL-DOES-NOT-CRACK-NEW-HARD-CLUSTER-PROBLEMS` (the dominant WRONG_ALGORITHM_* failures unmoved by a frontier code model = capability-bound, reinforcing `W130-T-ADMISSIBLE-SKETCH-IS-CAPABILITY-NOT-GENERATION-FIXABLE`). Stored regression trio PRESERVED (0 new mis-commits; the 480B even committed a CORRECT `pawnshop` via SINGLE_SURVIVOR; role-diverse reached correct `blueberrywaffle`/`sunandmoon` but the fixed selector correctly ABSTAINED = W128/W129 discipline holds under a stronger generator). Local rung: `qwen2.5-coder:32b` reachable + code-competent (smoke PASS) but CPU-bound on this host (`size_vram=0`, ≈1 tok/s) = throughput-impractical ⇒ the local lane cannot practically supply a stronger-than-Maverick generator; $0 local rung best-effort on `qwen2.5-coder:7b`. LANE γ: T1 FALSE ⇒ NO targeted resistant probe ⇒ **$0 resistant NIM**; T2 moot (FRONTIER_ELIGIBLE=NONE + the contamination/memorization rule makes any DEV_ONLY EXPOSED earn resistant-ineligible); stronger-model gate `NO_CERTIFIABLE_STRONGER_MODEL`, decision CID 258b6ed7 invariant, {KNOWN:1, UNKNOWN:4}; W123–W130 caps stay closed. **W89 (+5.56) + W105 (+7.00) STAND as the only two retirements** (none added, none retired). 2 new explicit-import-only modules + 3 scripts + 17 tests; RUNBOOK_W131 locked BEFORE any NIM. graphify refreshed START (`f0de740`) + END. `COO-9` stays lead; W132 = a PRIMARY-KNOWN-cutoff stronger model on the ICPC family (none currently — the gap is cutoff DISCLOSURE) / a resistant-by-construction instrument dated after such a model's cutoff / a genuinely different axis. Decision CID 258b6ed7 invariant. ultracode OFF. No version bump; no PyPI; `coordpy/__init__.py` untouched** · PRIOR: **W130 — generation-ceiling attack on the hard ICPC clusters + same-family EXPOSED dev bench + targeted resistant probe (3 lanes; operator-greenlit EXPOSED dev spend; 275 NIM dev). W129 proved the binding cap is GENERATION not selection (committed ≤ pool ceiling = baseline+1 < +2); W130 attacked the GENERATOR directly with the W129 selector held FIXED downstream. LANE α (NIM-free $0): `coordpy.generator_failure_atlas_v1` reconstructed the FULL old W128/W129 candidate pool (plain∪scaffold∪rda from the stored W128 sidecar; replay_misses=0 — perfect reconstruction), graded every candidate with a mechanical failure signature (the official execution path) + an OFFLINE accepted-algorithm admissibility heuristic (NEVER model-facing), and classified each hard-cluster problem's dominant GENERATOR-failure mode ⇒ 8/11 are pool-DEAD (3 WRONG_ALGORITHM_ADMISSIBLE + 2 HIDDEN_EDGE_STATE_MISS + 2 WRONG_ALGORITHM_NO_SKETCH capability + 1 PARSE_IO_FAILURE); the 2 pool-bearing misses are SELECTION_FIXABLE (W129's domain); implementation/parsing is NOT the bottleneck (every candidate parses; 7/8 pool-dead have ZERO public survivors = wrong algorithm). LANE β (EXPOSED dev spend ALLOWED): `coordpy.stronger_generator_slate_v1` — a stronger SAME-BUDGET generator slate (GG1 complexity-gated handoff / GG2 counterexample-to-rewrite / GG3 family anti-pattern coach / GG4 budget router / GGLEAD=GG1→GG2; every arm spends EXACTLY K=5 model calls, the gate/router/grading NIM-free) on the SAME 11 EXPOSED hard-cluster targets (`hard_dev_target_cid 546c1466…`, byte-identical to W128) with the W129 NIM-free SOLEAD selector held FIXED downstream so GENERATION is the only variable (275 NIM) ⇒ exactly **1 NEW pool solve**: GG2's in-loop PUBLIC-digest-driven REWRITE cracked `doubleup` (HIDDEN_EDGE, adhoc_math) — a problem the ENTIRE old W128/W129 pool missed — producing a structurally-new candidate the fixed selector committed and that passed secret leakage-clean ⇒ `W130-T-COUNTEREXAMPLE-REWRITE-LIFTS-ONE-HIDDEN-EDGE` (empirical — the FIRST W120–W130 GENERATION crack of a pool-DEAD problem; W128 lifted only via diversity on a selection-capped pool, W129 was pure selection). **NOT EARNED** (1 < the locked +2 earn bar; does not span ≥2 families/modes) ⇒ `W130-L-GENERATION-CEILING-DEV-BENCH-CAP` (empirical — a stronger same-budget generator does NOT lift the hard-cluster generation ceiling to the earn bar at the Maverick scale + K=5; the GENERATION-line sibling of the W123→W129 cap taxonomy). The dominant generator-fixable mode — the 3 WRONG_ALGORITHM_ADMISSIBLE problems (familyvisits/foodprocessor/hilbertshedgemaze, where a sketch NAMED the accepted technique family) — was cracked by NO arm ⇒ `W130-T-ADMISSIBLE-SKETCH-IS-CAPABILITY-NOT-GENERATION-FIXABLE` (empirical — the atlas idiom-overlap 'admissible' over-counts generator-fixability; a named technique ≠ a correct algorithm = capability-bound). GG1/GG3/GG4/GGLEAD added 0; GG3 killed (its family coach tripped the contiguous-block boilerplate leakage guard on foodprocessor+pawnshop, 0 solves) ⇒ `W130-T-ANTI-PATTERN-COACH-RISKS-BOILERPLATE-LEAKAGE` (mechanically-checked). Stored regression trio (blueberrywaffle/pawnshop/sunandmoon) PRESERVED (0 new SELECTION mis-commits; the W129 abstain discipline intact; the GG-generated pawnshop pool was even falsifier-separable ⇒ cashed out, not mis-committed). Controls PASS (gg1_gate complexity / gg2_rewrite); the hosted-controller literal bridge KILLED as fake-different (the cache planner is efficiency-only KV-prefix savings, NOT a capability lever — graphify-confirmed no ICPC-code path). LANE γ: T1 (dev EARNED) FALSE ⇒ NO targeted resistant probe ⇒ **$0 resistant NIM**; exposed control NOT bought; stronger-model gate `NO_CERTIFIABLE_STRONGER_MODEL`, decision CID 258b6ed7 invariant, {KNOWN:1, UNKNOWN:4}; W123–W129 caps stay closed. **W89 (+5.56) + W105 (+7.00) STAND as the only two retirements** (none added, none retired). 2 new explicit-import-only modules (`generator_failure_atlas_v1` + `stronger_generator_slate_v1`) + 3 scripts (atlas / gg-dev-bench / gate-recheck) + `tests/test_w130_generator_atlas_and_slate_v1.py` (15 tests, falsifiability-first incl. the complexity gate + GG1/GG2 controls + the controller-mining kill + the leakage-blocks-earn + span-requires-two earn-gate guards); RUNBOOK_W130 pre-registration locked BEFORE any NIM. graphify refreshed START (`4a55d53`) + END. `COO-9` stays lead; W131 = accept the bounded ceiling + the registered generation-ceiling cap / a code-COMPETENT local model (new trajectories at $0) / a primary-KNOWN reachable stronger-than-Maverick model (better generation AND selection) / a genuinely different axis. Decision CID 258b6ed7 invariant. ultracode OFF. No version bump; no PyPI; `coordpy/__init__.py` untouched** · PRIOR: **W129 — public-signal SELECTION ORACLE on the W128 hard-cluster miss pattern + same-family EXPOSED dev-bench validation + targeted resistant probe only if earned (3 lanes; operator-greenlit EXPOSED dev spend; 3 NIM total). W128 localized its cap to the verification-based SELECTION layer (pool 3/11 > committed 2/11); W129 attacked the SELECTOR directly. LANE α (NIM-free): a $0 recon reconstructed the W128 pools by REPLAYING stored generations (keyed by prompt_sha256) and graded every candidate (A0/B1/C2/D3) on PUBLIC+SECRET ⇒ on `pawnshop` the hidden-correct B1 and hidden-wrong A0 are byte-identical on every public sample AND model-derived case (`separable_on_derived=False`), no brute-force sketch — BUT A0 is O(N²) (recomputes `Counter(a[i:j+1])` each step) and B1 is O(N) incremental under the stated N≤300000, so A0 fails secret by TLE (empirically: A0 times out, B1 0.10s on a large instance) ⇒ `W129-T-PAWNSHOP-SELECTION-MISS-IS-COMPLEXITY-VERIFIER-BREAKABLE` (the W128 pawnshop 'selection cap' is a COMPLEXITY signal, NOT a hard public-signal information limit). Built `coordpy.public_signal_selection_oracle_v1` (explicit-import-only): SO1 public-derived falsifier stack (model DERIVED + auto FORMAT-PRESERVING sample mutations + a STRESS scale-up to the parsed constraint; DIFFERENTIAL crash/TLE/format falsification; auto+stress are FALSIFICATION-ONLY, excluded from the agreement signature), SO2 differential-disagreement (REAL bridge to `integrated_synthesis`; ABSTAINS structurally-distinct unanimous ties = the pawnshop discipline), SO3 verifier-final (mines `mathvista_bench_v2`; one model call CHOOSES or ABSTAINS on public-signal evidence), SO4 trust-weighted-abstain (trust = falsifier-survival; realizes the `integrity_trust_coupled_consensus_v1` ABSTAIN concept natively; the substrate `TrustWeightedConsensusController` literal bridge KILLED as latent-specific fake-different via `examine_trust_machinery_applicability_v1` — mechanically-checked, the W128 W79 sibling), SOLEAD composition; a fake-selection positive control PASSES; 29/29 tests. The NIM-free SOLEAD is ABSTAIN-DISCIPLINED: KEEPS the separable `blueberrywaffle` win (SO2 majority) + ABSTAINS the under-determined `pawnshop`+`sunandmoon` ties rather than mis-commit ⇒ **0 SELECTION mis-commits** (the only residual mis-commit is the generation-level single-survivor `doubleup`, where the pool holds no correct candidate) ⇒ `W129-T-ABSTAIN-DISCIPLINE-ELIMINATES-SELECTION-MISCOMMIT`. The generic NIM-free stress scale-up does NOT crack pawnshop (a format-preserving cycle scale-up keeps A0's window small; constructing the adversarial worst-case ≈ solving the problem — we did NOT hand-tune a bespoke worst-case) ⇒ `W129-L-NIMFREE-GENERIC-STRESS-CANNOT-CONSTRUCT-ADVERSARIAL-WORSTCASE`. LANE β0 (3 NIM, EXPOSED dev): the SO3 verifier-final (reads the candidate CODE) CHOSE pawnshop B1 citing exactly the O(N²)-vs-O(N)-vs-constraint argument (REFUTING the prior that the in-loop signal is non-discriminating even for a model judge) + kept `blueberrywaffle` (chose correct C2), but over-abstained `sunandmoon` (a both-correct tie) ⇒ committed 2/11 with 0 selection mis-commits = the SAME committed COUNT as W128 (it TRADES the sunandmoon win for the pawnshop cash-out); the locked β0 gate FAILED on OVER-ABSTENTION, NOT non-discrimination. BINDING finding: committed ≤ the GENERATION pool ceiling (3/11 = baseline+1) < the locked +2 earn bar regardless of selector quality ⇒ R2′ is structurally UNREACHABLE by a selection oracle ⇒ `W129-L-HARD-CLUSTER-GENERATION-CEILING-CAPS-SELECTION-EARN` (the binding cap MOVED from W128's SELECTION to W129's GENERATION ceiling; the honest remaining lever is a stronger GENERATOR, not a better selector; the cap taxonomy gains its sharpest localization). β1 fresh dev bench NOT fired (a +2 is unreachable by selection on a generation-bound bench ⇒ hope-funded). LANE γ: T1 (selector earns) FALSE ⇒ NO targeted resistant probe ⇒ **$0 resistant NIM**; stronger-model gate `NO_CERTIFIABLE_STRONGER_MODEL`, decision CID 258b6ed7 invariant, {KNOWN:1, UNKNOWN:4}; exposed control NOT bought; W123–W128 caps stay closed. **W89 (+5.56) + W105 (+7.00) STAND as the only two retirements** (none added, none retired). 1 new explicit-import-only module + 4 scripts (recon / selector-stored-eval / SO3-stored-probe / gate-recheck) + `tests/test_w129_public_signal_selection_oracle.py` (29 tests, falsifiability-first incl. the fake-selection positive control + the trust-machinery kill + the verifier-prompt no-leakage check); RUNBOOK_W129 pre-registration locked BEFORE any NIM. graphify refreshed START (`0f47803`) + END. `COO-9` stays lead; W130 = accept the bounded ceiling + the registered generation-ceiling cap / a stronger GENERATOR (a code-competent local model / a primary-KNOWN reachable stronger-than-Maverick model that produces fewer near-correct-but-wrong candidates) / a genuinely different axis. Decision CID 258b6ed7 invariant. ultracode OFF. No version bump; no PyPI; `coordpy/__init__.py` untouched** · PRIOR: **W128 — role-diverse algorithm SEARCH on the non-scaffoldable resistant ICPC clusters + same-family hard-cluster dev bench + targeted resistant probe (3 lanes; operator-greenlit EXPOSED dev spend). W127's `RESISTANT_SCAFFOLD_FRESH_GEN_CAP` proved a better SCAFFOLD does not transfer; W128 built a genuinely DIFFERENT mechanism for the atlas's NON-scaffoldable clusters. LANE α (NIM-free): `coordpy.role_diverse_algorithm_search_v1` — a role-diverse algorithm SEARCH (generate → verify → select → abstain at the SAME K=5 budget: 1 ANALYZE producing materially-different role artifacts [spec / invariants / complexity / 4 distinct algorithm sketches / derived counterexamples] + 4 IMPLEMENT, then NIM-free RDA1–RDA4 selection/abstain). The FIRST module bridging the role-diverse SYNTHESIS stack (`integrated_synthesis` / `role_invariant_synthesis`, community 35) to the ICPC resistant-code path (174/329; START 5–8 hops apart, no semantic edge); the W41/W42 abstain-on-divergence decisions are a REAL load-bearing bridge, the W79 substrate-controller literal bridge was examined + KILLED as fake-inapplicable (`W128-T-SUBSTRATE-CONTROLLERS-NOT-CODE-CONSENSUS-APPLICABLE`); fake-diversity detector + positive control. Hard-cluster target = 8 resistant problems (graph_flow 4 + simulation_grid 4, all non-scaffoldable); EXPOSED graph_flow supply = 0 ⇒ `W128-L-GRAPH-FLOW-EXPOSED-SUPPLY-CAP`. LANE β (EXPOSED dev spend ALLOWED): disjoint same-family EXPOSED hard-cluster dev bench (11 targets: simulation_grid 4 / adhoc_math 6 / greedy_scheduling 1; `hard_dev_target_cid 546c1466…`); 3 arms matched budget. The mechanism is REAL — all 11 runs classify genuine REAL-diversity (after a transparently-documented sketch-parser-bug recalibration: markdown `#### SKETCH A:` headers were missed ⇒ 9/11 spurious FAKE_DIVERSE on the first run ⇒ parser fixed `#{0,6}` + clean re-run) — and the diverse SEARCH LIFTS the generation ceiling (pool **3/11** > plain baseline **2/11**, reaching a simulation_grid program i.i.d. sampling missed) — **but NOT EARNED**: RDA4 commits **2/11** (net **+0**: +1 unique `blueberrywaffle` − 1 regression `pawnshop`), tying baseline AND the W127 scaffold line (2/11) ⇒ `ROLE_DIVERSE_HARD_CLUSTER_DEV_BENCH_NOT_EARNED` (failed R1a, net<+2) ⇒ `W128-L-ROLE-DIVERSE-HARD-CLUSTER-DEV-BENCH-CAP`. The load-bearing finding `W128-T-ROLE-DIVERSE-SEARCH-LIFTS-GENERATION-CEILING-BUT-SELECTION-CAPPED`: the bottleneck is the verification-based SELECTION layer (public samples + derived counterexamples, NO hidden-test oracle) — it abstains 7/11 and mis-commits `pawnshop` (the pool HAD a correct program), so the full mechanism nets +0; selection, NOT generation, is the cap (consistent with W125 looks-right-fails-hidden). LANE γ: stronger-model gate `NO_CERTIFIABLE_STRONGER_MODEL`, decision CID 258b6ed7 invariant, {KNOWN:1, UNKNOWN:4}; T1 (dev EARNED) is FALSE ⇒ targeted resistant probe NOT launched (T2 technically matches — the unique solve is simulation_grid — but T1 fails the net bar) ⇒ **$0 resistant NIM**; exposed control NOT bought. $0 resistant; 360 EXPOSED dev calls (30 validation + 165 parser-bug-invalidated + 165 valid). **W89 (+5.56) + W105 (+7.00) STAND as the only two retirements** (none added, none retired). 1 new explicit-import-only module + 3 scripts + `tests/test_w128_role_diverse_algorithm_search_v1.py` (15 tests, falsifiability-first incl. fake-diversity + leakage positive controls + the substrate-controller kill + earn-gate guards, direct-execution + pytest); RUNBOOK_W128 pre-registration locked BEFORE any NIM. graphify refreshed START (`0b323e6`) + END. `COO-9` stays lead; W129 = accept the bounded ceiling + the registered selection-cap / a stronger or code-competent model (better generation AND selection) / a better selection oracle (but the in-loop signal is non-discriminating on this family) / a genuinely different mechanism. Decision CID 258b6ed7 invariant. ultracode OFF. No version bump; no PyPI; `coordpy/__init__.py` untouched** · PRIOR: **W127 — resistant capability atlas + family-specific algorithm-scaffold GENERATION + targeted resistant fresh-generation probe (3 lanes; operator-greenlit EXPOSED dev spend). W126 proved $0 DETERMINISTIC synthesis is dead on the 22 uniformly-unsolved resistant problems (oracle ceiling 0/22 — capability failures); the honest remaining lever is FRESH algorithmic trajectory generation informed by a real diagnosis. LANE α (NIM-free): `coordpy.resistant_capability_atlas_v1` ⇒ the 22 are 95% wrong-algorithm (19 visible / 3 hidden; ≈0 TLE/crash) + algorithmically DIVERSE (top-2 family concentration ≈ 45–50% in both label spaces — NO dominant cluster) + the surface family classifier is THEME-biased (47% concordant with the actual-algorithm reference signal); scaffoldable subset 11/22 ⇒ `W127-T-RESISTANT-FAILURES-ARE-DIVERSE-WRONG-ALGORITHM-NOT-SURFACE`. LANE β (EXPOSED dev spend ALLOWED): `coordpy.family_scaffold_generation_v1` (G1 de-identified scaffold library / G2 family-group retriever / G3 scaffolded FRESH generation / G4 policy) validated on a disjoint same-family EXPOSED dev bench (25 teacher / 8 held-out targets, K=5, 80 NIM calls) ⇒ scaffold 3/8 vs baseline 1/8, net **+2** across 2 families, 0 regressions, leakage-clean ⇒ `EXPOSED_SCAFFOLD_DEV_BENCH_EARNED` (R1). The earn required correcting a boilerplate leakage-guard FALSE POSITIVE (per-line accepted check flagged universal idioms like `n, k = map(int, input().split())`; winning candidates verified structurally-DIFFERENT correct derivations sharing only boilerplate; corrected to a contiguous-block tripwire, positive control preserved, $0 re-grade — outcome-relevant, transparent) — but the earn is the locked MINIMUM (+2 on n=8/K=5) and confounded by exposed-problem memorization / prompt-framing / variance ⇒ `W127-T-EXPOSED-SCAFFOLD-DEV-BENCH-EARNS-WEAKLY-CONFOUNDED`. LANE γ: stronger-model gate `NO_CERTIFIABLE_STRONGER_MODEL`, decision CID 258b6ed7 invariant, {KNOWN:1, UNKNOWN:4}; R1∧R2 (string_processing scaffoldable cluster) earned the smallest honest targeted resistant probe — G3 scaffolded FRESH generation on the 6 string_processing scaffoldable resistant problems (old pool 0/6, 30 NIM calls) ⇒ **0/6 new solves** ⇒ `RESISTANT_SCAFFOLD_FRESH_GEN_CAP` ⇒ `W127-L-RESISTANT-SCAFFOLD-FRESH-GEN-CAP` (the dev-validated scaffold line creates ZERO new resistant solves; the exposed +2 did NOT transfer to the non-memorizable resistant field — most parsimoniously the exposed signal was memorization/variance/framing, NOT scaffold-taught algorithm; the FRESH-GENERATION-lever sibling of W123/W124/W125/W126 caps). Exposed control NOT bought (resistant-first; clean negative). $0 beyond the authorized dev bench (80) + earned probe (30) = 110 NIM calls. **W89 (+5.56) + W105 (+7.00) STAND as the only two retirements** (none added, none retired). 2 new explicit-import-only modules + 5 scripts + `tests/test_w127_capability_atlas_and_scaffold_v1.py` (33 tests, falsifiability-first incl. leakage positive controls, direct-execution); RUNBOOK_W127 pre-registration locked BEFORE the atlas + BEFORE any NIM. graphify refreshed START (`596ddf9`) + END. `COO-9` stays lead; W128 = accept the bounded ceiling + the registered resistant-scaffold cap / a code-competent local model / a primary-KNOWN reachable stronger-than-Maverick model / a genuinely different mechanism on the atlas's non-scaffoldable clusters. Decision CID 258b6ed7 invariant. ultracode OFF. No version bump; no PyPI; `coordpy/__init__.py` untouched** · PRIOR: **W126 — family-adapted repair SYNTHESIS on the official RESISTANT ICPC family + resistant-first $0 synthesis precursor + stronger-model gate (NIM-free, $0): W125 proved the field is GENERATION-CAPPED for $0 *re-routing* among the 11 generations; W126 pulled the only remaining honest lever — *can CoordPy SYNTHESIZE genuinely NEW resistant-code candidates* from the already-paid pool + an official-family teacher corpus, without leaking target answers? LANE α (MAIN, NIM-free): built `coordpy.family_adapted_repair_synthesis_v1` (explicit-import-only) — the FIRST module wiring the UNUSED repair/consensus arsenal (`adversarial_consensus_repair_v1.trust_weighted_consensus_v1` + `executor_grounded_patcher_v1` typed digest) onto the official-ICPC code path (graphify: the controller path + `compose_repair_integrity_pipeline_v1` were 4 hops apart through a trivial `str` node — NO real edge; the new module is a 1-hop `imports_from` bridge). New-trajectory slate: **S1** cross-candidate AST splice, **S2** digest-grounded repair (robust stdin / recursionlimit / entrypoint / yes-no casing; NO answer-fudging), **S3** family-motif harden (idioms mined from 108 EXPOSED-side accepted `.py` / 38 problem-disjoint problems, `corpus_cid 42d108d4…`), **S-CONS** trust-weighted output-consensus dispatcher (a NEW program voting the per-input plurality output — the lever W125 structurally could not express, able to pass a problem whose generations are each correct on a DIFFERENT subset of cases). No-leakage guard is provenance-aware (a secret byte-run already in the source generations is a base-model coincidence, not injection — verified by an emoticons `^_^` false-positive that was corrected; a planted secret absent from provenance still bites). On the **22 uniformly-unsolved** resistant problems: **894** synthesized leakage-clean candidate programs (avg 40.6/problem) + 66 consensus evals, graded on the official secret+sample cases ⇒ **ORACLE CEILING 0/22**, blind **0/22**, families **[]**, 22/22 leakage-clean ⇒ earn gate leakage✓ ∧ P1(≥2 new)✗ ∧ P2✗ ⇒ **`FRESH_RESISTANT_PILOT_NOT_EARNED_SYNTHESIS_DEAD`**. The ceiling is **0 under ORACLE selection** (not merely blind) ⇒ there is NO headroom to miss — the 22 are **capability failures** (Maverick-17B never produced the right algorithm in any of its 11 attempts; the 10–11 distinct-but-uniformly-wrong generations share no complementary-correct fragments), and $0 deterministic recombination/repair/consensus over wrong algorithms cannot manufacture a correct one. LANE β (resistant-first, $0): NOT earned ⇒ **$0 NIM**, no fresh resistant pilot, **exposed control NOT earned and NOT bought** (resistant-first); the new-trajectory hope requires NEW model capability, which a $0 precursor cannot signal. LANE γ: `stronger_model_cutoff_certification_v1` re-affirmed **`NO_CERTIFIABLE_STRONGER_MODEL`**, decision CID **258b6ed7 invariant**, `{KNOWN:1, UNKNOWN:4}`; the W123 battlefield-supply cap + W124 local-encoder cap stay CLOSED. NET: the synthesis slate is **REAL + arsenal-native + contract/leakage-clean** but the resistant field is **SYNTHESIS-CAPPED** for $0 new-trajectory creation ⇒ new limitation `W126-L-RESISTANT-SYNTHESIS-CAP` (the SYNTHESIS-lever sibling of W123 battlefield / W124 encoder / W125 re-routing caps) + `W126-T-SYNTHESIS-OF-CAPABILITY-FAILURES-IS-DEAD-NOT-BLIND-CAPPED`; **W89 (+5.56) + W105 (+7.00) STAND as the only two retirements** (none added, none retired). 1 new explicit-import-only module + 3 scripts + `tests/test_w126_family_adapted_repair_synthesis_v1.py` (19 tests, falsifiability-first, validated by direct execution — local pytest/attrs env broken) + a NIM-free grade-cache recon; RUNBOOK_W126 pre-registration locked BEFORE the precursor. graphify refreshed START (built from `7634ea4`) + END (new bridge edge verified). `COO-9` stays lead; `COO-51`; W127 = accept the bounded HumanEval-family ceiling + the registered synthesis cap / an OPERATOR-GREENLIT fresh hosted pilot funding the new-trajectory hope (explicitly NOT precursor-earned) / a code-competent LOCAL model (new trajectories at $0) / a primary-KNOWN reachable stronger-than-Maverick model. Decision CID 258b6ed7 invariant. ultracode OFF. No version bump; no PyPI; `coordpy/__init__.py` untouched** · PRIOR: **W125 — hosted controller-native code mechanism on the official RESISTANT ICPC family + resistant-first $0 headroom replay + stronger-model gate (NIM-free, $0): the FIRST time the programme mined its UNUSED hosted/controller arsenal (`hosted_router_controller_v12` / `hosted_logprob_router_v12` / `hosted_cache_aware_planner_v12` + `tool_call_substrate_v1` + `executor_grounded_patcher_v1`) as a real code mechanism instead of treating the saturated battlefield (W120–W123) or the weak local encoder (W124) as the last word. LANE α (MAIN, NIM-free): built `coordpy.controller_native_code_mechanism_v1` (explicit-import-only) — the FIRST module bridging the hosted-controller stack to the audited tool-call substrate (graphify-confirmed they were 6 hops apart with NO edge) — a C1/C2/C3 controller-native slate promoting the W124 M6 PATCH/REPLAN/ABSTAIN contract into an executable mechanism; the pre-committed structural fake-different test BITES (reflexion B + a C0 negative control classify `FAKE_DIFFERENT`; C1/C2/C3 classify `REAL`; lead **C3** = audited-tool-plane + digest-router superset, cid `7989655f…`) and all four NIM-free contract checks PASS (audit-chain re-hash+tamper, idempotency, never-reads-secret, routing determinism, same-budget) ⇒ **the hosted controller stack is a REAL new mechanism on code, NOT fake-different**. LANE β (resistant-first, $0): a $0 replay over the 330 already-paid real Maverick generations on the EXACT W120 resistant 30-slice (CID `01bf9ef8…`), re-grading every generation on the official secret+sample cases ⇒ A1 **7/30** (== W120), pool-union (A0∪A1∪B on secret) **8/30** (the ENTIRE real Maverick generation pool reaches only 8/30), `oracle_pool_headroom` **+1** (inside the ±3.34 null band AND needs an oracle), C2 blind-select **7/30** + C3 digest-routed walk **7/30** ⇒ **`blind_selection_headroom = 0`** (a hidden-test-BLIND controller recovers ZERO A1-fail problems); the two killer diagnostics: `reflexion_divergence` **23/30** (reflexion is STUCK almost everywhere ⇒ a digest-router WOULD diverge) but `looks_right_fails_hidden` **= 10** (the only blind in-loop signal — public sample tests — is NON-DISCRIMINATING) ⇒ there is **nothing to route TO** ⇒ earn gate E1✓ E2✓ **E3a✗ (0<2)** E3b✓ ⇒ **`FRESH_RESISTANT_PILOT_NOT_EARNED_HEADROOM_CAP`**, $0 NIM, no fresh pilot; exposed control NOT earned (resistant-first, RUNBOOK §8). LANE γ: `stronger_model_cutoff_certification_v1` re-affirmed **`NO_CERTIFIABLE_STRONGER_MODEL`**, decision CID **258b6ed7 invariant**, `{KNOWN:1, UNKNOWN:4}`; the local transformer-native line stays CLOSED (no new local code-model supply). NET: the controller arsenal is **REAL + contract-clean** (mechanism question: YES) but the resistant field is **GENERATION-CAPPED** for $0 re-routing (spend question: NOT earned) ⇒ new limitation `W125-L-RESISTANT-GENERATION-CAP` (the MECHANISM-lever sibling of W123's post-cutoff battlefield-supply cap + W124's local-encoder-supply cap); **W89 (+5.56) + W105 (+7.00) STAND as the only two retirements** (none added, none retired). 1 new explicit-import-only module + 3 scripts + `tests/test_w125_controller_native_code_mechanism_v1.py` (13 tests, validated by direct execution — local pytest/attrs env broken); RUNBOOK_W125 pre-registration locked BEFORE the probe. graphify refreshed START (built from `92ccccfc`) + END. `COO-9` stays lead; `COO-50`; W126 = accept the bounded HumanEval-family ceiling + the registered generation-cap / an OPERATOR-GREENLIT fresh hosted controller pilot funding the new-trajectory hope on the 22 uniformly-unsolved resistant problems (explicitly NOT precursor-earned) / a code-competent LOCAL model (reopens W124 M4 + lets the controller gain be shown locally at $0) / a primary-KNOWN reachable stronger-than-Maverick model. Decision CID 258b6ed7 invariant. ultracode OFF. No version bump; no PyPI; `coordpy/__init__.py` untouched** · PRIOR: **W124 — transformer-native code-intervention line on the matched ICPC family + learned-memory/controller line + hosted gate (NIM-free, $0): the FIRST time the programme ran its unused transformer/substrate arsenal against the code-closure problem instead of treating the saturated battlefield as the only lever. LANE α (MAIN, local): M4 = AST-boundary hidden-state probe on the REAL `distilbert/distilgpt2` (the only locally-loadable transformer; 82M general LM, CPU, torch 1.13.1/transformers 4.28.1) over 1,570 official-`grade_on_secret_v1`-labelled Maverick ICPC generations recovered from the on-disk W120/W121/W122 `*_reflexion_calls.jsonl` (52 pos / 60 problems; negatives subsampled to n=312, seed 124000), vs an 11-feature SURFACE baseline, problem-disjoint grouped CV, pooled OOF ROC-AUC ⇒ **AUC_hidden 0.6345 ≈ AUC_surface 0.6343, Δ=+0.0001 ≪ +0.05 floor** (resistant slice: surface 0.7639 BEATS hidden 0.6991) ⇒ **`M4_CLOSE_BLIP_NOT_A_GAIN`** (`real_signal=False`): distilgpt2 hidden state adds NOTHING over surface confounds for code-correctness ⇒ the transformer-native hidden-state line is BLOCKED AT THE PRECURSOR on this host, blocker = **local code-model-encoder supply** (no code-competent local model; transformers 4.28.1 too old for modern code models AND for the repo's own `transformers_runtime_v1`, which needs ≥~4.36 `attn_implementation`), a hardware/model-supply cap NOT a refutation of the mechanism idea; M5 (learned repair-steering projector) GATED OFF (`NOT_RUN_M4_SIGNAL_POOR`, and local-only since NIM exposes no hidden state); M6 (deterministic tool-call-substrate patch/replan/abstain controller over the executor failure digest, materially different from prose reflexion, never reads the hidden test) ships as an EXECUTABLE text-translatable contract but stays contract-only (no competent local generator). LANE β (learned-memory/controller, mandatory): on the reflexion-rescue decision (139 A1-failed problems, 14 rescue events, 10.07% base) a learned controller scores CV balanced-accuracy **0.502 ≈ chance** (Δ+0.002 ≪ +0.05) ⇒ `differentiable_memory_substrate_v1` / `composed_learned_memory_v1` are synthetic-recall models architecturally mismatched + data-starved here ⇒ **`TOO_SYNTHETIC_NOT_WARRANTED`**. LANE γ (hosted gate / truth): `stronger_model_cutoff_certification_v1` re-affirmed **`NO_CERTIFIABLE_STRONGER_MODEL`**, decision CID **258b6ed7 invariant**, `{KNOWN:1, UNKNOWN:4}` (Maverick KNOWN Aug-2024 certifiable-but-settled; Qwen3-Coder-480B/DeepSeek-V4-pro/Mistral-Small-4-119B-2603/GLM-5 UNKNOWN-from-primary); hosted spend MOOT (no Lane-α gain to translate). NET: **NO hosted Maverick probe earned ($0 NIM)**; **W89 (+5.56) + W105 (+7.00) STAND as the only two retirements** (none added, none retired); new `W124-L-*` limitation carry-forwards localize the blocker to local code-model-encoder supply (the MODEL-axis sibling of W123's post-cutoff BATTLEFIELD-supply cap) and register the learned-memory line as not-warranted; M6 controller contract shipped. 1 new explicit-import-only module (`coordpy.transformer_native_code_intervention_v1`) + 4 scripts + `tests/test_w124_transformer_native_code_intervention_v1.py` (11 tests, validated by direct execution — local pytest/attrs env broken). graphify refreshed START (built from `1f10943d`) + END. `COO-9` stays lead; `COO-49`; W125 = accept the bounded HumanEval-family ceiling + the registered encoder-supply limitation / fire when a code-COMPETENT local model becomes loadable (newer transformers + a small code model, or a reachable GPU) to re-test the M4 precursor on a real code encoder / a primary-KNOWN reachable stronger-than-Maverick model; decision CID 258b6ed7 invariant; no version bump; no PyPI; coordpy/__init__.py untouched** · PRIOR: **W123 — large-n matched ICPC closure ATTEMPT: official-family supply census (NIM-free, live gh-API-verified, census_cid bf7b2efbc724) proves the >=100/field matched battlefield is SUPPLY-UNREACHABLE — RESISTANT hard-capped at 51 raw/~45 tier-1 (4 post-cutoff surfaces RMRC 2024-25/2025-26 + ECNA 2024-25/2025-26, all mined by W120, no 5th, <100 even at 100% yield) while EXPOSED scales past 100 (135 raw/~113 tier-1, 11 pre-cutoff surfaces) => blocked SOLELY on the post-cutoff axis; Lane-beta large-n spend gate stayed shut ($0 NIM, no pilot, no n=30 seeds); Lane-gamma stronger-model gate CLOSED+MOOT {KNOWN:1, UNKNOWN:4}; the n=30 caveat is now POST-CUTOFF-SUPPLY-BOUND (W123-L); W89+W105 STAND (only two retirements, no third); coordpy.icpc_largen_supply_census_v1 + 2 scripts + 7 tests; graphify refreshed START+END; COO-9 lead; COO-48; W124 = accept the bounded HumanEval-family ceiling / a NEW post-cutoff RMRC-ECNA 2026-27+ drop / a primary-KNOWN reachable stronger-than-Maverick model; decision CID 258b6ed7 invariant; no version bump; no PyPI; coordpy/__init__.py untouched** · PRIOR: **W122 — ran the FULL pre-committed matched-family paired-seed CLOSURE on official ICPC (the W121 §9 optional paired-seed tightening PROMOTED to the main empirical lane): seed 120002 then the EARNED tiebreaker seed 120003 on BOTH the EXACT W120 resistant 30-slice (CID 01bf9ef8) and the EXACT W121 exposed 30-slice (CID 32d15db5), Maverick, same grader/evaluator/K, only the seed changed (1320 NIM calls). Seed 120002: RESISTANT +3.33 FAIL / EXPOSED +13.33 PASS_NON_MECHANISM_DRIVEN ⇒ 2-seed B4 ⇒ earned the 3rd seed. Seed 120003: RESISTANT +10.00 PASS_NON_MECHANISM_DRIVEN (9/9, MLB-2 16.7%, 3 rescues letterballoons+bigand+conveyorbeltsushi / 0 regr, merkle adf55ff9) / EXPOSED +10.00 PASS_NON_MECHANISM_DRIVEN (9/9, MLB-2 18.5%, 3 rescues blueberrywaffle+champernownecount+icouldhavewon / 0 regr, merkle d88d025f). FINAL 3-seed means RESISTANT +4.44 (from [+0.00,+3.33,+10.00], OUT of ±3.34 band, in the 3.34–5.00 gap) / EXPOSED +8.89 (from [+3.33,+13.33,+10.00], OUT of band); both all_seeds_clean_pass=false, neither shows_margin ⇒ code-emitted branch AMBIGUOUS_THIRD_PAIRED_SEED_EARNED (B4), caveat_closed=false; B4-after-the-3rd-seed is terminal (NO 4th seed). CLOSURE NOT ACHIEVED: the 2-seed 'resistant null vs exposed popped' asymmetry DISSOLVED — the RESISTANT field also spiked +10.00 — so both fields show rescue-concentrated non-mechanism variance and the matched contrast is unresolvable at n=30 (±10pp from ~3 rescues). The literal single-seed caveat is retired (3 seeds/side) but REPLACED by a small-n-variance limitation; the W121 'weakened' read is NOT multi-seed-confirmed (exposed did not stay flat) and contamination is NOT established (resistant did not stay null) — the 3-seed data further UNDERCUTS a clean contamination read: contamination-RESISTANT code spikes just like exposed ⇒ sampling variance, not exposure; the faint exposed>resistant ordering (+8.89>+4.44) is exposure-consistent but non-mechanism-driven/noisy. LANE β: M3 KILLED NIM-free (m3_exclusive_signal_fraction=0.000 < 0.33 floor; on official ICPC the hidden oracle is SECRET token-diff, denying M3 its expected/actual differentiator ⇒ the same-family ceiling is MECHANISM-ROBUST, not merely reflexion-specific; $0). LANE γ: stronger-model gate STRUCTURALLY CLOSED (the resistant field is anchored to Maverick Aug-2024; only a primary-KNOWN cutoff ≤~Aug-2024 could be resistant-certified, Maverick the unique reachable such model; {KNOWN:1, UNKNOWN:4}; decision CID 258b6ed7 invariant; $0). NO third retirement (W89+W105 STAND, the only two); W122 adds none (every non-FAIL seed is non-mechanism-driven). Shipped coordpy/coordpy_icpc_paired_seed_closure_v1.py (explicit-import-only; reuses the W121 interpreter/constants + W120/W121 benches + verbatim W108 gates ⇒ graphify-confirmed reuse edge interpret_paired_closure_v1→interpret_exposed_vs_resistant_v1) + scripts/run_w122_paired_seed_pilot.py + scripts/run_w122_mechanism_signal_audit_v1.py + tests/test_w122_paired_seed_closure_v1.py (17 tests, falsifiability-first); fixed a W121 doc CID typo (f7cdc917→01bf9ef8). graphify refreshed START (6b05ccd2) + END. COO-9 stays lead; COO-46; W123 = accept the bounded HumanEval-family ceiling / genuinely different axis / escalate to larger n PER FIELD (≥100/field, not more n=30 seeds, NO 4th) OR a primary-KNOWN stronger-than-Maverick model on BOTH matched ICPC battlefields; 32nd consecutive preflight/earn-discipline validation (W93–W122); no version bump; no PyPI; coordpy/__init__.py untouched** · PRIOR: **W121 — built the matched EXPOSED official-ICPC control on the SAME official package family as W120 and ran the same-model same-mechanism contrast (the W120 §10 FAIL-branch "genuinely different axis"; the within-family within-model exposure control the programme had not yet run). Lane α: from the SAME two official `github.com/icpc` families W120 used (`na-ecna-archive` + `na-rocky-mountain-*-public`), the immediately-preceding PRE-cutoff editions of the same regionals (RMRC 2021 2022-03-14 + ECNA 2022-2023 2022-11-12 + RMRC 2022-2023 2023-02-25 + ECNA 2023-2024 2023-11-11) under byte-identical R1/R2/R4..R8 discipline with the date rule FLIPPED (E3: date ≤ 2024-08-31 = EXPOSED) ⇒ `coordpy_icpc_exposed_control_v1` = 42 tier-1 PURE pass-fail EXPOSED tasks ≥ 30 (50 seen, 48 admitted; +5 float +1 custom; 2 custom-no-validator excluded; SHA 653e3682, manifest CID 8acbc7cc, 30-slice CID 32d15db5; grader self-test 30 all-pass / 637 official secret cases, ≥4 per surface; RMRC 2023-2024 EXCLUDED [typed: minimal package, no shipped problem_statement]). Lane β: the W114 gate with C2→C2e (≥30 problems AT OR BEFORE the cutoff) certifies Maverick on the EXPOSED side; tier-2 all UNKNOWN ⇒ not certifiable (mirrors resistant); LOCKED docs/RUNBOOK_W121.md, clean canary, then RAN the EARNED pilot (meta/llama-4-maverick-17b-128e-instruct × EXPOSED 30-slice, 1×30×K5 = 330 NIM calls, ~70 min; same grader/evaluator/K as W120): A0 6.67 / A1 26.67 / B 30.00 %; **B−A1 = +3.33 pp; MLB-1 93.33% PASS / MLB-2 25.00% FAIL; 8/9; verdict FAIL** (net +1 problem: rescues rsamistake+isbnconversion, regression icouldhavewon). Lane γ: shipped `coordpy_icpc_exposed_control_v1` (reuses the W120 classifier/oracle/slice-selector + W114 cutoff registry + W117 run_upstream_construction_v1 ⇒ decision CID 258b6ed7 invariant) + `MatchedFamilyComparisonV1` (differs_only_in_cutoff_side=True; same org/format/grader/tiers/difficulty/model) + the pre-committed three-branch interpret_exposed_vs_resistant_v1 + W122FireConditionV1 + 3 scripts + 16 tests. THE ANSWER: EXPOSED +3.33pp vs RESISTANT +0.00pp (W120) both within the ±3.34pp null band ⇒ the matched-family exposure flip did NOT reproduce the retirement-grade HumanEval-family margins (+5.56/+7.00); the contamination hypothesis predicted a clean exposed margin (absent) while difficulty/family-ease predicted a null even when exposed (matched) ⇒ contamination-confound WEAKENED (first within-family within-model exposure control), difficulty/family-ease implicated, bounded ceiling HARDENS to HumanEval-family-(ease/structure)-specific @ 70B; NOT refuted (faint sub-floor exposure-consistent gradient: exposed +3.33 > resistant +0.00, exposed rescue 25% > resistant 8%; single seed); difficulty comparability empirically supported (exposed A0 6.67% ≤ resistant A0 20.00% ⇒ exposed not easier). NO third retirement (W89+W105 STAND); paired seed NOT earned (null-side of band; W106 discipline); COO-9 lead; COO-45; W122 = accept the hardened bounded ceiling / genuinely different axis OR a primary-KNOWN stronger-than-Maverick model on BOTH matched ICPC battlefields OR (optional) one paired seed on BOTH; 31st preflight/earn-discipline validation (W93–W121); no version bump; no PyPI; coordpy/__init__.py untouched · PRIOR: W120 — CLOSED the official-ICPC count gap on official surfaces only (Route α1 RMRC exclusion audit re-derived all 26 RMRC problems from each official problem.yaml+tree and cleanly corrected draftlottery pure-pass-fail→float [W119 headline-24 holds, composition sharpened]; Route α2 added a NEW official surface icpc/na-ecna-archive [NA East Division 2024-11-11 + 2025-11-10] = 25 post-cutoff Kattis packages, grader self-test 6/6 problems 149/149 cases PASS) ⇒ combined `coordpy_icpc_battlefield_v1` = 45 tier-1 PURE pass-fail resistant tasks ≥ MIN_RESISTANT_SLICE 30 with margin, NO loosening (+3 float +1 custom-with-validator = 49 gradeable; 2 typed exclusions [interactive + custom-no-validator]; snapshot SHA b212866f, manifest CID bf55bb6c, 30-slice CID 01bf9ef8; combined grader self-test 165/165 each surface); certified Maverick (KNOWN Aug-2024 re-verified verbatim from Meta llama4 MODEL_CARD.md, reachable in the 118-model NIM catalogue; C2 flips 24→45 ⇒ FIRST certifiable on this family); tier-2 Qwen3-Coder-480B/DeepSeek-V4-pro/Mistral-Small-4/GLM-5 still UNKNOWN-from-primary; LOCKED docs/RUNBOOK_W120.md, ran a clean canary, then RAN the EARNED pilot (meta/llama-4-maverick-17b-128e-instruct × the deterministic tier-1 30-slice, 1×30×K5 = 330 NIM calls, ~54 min; grader = official secret cases, reflexion feedback = public samples+verdict+stderr only): A0 20.00 / A1 23.33 / B 23.33 %; B−A1 = +0.00 pp; MLB-1 83.33% (25/30) PASS / MLB-2 8.00% (2/25) FAIL; 6/9 gates; verdict FAIL → BOUNDED_CEILING_HOLDS_ON_RESISTANT_ICPC (reflexion genuinely invoked but net-zero: 2 rescues − 1 regression). NO third retirement (W89+W105 STAND, the only two); resistant superiority now 0 CLEAN across FOUR settings (W108 −3.33 / W110 +0.00 / W113 +0.00 / W120 +0.00); the W114–W119 'no certifiable resistant instrument' escape is CLOSED — we BUILT it, certified Maverick, ran it, and the mechanism did NOT transfer; contamination-confound STRENGTHENED (4th + cleanest resistant null on a ≥30 grader-clean OFFICIAL post-cutoff battlefield) but NOT proven (single seed 120001; ICPC-vs-HumanEval difficulty + Python-TLE floor unexcluded); decision CID 258b6ed7 invariant; $0 beyond the one earned pilot; COO-9 lead; COO-44; W121 = accept the now-tested bounded resistant ceiling / genuinely different axis (optional cheap multi-seed to close the single-seed caveat) OR a reachable stronger-than-Maverick model disclosing a primary-KNOWN cutoff; 30th consecutive preflight/earn-discipline validation (W93–W120); no version bump; no PyPI; coordpy/__init__.py untouched**
