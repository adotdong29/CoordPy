# Wevra Results — bundle-contradiction-aware trust-weighted disambiguator (SDK v3.20, W19 family)
...
> byte and full-programme regression at 555 / 555 wevra tests.**
>
...
* **W19-3 (full programme regression).** On R-58 default and on
  ...
  prior wevra tests pass** (450 / 450 in the wevra suite: 405
  pre-existing + 45 new W19 tests = 450 total; broader regression
  555 / 555 across the wevra-prefix suite).
...
Full programme regression: 405 pre-existing wevra tests + 45
new W19 tests = **450 / 450** in the targeted wevra suites;
**555 / 555** across all `test_wevra_*.py` files. The W19
...
```
python3 -m pytest vision_mvp/tests/test_wevra_bundle_contradiction.py
```

Cross-regime regression on the wevra suite:

```
python3 -m pytest vision_mvp/tests/test_wevra_*.py
```
...
* ``vision_mvp/tests/test_wevra_bundle_contradiction.py`` —
