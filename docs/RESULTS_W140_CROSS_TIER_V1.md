...
- **Anchor confirmation — cross-tier usefulness is FAMILY-DEPENDENT; SHARED cross-tier lift CONFIRMED on the overlap; strict ≥2-SHARED-family earn endpoint-blocked on cell-4.** The anchor (`meta/llama-3.3-70b-instruct`, the frontier target) probe ran on the SAME hard cells, and the per-cell A1 reveals that **the two tiers' needed-and-unknown technique sets DIFFER** (the falsifier-first A1≤0.34 gate makes this machine-checkable):
  ...
  | `max_j_minus_i_le@50000` (prefix/suffix) | 50% (knows it) | **0%** | no — anchor-only | _qualifies; T2 endpoint-blocked_ |
  | `kth_smallest_pair_distance@20000` (binary-search-on-answer) | 0% | _endpoint-blocked_ | _pending_ | _endpoint-blocked_ |

  **On the SHARED-difficulty technique (two-deque @ N=30000) BOTH tiers are at A1=25% AND BOTH are lifted +75pp** (T2 and T4) — a **CONFIRMED cross-tier (anchor∧weak) teacher-compiled lift**. On techniques only ONE tier lacks, it is a tier-specific lift (the anchor already knows monotonic-stack ⇒ weak-tier *democratization* there; the anchor lacks prefix/suffix where the 8B doesn't). So **cross-tier usefulness is family-dependent — the tutor lifts EACH tier on the techniques IT lacks, and the cross-tier lift holds on the overlap.** The strict §7b **≥2-SHARED-family** earn needs a 2nd family both tiers lack (cell-4 `kth`), but the NIM endpoint degraded/oscillated mid-run (gen ~6.5s↔~30-36s/call, the dev-crashing instability) ⇒ cell-4 endpoint-blocked. The strict ≥2-shared-family earn is therefore **1-of-≥2 confirmed** (two-deque shared cross-tier +75 both tiers/both arms; the 2nd shared family endpoint-blocked).
