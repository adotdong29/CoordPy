...
- **Anchor confirmation — endpoint-degraded.** The anchor (`meta/llama-3.3-70b-instruct`) probe ran on the SAME hard cells. Cell-1 `subarrays_sum_and_range@30000`: **A1=25% — the FRONTIER model ALSO qualifies (it too struggles on the needed-and-unknown two-deque at scale)**, so this is NOT a saturated/substitute regime at the anchor — a tutor lift there would make it a strict anchor∧weak earn. But the NIM endpoint degraded mid-run (gen latency ~6.5s → ~30s/call, the same instability that crashed the dev run on an HTTP-500), making the full 4-cell anchor run impractical to complete this session. The anchor confirmation is therefore PARTIAL (cell-1 qualifies; full per-cell T2 lift across ≥2 cells is endpoint-blocked).
