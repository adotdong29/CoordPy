...
* **Backward-compat preserved byte-for-byte.** With
  ...
  Full suite regression: **545 / 545 coordpy tests pass** before the
...
``vision_mvp/coordpy/team_coord.py`` (``OutsideWitnessOracle``
...
* **W20-3** *(proved-empirical full programme regression,
  585/585 coordpy tests pass)*. On R-54..R-66 default banks, the
  ...
  suites (442 prior coordpy tests).
...
* ``vision_mvp/coordpy/__init__.py`` — ``SDK_VERSION = "coordpy.sdk.v3.21"``.
...
python3 -m unittest vision_mvp.tests.test_coordpy_outside_information
