...
provenance-stamped **capsule** — never a raw prompt string. Context
Zero is the research programme that produced it.
...
| **Wevra SDK** (`vision_mvp.wevra`) | Profile-driven context-capsule runtime for SWE-bench-Lite-shape banks; `RunSpec` → provenance-stamped report whose root is a sealed `RUN_REPORT` capsule. | **Stable v3** — public contract. |
