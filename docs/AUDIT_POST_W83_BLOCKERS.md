...
| #31 | P1 MoE substrate | Not claimed | STILL OPEN ON SUBSTRATE; PARTIALLY ADVANCED W85 (Mixtral-8x22B reachable via NIM as text-only; substrate-side MoE routing axis still requires self-hosted MoE weights + GPU). Honest carry-forward `W86-L-MOE-SUBSTRATE-NEEDS-WEIGHTS-AND-GPU-CAP`. |
