...
> CoordPy integration boundary for **two-Mac sharded stronger-model
...
* The integration boundary (`MLXDistributedBackend`,
  ...
  (`test_coordpy_llm_backend.py::MLXDistributedBackendWireShapeTests`).
  When Mac 2 returns, the runbook is the path. The CoordPy side
...
distributed-inference statement CoordPy can make right now is
...
> contract test
> ``test_coordpy_scale_vs_structure.py::Phase53AuditOkGridTests``.
...
>
> **Anchor.**
> ``test_coordpy_scale_vs_structure.py::LLMExtractorBackendDuckTypingTests``.
...
>
> **Anchor.**
> ``test_coordpy_scale_vs_structure.py::ParseRoleResponseRobustnessTests``
> (16 cases).
...
| synthetic + Phase-52 noise (`spurious=0.50`, harder noise) | **falsified** (W4-C1 honest reading already disclosed in SDK v3.5 RESULTS_COORDPY_TEAM_COORD.md) |
...
* **CoordPy two-Mac sharded-inference status:** unchanged from SDK
  v3.6. Experimental infrastructure, not product. No
  ``pip install coordpy[mlx_distributed]`` extra.
  ``MLXDistributedBackend`` is a one-class HTTP adapter; the
  cluster bring-up belongs in the runbook.
* **CoordPy single-Mac stronger-model status:** also unchanged.
  ``OllamaBackend`` adapter is the path; no SDK contract change.
* **CoordPy capsule team-coord status:** unchanged. The W4 family
...
* **CoordPy single-run product runtime contract:** unchanged.
...
* **`vision_mvp/coordpy/__init__.py`** — `SDK_VERSION` bumped to
  `coordpy.sdk.v3.7`.
* **`docs/RESULTS_COORDPY_SCALE_VS_STRUCTURE.md`** *(this file)*.
...
# wrote /tmp/coordpy-distributed/phase53_scale_vs_structure_K4.json
```
