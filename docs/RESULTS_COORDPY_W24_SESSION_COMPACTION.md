...
* **Backward-compat preserved byte-for-byte.**
  ...
  * **W24-3-B** (full programme regression). 121/121 coordpy-anchor +
...
land in
``vision_mvp/coordpy/team_coord.py`` (``SessionCompactEnvelope`` /
...
* **W24-3-B** *(proved-empirical full programme regression,
  121/121 coordpy-anchor + capsule + recent-phase tests pass + 33/33
...
| W24-3-B (regression) | ``test_phase69_*`` + ``test_phase70_*`` + ``test_phase71_*`` + ``test_coordpy_capsules`` + ``test_theorems``; pass |
...
* Prior milestone (SDK v3.24 W23):
  ``docs/RESULTS_COORDPY_W23_CROSS_CELL_DELTA.md``.
