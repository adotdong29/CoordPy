...
### 6.1 The strongest current Wevra "multi-agent context" thesis
...
| ----------------------------------------------------- | -------- | ----------------------------------------------------------------------------------- |
| 1. Code anchor in `team_coord.py` + SDK re-export     | ✅       | `BundleAwareTeamDecoder`, `decode_admitted_role_view`, `CAUSAL_CLAIM_KINDS_PER_ROOT_CAUSE` exported from `vision_mvp.wevra`. |
...
| W10-3   | Backward-compat with W7-2 / W8 / W9 on R-54 / R-55 / R-56       | proved-empirical (n=10 each)      | Backward-compat probe in `RESULTS_WEVRA_BUNDLE_DECODER.md` § 3.                             |
...
* Code: `vision_mvp/wevra/team_coord.py`
  (`BundleAwareTeamDecoder`, `CAUSAL_CLAIM_KINDS_PER_ROOT_CAUSE`,
  `_decoded_root_cause`); `vision_mvp/wevra/__init__.py`
  (re-exports).
* Benchmark: `vision_mvp/experiments/phase57_decoder_forcing.py`.
* Tests: `vision_mvp/tests/test_wevra_bundle_decoder.py`.
