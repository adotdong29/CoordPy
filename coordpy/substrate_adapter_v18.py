"""W73 — Substrate Adapter V18.

Strictly extends W72's ``coordpy.substrate_adapter_v17``. V18 adds
three new capability axes that the W73 V18 substrate satisfies and
hosted backends do not:

  * ``replacement_repair_trajectory_cid`` — V18 per-turn content-
    addressed SHA-256 over V17 restart-repair-trajectory CID +
    replacement events + contradiction events + replacement windows
  * ``replacement_after_ctr_per_layer`` — V18 per-layer argmax in
    [0..9] (V17's [0..8] + replacement_after_contradiction_then_rejoin)
  * ``replacement_pressure_gate_per_layer`` — V18 per-layer
    replacement-conditioned throttle

V18 adds a new top tier:

  * ``substrate_v18_full`` — only the W73 V18 in-repo runtime
...
W73_SUBSTRATE_ADAPTER_V18_SCHEMA_VERSION: str = (
    "coordpy.substrate_adapter_v18.v1")

W73_SUBSTRATE_TIER_SUBSTRATE_V18_FULL: str = "substrate_v18_full"

W73_SUBSTRATE_V18_NEW_AXES: tuple[str, ...] = (
    "replacement_repair_trajectory_cid",
    "replacement_after_ctr_per_layer",
    "replacement_pressure_gate_per_layer",
)

W73_SUBSTRATE_V18_CAPABILITY_AXES: tuple[str, ...] = (
    *W72_SUBSTRATE_V17_CAPABILITY_AXES,
...
class SubstrateCapabilityV18:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema": W73_SUBSTRATE_ADAPTER_V18_SCHEMA_VERSION,
    ...
    def cid(self) -> str:
        return _sha256_hex({
            "kind": "substrate_capability_v18",
            "capability": self.to_dict()})


def _decide_tier_v18(caps: dict[str, str]) -> str:
...
def probe_tiny_substrate_v18_adapter(
        *, label: str = "tiny_substrate_v18",
) -> SubstrateCapabilityV18:
    caps = {ax: "yes" for ax in W73_SUBSTRATE_V18_CAPABILITY_AXES}
    return SubstrateCapabilityV18(
        backend_name=str(label),
        backend_url="in-process://coordpy.tiny_substrate_v18",
        capabilities=tuple(
            (ax, caps[ax])
            for ax in W73_SUBSTRATE_V18_CAPABILITY_AXES),
        tier=_decide_tier_v18(caps),
        probe_notes=(
            "tiny in-repo numpy transformer V18 (20 layers, GQA "
            "8q/4kv, RMSNorm, SwiGLU, d_model=64, d_key=8, "
            "vocab=259) + V17 axes + per-turn replacement-repair-"
            "trajectory CID + per-layer replacement-after-"
            "contradiction-then-rejoin label + per-layer "
            "replacement-pressure gate + KV bridge V18 fourteen-"
            "target ridge + cache V16 thirteen-objective ridge + "
            "replay V14 21-regime ridge + replacement-aware routing "
            "head + deep substrate hybrid V18 eighteen-way loop + "
            "multi-agent substrate coordinator V9 + team-consensus "
            "controller V8",
...
def probe_v17_substrate_adapter_as_v18(
        cap: SubstrateCapabilityV17,
) -> SubstrateCapabilityV18:
    base = {ax: val for ax, val in cap.capabilities}
    for ax in W73_SUBSTRATE_V18_NEW_AXES:
        base.setdefault(ax, "no")
    tier = _decide_tier_v18(base)
    return SubstrateCapabilityV18(
        ...
        probe_notes=tuple(cap.probe_notes) + (
            "wrapped from W72 substrate adapter V17",),
    )


def probe_synthetic_v18_adapter(
        *, label: str = "synthetic",
) -> SubstrateCapabilityV18:
    caps = {ax: "no" for ax in W73_SUBSTRATE_V18_CAPABILITY_AXES}
    caps["text"] = "yes"
    return SubstrateCapabilityV18(
        backend_name=str(label),
...
@dataclasses.dataclass(frozen=True)
class SubstrateAdapterV18Matrix:
    probed_at_ns: int
    capabilities: tuple[SubstrateCapabilityV18, ...]

    def by_name(self) -> dict[str, SubstrateCapabilityV18]:
        return {c.backend_name: c for c in self.capabilities}

    def has_v18_full(self) -> bool:
        return any(
            c.tier == W73_SUBSTRATE_TIER_SUBSTRATE_V18_FULL
    ...
    def cid(self) -> str:
        return _sha256_hex({
            "kind": "substrate_adapter_v18_matrix",
            "capabilities": [
                c.cid() for c in self.capabilities],
        })


def probe_all_v18_adapters(
        *, probe_ollama: bool = False,
        probe_openai: bool = False,
        ollama_url: str | None = None,
) -> SubstrateAdapterV18Matrix:
    caps: list[SubstrateCapabilityV18] = []
    caps.append(probe_tiny_substrate_v18_adapter())
    caps.append(probe_synthetic_v18_adapter())
    if probe_ollama:
        oll = probe_ollama_adapter(
            ollama_url=ollama_url
            or "http://localhost:11434")
        v18 = probe_v17_substrate_adapter_as_v18(
            SubstrateCapabilityV17(
