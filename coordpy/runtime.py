"""CoordPy unified mock / real runtime.
...
split. Slice 2 unifies the two behind ``run_sweep(spec)``:
  ...
  * **Real mode — acknowledged**: the LLM-backed sweep runs
    *in-process* against the configured Ollama endpoint. The report
    ...
    path.
  * **Real mode — not acknowledged**: CoordPy refuses to start the
...
def _load_bank(spec: SweepSpec, n_distractors: int):
    from coordpy.extensions.taskbank import get_task_bank
    from coordpy._internal.tasks.swe_bench_bridge import build_synthetic_event_log
...
def _select_sandbox(spec: SweepSpec):
    from coordpy.extensions.sandbox import get_sandbox
...
def _mock_cells(spec: SweepSpec,
    ...
    from coordpy._internal.tasks.swe_sandbox import run_swe_loop_sandboxed
...
def _real_cells(spec: SweepSpec,
    ...
    Lives here in ``coordpy.runtime`` (not in `experiments/phase42`)
    so the CoordPy SDK owns the unified path. Reuses the exact same
    ...
    from coordpy._internal.tasks.swe_patch_parser import (
        ALL_PARSER_MODES, parse_patch_block,
    )
    from coordpy._internal.tasks.swe_bench_bridge import parse_unified_diff
    from coordpy._internal.tasks.swe_sandbox import run_swe_loop_sandboxed
...
def _synthetic_cells(spec: SweepSpec,
    ...
    Used by:
      ...
      * The cross-model parser-boundary research experiment
        ...
        real parser.
    """
    from coordpy.synthetic_llm import (
...
def run_sweep(spec: SweepSpec,
    ...
    ``mpirun``. The CoordPy spine is unchanged — the PROMPT /
...
def _resolve_endpoint(declared: str | None) -> str | None:
    """Apply env-var overrides to a profile's declared endpoint.
      ...
      COORDPY_OLLAMA_URL_MAC1 — overrides profiles whose declared URL
                              matches ``192.168.12.191``
      COORDPY_OLLAMA_URL_MAC2 — overrides ``192.168.12.248``
      COORDPY_OLLAMA_URL      — overrides any endpoint (last resort)
    """
    if declared is None:
        return os.environ.get("COORDPY_OLLAMA_URL")
    if "192.168.12.191" in declared and os.environ.get(
            "COORDPY_OLLAMA_URL_MAC1"):
        return os.environ["COORDPY_OLLAMA_URL_MAC1"]
    if "192.168.12.248" in declared and os.environ.get(
            "COORDPY_OLLAMA_URL_MAC2"):
        return os.environ["COORDPY_OLLAMA_URL_MAC2"]
    if os.environ.get("COORDPY_OLLAMA_URL"):
        return os.environ["COORDPY_OLLAMA_URL"]
    return declared
