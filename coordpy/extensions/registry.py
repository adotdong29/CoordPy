...
Three entry-point groups are searched:
  ...
  * ``coordpy.sandboxes``    → ``register_sandbox(name, factory)``
  * ``coordpy.task_banks``   → ``register_task_bank(name, factory)``
  * ``coordpy.report_sinks`` → ``register_report_sink(name, factory)``
...
_DISCOVERY_GROUPS = {
    "coordpy.sandboxes": register_sandbox,
    "coordpy.task_banks": register_task_bank,
    "coordpy.report_sinks": register_report_sink,
}


def discover_entry_points(*, overwrite: bool = False) -> dict[str, Any]:
    """Scan installed packages for CoordPy extension entry points.

    Returns a summary dict::

        {
          "discovered": {"coordpy.sandboxes": ["my_firecracker", ...], ...},
...
def all_extensions() -> dict[str, list[str]]:
    ...
    return {
        "coordpy.sandboxes": list_sandboxes(),
        "coordpy.task_banks": list_task_banks(),
        "coordpy.report_sinks": list_report_sinks(),
    }
