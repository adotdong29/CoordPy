"""Minimal ReportSink that renders a Wevra product report as Markdown.
...
When Wevra's extension registry first resolves report sinks, it calls
``register()`` below, which asks Wevra to register a factory for a
``ReportSink`` named ``"markdown"``. After that, the sink is usable
from the Wevra CLI (``wevra --report-sink markdown``) and from
...
class MarkdownReportSink:
    """Render a Wevra product report as a short Markdown digest."""
...
def register() -> None:
    """Register the markdown sink factory with Wevra.

    Imported lazily so that simply importing this package does not
    force a transitive import of ``vision_mvp.wevra``. Wevra calls
