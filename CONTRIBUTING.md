...
The public surface is `coordpy.adk` (the library-first front door —
`Agent` / `Runner` / `Session` / `State` / `Memory` / `Artifacts`)
plus the stable `coordpy` SDK (`coordpy.RunSpec` / `coordpy.run` /
`RunReport`, `AgentTeam`, the capsule primitives) and the console
scripts (`coordpy-team`, `coordpy-capsule`, `coordpy-subject`,
`coordpy`, `coordpy-import`, `coordpy-ci`). `dir(coordpy)` lists the
stable surface; research / experimental names ship under
`coordpy.__experimental__` and carry no stability promise.

The capsule lifecycle invariants are load-bearing. Touching one
needs a regression test that names the invariant in its
docstring.

Capsule construction and admission, all in `src/coordpy/capsule.py`:
...
Team-coordination lifecycle, in `src/coordpy/lifecycle_audit.py` and
`src/coordpy/team_coord.py`:
...
|---|---|---|
| `CAPSULE_VIEW_SCHEMA` (`coordpy.capsule_view.v1`) | `src/coordpy/capsule.py` | yes |
| `PROVENANCE_SCHEMA` (`coordpy.provenance.v1`) | `src/coordpy/provenance.py` | yes |
| `PRODUCT_REPORT_SCHEMA` (`phase45.product_report.v2`) | `src/coordpy/_internal/product/runner.py` | yes |
| `CI_VERDICT_SCHEMA` | `src/coordpy/_internal/product/ci_gate.py` | indirectly |
| `IMPORT_AUDIT_SCHEMA` | `src/coordpy/_internal/product/import_data.py` | indirectly |

Don't change any of these without a version bump (procedure below).

`coordpy.__experimental__` (defined in `src/coordpy/__init__.py`) is
...
2. **Update producers.** Find the code that emits the schema
   ...
   producer is `build_manifest` in `src/coordpy/provenance.py`; for
...
4. **Re-run with the smoke profile.** If `local_smoke` itself
   ...
   `src/coordpy/capsule.py`, `src/coordpy/capsule_runtime.py`, or
   `src/coordpy/_internal/product/runner.py`.
...
5. **Run the lifecycle audit.** `coordpy-capsule audit --report
   ...
   `src/coordpy/lifecycle_audit.py`; the audit output names the
   capsule kind and CID that broke the invariant.

## Adding a backend

The supported backend layout is `coordpy.LLMBackend` (a Protocol
defined in `src/coordpy/llm_backend.py`). To add a new one:
...
- Run `./scripts/release/smoke.sh` and confirm it ends with
...
`./scripts/release/smoke.sh` does not enforce this; reviewers should
flag any file matching the private patterns that sneaks into a
PR.

## Releasing

The version is single-source-of-truth in `src/coordpy/_version.py`.
`pyproject.toml` reads it dynamically.

Full runbook: [`RELEASING.md`](RELEASING.md). Quick path:

```bash
./scripts/release/release.sh check     # build, twine-check, smoke-test
./scripts/release/release.sh upload    # then push to PyPI
```
