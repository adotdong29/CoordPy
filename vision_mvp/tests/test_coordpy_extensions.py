"""Contract tests for the CoordPy extension system.
...
class SandboxExtensionTests(unittest.TestCase):
    ...
    def test_builtins_registered_lazily(self):
        from vision_mvp.coordpy import extensions
    ...
    def test_get_builtin_sandbox_satisfies_protocol(self):
        from vision_mvp.coordpy.extensions import (
    ...
    def test_duplicate_registration_rejected(self):
        from vision_mvp.coordpy.extensions import register_sandbox
    ...
    def test_unknown_sandbox_raises(self):
        from vision_mvp.coordpy.extensions import get_sandbox
...
class TaskBankExtensionTests(unittest.TestCase):
    ...
    def test_builtin_jsonl_loader_exists(self):
        from vision_mvp.coordpy.extensions import list_task_banks
    ...
    def test_builtin_jsonl_loader_loads_bundled_bank(self):
        ...
        self.assertEqual(bundle.schema, "coordpy.task_bank.v1")
...
class ReportSinkExtensionTests(unittest.TestCase):
    ...
    def test_builtins_registered(self):
        from vision_mvp.coordpy.extensions import list_report_sinks
        names = list_report_sinks()
        for required in ("stdout", "jsonfile"):
            self.assertIn(required, names)

    def test_jsonfile_sink_writes_report(self):
        from vision_mvp.coordpy.extensions import get_report_sink
    ...
    def test_worked_example_sink_end_to_end(self):
        ...
        from vision_mvp.coordpy.extensions.examples.jsonl_report_sink import (
            JsonlWithMetaSink,
        )
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual(
                meta["provenance_schema"], "coordpy.provenance.v1")


class RegistryDiscoveryTests(unittest.TestCase):

    def test_all_extensions_reports_three_groups(self):
        from vision_mvp.coordpy.extensions import all_extensions
        ext = all_extensions()
        self.assertIn("coordpy.sandboxes", ext)
        self.assertIn("coordpy.task_banks", ext)
        self.assertIn("coordpy.report_sinks", ext)
    ...
    def test_discover_entry_points_returns_shape(self):
        ...
        # shape.
        from vision_mvp.coordpy.extensions import discover_entry_points
        result = discover_entry_points()
        self.assertIn("discovered", result)
        self.assertIn("errors", result)
        self.assertIn("coordpy.sandboxes", result["discovered"])
