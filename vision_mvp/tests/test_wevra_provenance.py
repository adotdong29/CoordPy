"""Wevra provenance manifest tests.

Every Wevra run must carry a provenance block with the fields that
...
class ProvenanceManifestTests(unittest.TestCase):
    ...
    def test_build_manifest_shape(self):
        ...
        m = build_manifest(
            ...
            argv=["wevra", "--profile", "local_smoke"],
        )
        for k in REQUIRED_TOP_LEVEL:
            self.assertIn(k, m, f"missing provenance key: {k}")
        self.assertEqual(m["schema"], "wevra.provenance.v1")
    ...
    def test_runner_emits_provenance_on_every_run(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            prov = rep["provenance"]
            self.assertEqual(prov["schema"], "wevra.provenance.v1")
            ...
            self.assertEqual(
                disk_prov["schema"], "wevra.provenance.v1")
    ...
    def test_provenance_json_is_valid_json(self):
        ...
        self.assertEqual(round_tripped["schema"], "wevra.provenance.v1")


class CLITests(unittest.TestCase):
    """Smoke-test the console scripts via their Python entry points."""

    def test_wevra_cli_version_flag(self):
        from vision_mvp.wevra import _cli
        # --version is a pure stdout path, exit 0.
        rc = _cli._cmd_run(["--profile", "local_smoke",
                            "--out-dir", "/tmp/unused-wevra-version",
                            "--version"])
        self.assertEqual(rc, 0)

    def test_wevra_cli_runs_local_smoke(self):
        from vision_mvp.wevra import _cli
        with tempfile.TemporaryDirectory() as td:
            rc = _cli._cmd_run(["--profile", "local_smoke",
                                "--out-dir", td])
    ...
    def test_wevra_ci_cli_on_fresh_report(self):
        from vision_mvp.wevra import _cli
        with tempfile.TemporaryDirectory() as td:
    ...
    def test_wevra_import_cli_on_missing_file(self):
        from vision_mvp.wevra import _cli
