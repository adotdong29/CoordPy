...
The tests use the public surface only (``from vision_mvp.coordpy
...
class CapsuleContractTests(unittest.TestCase):
    ...
    def test_c2_unknown_kind_rejected(self):
        from vision_mvp.coordpy import ContextCapsule, CapsuleBudget
...
class CapsuleCLITests(unittest.TestCase):
    """Exercise the ``coordpy-capsule`` CLI."""

    def test_cli_cid_and_view(self):
        from vision_mvp.coordpy._cli import _cmd_capsule
        from vision_mvp.coordpy import RunSpec, run
        import contextlib
        import io
        with tempfile.TemporaryDirectory() as td:
