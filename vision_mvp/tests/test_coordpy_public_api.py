"""CoordPy SDK public-surface contract tests.

These tests LOCK the public API of ``vision_mvp.coordpy``. Any symbol
removed or renamed here is a breaking SDK change and must bump the
``SDK_VERSION`` in ``vision_mvp/coordpy/__init__.py``.
"""
...
class CoordPySurfaceTests(unittest.TestCase):
    """Lock the set of public symbols on the ``coordpy`` package."""
    ...
    def test_top_level_importable(self):
        import vision_mvp.coordpy as w
    ...
    def test_sdk_version_is_v3_11(self):
        ...
        # ``vision_mvp.coordpy.team_coord`` (multi-agent coordination
    ...
    def test_cohort_coherence_admission_policy_is_exported(self):
        ...
        # from the top-level ``vision_mvp.coordpy`` namespace. The
        # buffered factory ``from_candidate_payloads`` is the
        # load-bearing constructor for the W7-2 anchor.
        from vision_mvp.coordpy import TeamCohortCoherenceAdmissionPolicy
    ...
    def test_cross_role_corroboration_admission_policy_is_exported(self):
        ...
        # importable from the top-level ``vision_mvp.coordpy`` namespace.
        # The buffered factory ``from_candidate_stream`` is the
        # load-bearing constructor for the W8-1 anchor.
        from vision_mvp.coordpy import TeamCrossRoleCorroborationAdmissionPolicy
    ...
    def test_llm_backend_surface_is_exported(self):
        ...
        # the top-level ``vision_mvp.coordpy`` namespace (additive
    ...
    def test_team_coord_surface_is_exported(self):
        ...
        # be importable from the top-level ``vision_mvp.coordpy``
    ...
    def test_runspec_is_frozen_dataclass(self):
        from vision_mvp.coordpy import RunSpec
    ...
    def test_coordpy_config_frozen_and_env_overridable(self):
        ...
        with self.assertRaises(ValueError):
            CoordPyConfig(sandbox="nonsense")

    def test_coordpy_config_from_env(self):
        from vision_mvp.coordpy import CoordPyConfig
    ...
    def test_run_spec_round_trip(self):
        from vision_mvp.coordpy import RunSpec, run, CAPSULE_VIEW_SCHEMA
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual(
                report["provenance"]["schema"], "coordpy.provenance.v1")
    ...
    def test_product_path_still_works_for_backcompat(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual(
                rep["provenance"]["schema"], "coordpy.provenance.v1")
