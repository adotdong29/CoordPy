"""Phase 12 — 3-leg CASR benchmark.

The central falsifiable test of this project's thesis.

Task: 12 LLM agents each write one function of TinyStore (a mini in-memory
key-value library). The 12 functions compose into one module that must pass
31 automated tests.
...
Usage:
    python -m vision_mvp.experiments.phase12_casr_benchmark
    python -m vision_mvp.experiments.phase12_casr_benchmark --out results_phase12.json
"""
...
from vision_mvp.tasks.library_v2 import (
...
def round2_prompt(specialty: str, own_draft: str,
    ...
    parts = [
        ...
        f"building TinyStore (an in-memory key-value library).",
...
def run(model: str, out_path: str | None, max_retries: int, ablation_seed: int) -> dict:
    ...
    log(f"Tests: 31 (weighted score in [0, 1])")
    ...
    log(f"  full:     {full_tok:>7} prompt tok   score={full_sc:.3f} ({legs['full']['score']['n_passed']}/31)")
    log(f"  casr:     {casr_tok:>7} prompt tok   score={casr_sc:.3f} ({legs['casr']['score']['n_passed']}/31)")
    log(f"  ablation: {abl_tok:>7} prompt tok   score={abl_sc:.3f} ({legs['ablation']['score']['n_passed']}/31)")
    log(f"  round-1 only (no coord): score={round1_score['weighted_score']:.3f} "
        f"({round1_score['n_passed']}/31)")
    log("")
