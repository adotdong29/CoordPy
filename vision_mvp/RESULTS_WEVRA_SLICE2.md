# Wevra Slice 2 — Extension system, unified runtime, real cluster validation

*2026-04-22*

Slice 1 (2026-04-21) shipped the Wevra SDK boundary, provenance
manifest, and console scripts. Slice 2 makes Wevra a genuinely
...
artifact produced through the Wevra surface itself.

---

## 1. What shipped

### 1.1 Extension system (`vision_mvp/wevra/extensions/`)
...
| --------------------- | ----------------- | --------------------- | --------------------------------- |
| Execution boundary    | `SandboxBackend`  | `wevra.sandboxes`     | `in_process`, `subprocess`, `docker` |
| Evaluation input      | `TaskBankLoader`  | `wevra.task_banks`    | `jsonl`                           |
| Post-run emission     | `ReportSink`      | `wevra.report_sinks`  | `stdout`, `jsonfile`              |

Third-party packages extend Wevra in two equivalent ways:

  * In-process: `wevra.extensions.register_sandbox("my_box", MyBackend)`.
  * Declarative: an `entry_points` block in the plugin package's
    `pyproject.toml`; Wevra discovers it via
    `wevra.extensions.discover_entry_points()`.

A worked `ReportSink` example (`JsonlWithMetaSink`) ships under
`vision_mvp/wevra/extensions/examples/` and is exercised end-to-end
by `test_wevra_extensions.py::ReportSinkExtensionTests::test_worked_example_sink_end_to_end`
...
### 1.2 Unified runtime (`vision_mvp/wevra/runtime.py`)
...
`run_sweep(spec)`:
  ...
  * **Real mode, not acknowledged** (default): Wevra refuses to
...
block is now schema-unified under `wevra.sweep.v2`, and the enclosing
...
### 1.3 Production hardening
  ...
  * **Env-driven endpoints**: `WEVRA_OLLAMA_URL_MAC1`,
    `WEVRA_OLLAMA_URL_MAC2`, `WEVRA_OLLAMA_URL` override the
    profile-declared URL at runtime. Resolved inside
    `wevra.runtime._resolve_endpoint`; tested by
    `test_wevra_runtime.EnvEndpointOverrideTests`. No hard-coded
    cluster IP sits on a path that a consumer has to patch.
  * **Release workflow**: `.github/workflows/wevra-ci.yml`
    (SDK contract tests on 3.10/3.11/3.12, console-script smoke,
    `python -m build` sdist+wheel, release on tag).
  * **CHANGELOG discipline**: `CHANGELOG.md` now tracks Wevra SDK
  ...
  * **Operator-grade failure messages**: the unified runtime wraps
    ...
    stack trace out of `wevra.run`.
  * **Provenance remains first-class**: unchanged from Slice 1 — every
    run (mock, real-staged, real-executed) emits a
    `wevra.provenance.v1` manifest.

### 1.4 Cluster-backed Wevra validation

`vision_mvp/artifacts/wevra_slice2_g1/` holds a real ASPEN
...
run(RunSpec(profile="aspen_mac1_coder",
            out_dir="vision_mvp/artifacts/wevra_slice2_g1",
...
100 % on the bundled bank) via the Wevra surface, not the
...
`sweep_result.json`). `wevra-ci` was then run on the report and
...
`wevra.runtime.run_sweep` invokes.
...
block, the sweep cells produced by `wevra.runtime.run_sweep` are
...
**Claim (proved, constructive).** Every call to `wevra.run(spec)`
...
**Proof sketch.** The manifest (schema `wevra.provenance.v1`)
...
sandbox backend available, `wevra.run` is deterministic up to
LLM nondeterminism, which is bounded by `temperature=0.0`
(enforced in `wevra.runtime._real_cells`) and the raw-response
...
### W2-4. Conditions under which Wevra is a true drop-in SDK

**Conjecture (testable).** Wevra is a true drop-in SDK for an
external operator iff the following hold:

  1. **E1** — the operator can install via `pip install wevra` and
     the console scripts `wevra`, `wevra-import`, `wevra-ci` work
  ...
  3. **E3** — the operator can execute a real-LLM sweep against a
     configured endpoint via `wevra.run(RunSpec(...,
     ...
     *satisfied*; canonical artifact
     `vision_mvp/artifacts/wevra_slice2_g1/`.)
  ...
  5. **E5** — the SDK has a release automation path producing
     ...
     `.github/workflows/wevra-ci.yml`; not yet exercised on a
...
product-path gap. The conjecture says: once all six hold, Wevra
...
New Slice 2 test modules (all passing):
  ...
  * `vision_mvp/tests/test_wevra_extensions.py` — 10 tests
    (protocol conformance, registry roundtrip, entry-point
    discovery shape, worked-example sink end-to-end).
  * `vision_mvp/tests/test_wevra_runtime.py` — 10 tests
...
Updated (still passing):
  ...
  * `test_wevra_public_api.py` — `SDK_VERSION` bumped to
    `wevra.sdk.v2`, `REQUIRED_SYMBOLS` extended, `RunSpec` field
    set extended.
  * `test_wevra_provenance.py` — unchanged behavior on v2 runner.
  * `test_phase45_product.py` — unchanged behavior under the
    unified runtime (backwards-compatible at the data level).

Real cluster validation:

  * `vision_mvp/artifacts/wevra_slice2_g1/product_report.json` —
    `profile=aspen_mac1_coder`, `sweep.executed_in_process=true`,
    `sweep.schema=wevra.sweep.v2`, `sweep.model=qwen2.5-coder:14b`,
    `sweep.endpoint=http://192.168.12.191:11434`, 4 instances.
  * `vision_mvp/artifacts/wevra_slice2_g1/ci_verdict.json` —
