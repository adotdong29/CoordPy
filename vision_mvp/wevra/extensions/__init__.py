...
Each extension point has:
  ...
  3. ``entry_points`` discovery under the groups
     ``"wevra.sandboxes"``, ``"wevra.task_banks"``,
     ``"wevra.report_sinks"``.

An external package can add an extension by either
``wevra.extensions.register_sandbox("my_box", MySandbox)`` at import
time, or by declaring an ``entry_points`` block in its own
``pyproject.toml`` that Wevra discovers via
``importlib.metadata.entry_points()``.

This surface is contract-tested
(``vision_mvp/tests/test_wevra_extensions.py``). Any rename or
