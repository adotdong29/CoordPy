"""Layered Wevra API — three tiers for three audiences.
...
This module exposes three thin classes — ``WevraSimpleAPI``,
``WevraBuilderAPI``, ``WevraAdvancedAPI`` — that route to the same
...
# ---------------------------------------------------------------------------
# Tier 1 — WevraSimpleAPI: "one-call" for end users.
# ---------------------------------------------------------------------------


class WevraSimpleAPI:
    """High-level, one-call Wevra surface.
    ...
    **Audience**: end users who want to execute a named profile and
    ...
    Example::
        ...
        api = WevraSimpleAPI()
        report = api.run_profile("local_smoke", out_dir="/tmp/wevra-out")
...
# ---------------------------------------------------------------------------
# Tier 2 — WevraBuilderAPI: fluent configuration for developers.
# ---------------------------------------------------------------------------


@dataclasses.dataclass(frozen=True)
class BuilderSpec:
    """Resolved output of a ``WevraBuilderAPI.build()`` call.

    A plain dataclass so callers can inspect it, serialise it, or
    forward it into whatever runner they already have. ``to_run_spec``
    produces a ``RunSpec`` suitable for ``wevra.run(...)``.
    """
...
class WevraBuilderAPI:
    ...
    **Audience**: application developers who want a programmatic
    ...
    Example::
        ...
        spec = (WevraBuilderAPI()
                .with_profile("local_smoke")
                .with_out_dir("/tmp/wevra-out")
                .with_role("code_writer", budget=4096)
                .with_compression("medium")
                .build())
        report = spec.to_run_spec()  # feed into wevra.run
    """
    ...
    def with_profile(self, profile: str) -> "WevraBuilderAPI":
        self._profile = profile
        return self

    def with_out_dir(self, out_dir: str) -> "WevraBuilderAPI":
        self._out_dir = out_dir
        return self

    def with_role(self, role: str, *, budget: int) -> "WevraBuilderAPI":
        if budget <= 0:
            raise ValueError(f"role budget must be > 0, got {budget}")
        self._roles.append((role, int(budget)))
        return self

    def with_compression(self, level: str) -> "WevraBuilderAPI":
    ...
    def with_heavy_acknowledged(self, ack: bool = True) -> "WevraBuilderAPI":
...
# ---------------------------------------------------------------------------
# Tier 3 — WevraAdvancedAPI: direct access to capsule substrate.
# ---------------------------------------------------------------------------
